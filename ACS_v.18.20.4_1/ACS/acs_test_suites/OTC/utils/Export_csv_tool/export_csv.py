#!/usr/bin/env python
"""
Export csv from ET for convert results
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
                        description="Generates ET cases \
    ...
    parser.add_argument("-d", "--dest",
                        type=str,
                        required=False,
                        default='../ACS_to_TRC/ET/AFT',
                        help="dest top folder. (default is ../ACS_to_TRC/ET/AFT)",)
    ...
    parser.add_argument("-p", "--passwd",
                        ...
                        required=True,
                        help="password",)
    parser.add_argument("-al", "--AFT_Light",
                        action='store_const', const=1,
                        help="Generate AFT Light required CSV format",)
...
def execute_tql(tql, user, passwd):
    ...
    for item in items:
        ...
        full_package_name_module = '.'.join(full_package_name.split('.')[1:])
        full_package_name = '|'.join(full_package_name.split('.')[0:])
        full_case_name = ','.join([full_package_name, item['Name']])
        root_module_name = full_package_name_module.split('.')[0]
        if root_module_name not in result:
            continue
        result[root_module_name].append(full_case_name)
    return result

def dump_csv(file_path, case_names, aft_light):
    """dump csv contents to csv file"""
    try:
        if os.path.exists(file_path) and not aft_light:
        ...
        if not aft_light:
            with open(file_path, 'w') as fd:
                ...
                for case_name in case_names:
                    fd.write(case_name.encode('UTF-8') + "\n")
        else:
            with open(file_path, 'a') as fd:
                for case_name in case_names:
                    fd.write(case_name.split(',')[1].encode('UTF-8') + "\n")
    except Exception, err:
...
if __name__ == '__main__':
    ...
    info("Input ET Query file: " + str(OPTIONS.input_file))
    info("User name: " + OPTIONS.user)
    if OPTIONS.AFT_Light:
        info("Generate AFT LIGHT output csv file: True")
        full_name = os.path.join(OPTIONS.dest,"AFT_Light.csv")
        if os.path.isfile(full_name):
            os.remove(full_name)

    tqls = parse_config(OPTIONS.input_file)
    for platform, tql in tqls:
        info("Processing Package: %s\nET Query: %s" % (platform, tql))
        module_cases = execute_tql(tql, OPTIONS.user, OPTIONS.passwd)

        for module, case_names in module_cases.items():
            if not case_names:
                continue
            if not OPTIONS.AFT_Light:
                name = '%s.csv' % module
                full_name = os.sep.join([OPTIONS.dest, name])
                info("Start to create: %s" % full_name)
            else:
                ind = True
            dump_csv(full_name, case_names, OPTIONS.AFT_Light)
    if ind:
        info("AFT Light CSV formatted file created here: %s" % full_name)
