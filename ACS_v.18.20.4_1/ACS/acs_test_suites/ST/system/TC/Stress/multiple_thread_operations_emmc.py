...
def operate_emmc(a_tid):
	...
	time.sleep(1)
	#read file
	log("read file in emmc...")
	os.system("adb shell \"cat /data/thread_%s.txt\"" %a_tid)
	time.sleep(1)
	#remove file
	log("remove file in emmc...")
...
if __name__ == "__main__":
	if os.path.exists(r'multiple_thread_operations_emmc.log'):
		os.remove(r'multiple_thread_operations_emmc.log')
	logfile = os.getcwd() + "/multiple_thread_operations_emmc.log"
	...
	thread_pool = []
	for i in range(50):
		th = threading.Thread(target = operate_emmc, args = (i, ))
	...
	if checkCrash():
		print "multiple thread operations emmc passed"
	else:
		print "multiple thread operations emmc failed"
