...
def operate_emmc_sdcard(a_tid):
	...
	time.sleep(1)
	duResponce = os.popen("adb shell du /data/thread_%s.txt" %a_tid)
	duOutput = duResponce.read().strip()
	duResult = int(duOutput.split()[0])
	while duResult < 1024000:
		os.system("adb shell \"ls -l /data/ >> /data/thread_%s.txt\"" %a_tid)
		time.sleep(1)
		duResponce = os.popen("adb shell du /data/thread_%s.txt" %a_tid)
		duOutput = duResponce.read().strip()
...
if __name__ == "__main__":
	if os.path.exists(r'multiple_thread_operations_emmc_sdcard_2.log'):
		os.remove(r'multiple_thread_operations_emmc_sdcard_2.log')
	logfile = os.getcwd() + "/multiple_thread_operations_emmc_sdcard_2.log"
