	...
	if not create_file_client("file_a.txt", 1024):
		...
		if not copy_file_client("file_a.txt", "file_b.txt"):
			...
			for i in range(1, 10001):
				if not copy_file_client("file_b.txt", "file_c.txt"):
					log("md5 test failed")
					print "md5 test failed"
				else:
					if not copy_file_client("file_c.txt", "file_b.txt"):
						log("md5 test failed")
						print "md5 test failed"
					else:
...
if __name__ == '__main__':
	if os.path.exists(r'ST_SYST_FS_eMMC_159.log'):
		os.remove(r'ST_SYST_FS_eMMC_159.log')
	logfile = os.getcwd() + "/ST_SYST_FS_eMMC_159.log"
