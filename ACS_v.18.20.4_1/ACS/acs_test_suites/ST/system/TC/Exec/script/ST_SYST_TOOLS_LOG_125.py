...
class ST_SYST_TOOLS_LOG_125(unittest.TestCase):
	logfile = os.getcwd() + "/ST_SYST_TOOLS_LOG_125.log"
	...
	def emmcap_sdapk_aplog(self):
		...
		for line1 in line:
			if 0 > line1.find("aplog:") and -1 < line1.find("aplog"):
				print "/logs/aplog exist will be rmed"
		...
		if False == existflag:
			...
			existd = False
		if False == existd or False == exists:
			return False 
		return True

	def test_emmcap_sdapk_aplog(self):
		self.log("ST_SYST_TOOLS_LOG_125: start!")
		if not self.emmcap_sdapk_aplog():
			self.log("ST_SYST_TOOLS_LOG_125: fail")
			print "ST_SYST_TOOLS_LOG_125: fail"
		else:
			self.log("ST_SYST_TOOLS_LOG_125: pass")
			print "ST_SYST_TOOLS_LOG_125: pass"
		self.log("ST_SYST_TOOLS_LOG_125: end!")
		
if __name__ == '__main__':
	if os.path.exists(r'ST_SYST_TOOLS_LOG_125.log'):
		os.remove(r'ST_SYST_TOOLS_LOG_125.log')
	os.system("adb shell ls /logs/aplog > 1.txt")
	file = open("1.txt")
	line = file.readlines()
	file.close()
