...
class ST_SYST_TOOLS_LOG_127(unittest.TestCase):
	logfile = os.getcwd() + "/ST_SYST_TOOLS_LOG_127.log"
	...
	def emmcapk_sdapk_aplog(self):
		...
		if False == existflag:
			...
			exists = False
		if existd == False or False == exists:
			return False
		return True			

	def test_emmcapk_sdapk_aplog(self):
		self.log("ST_SYST_TOOLS_LOG_127: start!")
		if not self.emmcapk_sdapk_aplog():
			self.log("ST_SYST_TOOLS_LOG_127: fail")
			print "ST_SYST_TOOLS_LOG_127: fail"
		else:
			self.log("ST_SYST_TOOLS_LOG_127: pass")
			print "ST_SYST_TOOLS_LOG_127: pass"
		self.log("ST_SYST_TOOLS_LOG_127: end!")
		
if __name__ == '__main__':
	if os.path.exists(r'ST_SYST_TOOLS_LOG_127.log'):
		os.remove(r'ST_SYST_TOOLS_LOG_127.log')
	os.system("adb shell ls /logs/aplog > 1.txt")
	file = open("1.txt")
	line = file.readlines()
	file.close()
