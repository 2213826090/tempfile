...
# pylama: ignore=E501
import os
from Core.Report.Live.ReportUtils.protocols.Rest import HTTPRESTSession
from Core.Report.ACSLogging import LOGGER_FWK
from Core.Report.scripts.acsTCRZipPush import safe_remove_file
...
PROD_SERVER_API_URL = "https://api.tcr.sh.intel.com/1"
...
class TCRAdapter(HTTPRESTSession):
    ...
    def send_campaign_resource(self, header, payload):
        ...
        response = self.send_resource_event(url=rest_api_url,
                                            ...
                                            headers=headers)
        # Remove the campaign resource if it is correctly sent
        if response and response.get('campaignId') == campaign_id:
            safe_remove_file(os.path.dirname(file2push))
