...
#########################################################################
#
# @filename:    boot_to_bootloader.py
# @description: Reboot to fastboot via the reboot-bootloader adb command
#
#
# @run example:
#
...
while i < 20 :
    ...
    if serialNum in output:
        state = 'bootloader'
        DEVICE.run_cmd("fastboot reboot bootloader", 5)
    ...
    if serialNum in output:
        state = 'android'
        DEVICE.run_cmd("adb reboot bootloader", 5)
...
if state == 'unknown':
    ...
    while i < 10 :
        ...
        if serialNum in output:
            VERDICT = SUCCESS
            # Get device back to android mode 
    ...
    if i == 10 :
        VERDICT = FAILURE
        PRINT_ERROR("Failed to boot to bootloader")
