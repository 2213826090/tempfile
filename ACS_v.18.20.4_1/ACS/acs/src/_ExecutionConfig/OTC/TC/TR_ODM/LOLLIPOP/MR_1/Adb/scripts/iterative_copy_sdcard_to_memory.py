...
for i in range(0, iterations):
    ...
    # copy the file to dev_memory_path
    exec_status, output = DEVICE.run_cmd("adb push {0} {1}".format(gen_path + "/" + file_name, sdcard_memory_path), 3)
    exec_status, output = DEVICE.run_cmd("adb shell cp {0} {1}".format(sdcard_memory_path + "/" + file_name,\
                                        dev_memory_path + "/" + file_name), 3)

    # check the content of the file
    exec_status, output = DEVICE.run_cmd("adb pull {0} {1}".format(dev_memory_path + "/" + file_name,\
