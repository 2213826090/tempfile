...
function check_mounted {
        step0a=$(./scripts/mountstats.sh /mnt/media_rw/sdcard1 vfat)
        step0b=$(./scripts/mountstats.sh /storage/sdcard1 fuse)
...
if [ -z $1 ]
then
        echo "Usage: ./createFolderSDcard.sh <name>"
        exit 1
fi
...
output2=$(adb shell mkdir /storage/sdcard1/$1)
if [ $output2 ]
then
...
if [ -z $output3 ]
then
        echo "FAIL - Folder is not created"
        exit 1
else
        echo "PASS"
fi
