...
class range_rotation_vector(UIATestBase):
    ...
    def setUp(self):
        ...
        super(range_rotation_vector, self).setUp()

    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(range_rotation_vector, self).tearDown()

    def test_range_rotation_vector(self):
        '''
        check the max range of rotation_vector
        '''
        fileName = os.path.split(os.path.realpath(__file__))[1]
        type = fileName.split('_')[0].strip()
        sensor_type = fileName.split('_')[1].strip() + fileName.split('_')[2].split('.')[0].strip()

        cfg = self.config.read(self.cfg_file,'range_rotation_vector')
        ...
        for loop in range(10):
            ...
            self.sensorImpl.logger.info("Loop %d/10 range_rotation_vector" %(loop+1))
