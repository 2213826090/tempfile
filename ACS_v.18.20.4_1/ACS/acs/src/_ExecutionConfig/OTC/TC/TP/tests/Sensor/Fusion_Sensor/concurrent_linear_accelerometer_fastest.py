...
class concurrent_linear_accelerometer_fastest(UIATestBase):
    ...
    def tearDown(self):
        ...
        super(concurrent_linear_accelerometer_fastest, self).tearDown()

    def test_concurrent_linear_accelerometer_fastest(self):
        '''
        test the freq and delay for concurrent fastest mode linear_accelerometer
        '''
        fileName = os.path.split(os.path.realpath(__file__))[1]
        freq_type = fileName.split('_')[3].split('.')[0].strip()

        cfg = self.config.read(self.cfg_file,'concurrent_linear_accelerometer_fastest')
        ...
        for loop in range(10):
            ...
            self.sensorImpl.logger.info("Loop %d/10 concurrent_linear_accelerometer_fastest" %(loop+1))
