...
function install_internal_debian(){
    ...
    if [ $? -ne 0 ];then
        ...
        echo "    Downloading the debian package from \"http://jenkins-sys.sh.intel.com/releases/latest/Ubuntu$OS_Ver/\""
        echo "    dpkg -i $1.xxx.deb"
        echo "Installed package: $2"
        echo "For cleaning up all installed packages:"
        echo "    ./oat_deploy_deps.sh -r"
        exit 255
...
function install_testcases_deps(){
    ...
    install_internal_debian "igas-engine"
    install_internal_debian "beanstalkd"
    install_internal_debian "python-yaml"
}

function uninstall_pybeanstalk(){
    echo "Uninstalling pybeanstalk"
    if [ -d "0.11.1" ];then
        cd 0.11.1
        cat pybeanstalk_files.txt | xargs sudo rm -rf
        if [ $? -ne 0 ];then
            echo "WARNING:fail to uninstall pybeanstalk. please uninstall it manually:"
            echo "    cd 0.11.1"
            echo "    cat pybeanstalk_files.txt | xargs sudo rm -rf"
            cd ..
            exit 255
        fi
        cd ..
    else
        echo "WARNING: the pybeanstalk folder doesn't exists, skip to uninstall pybeanstalk"
    fi
}

function install_pybeanstalk(){
    wget http://jenkins-sys.sh.intel.com/tools/ata/pybeanstalk-0.11.1.tar.gz 
    tar -xvzf pybeanstalk-0.11.1.tar.gz
    cd 0.11.1
    python setup.py install --record pybeanstalk_files.txt
    if [ $? -ne 0 ];then
        echo "WARNING:fail to install pybeanstalk. please install it manually following below steps:"
        echo "    Downloading the tar ball from \"http://pybeanstalk.googlecode.com/files/pybeanstalk-0.11.1.tar.gz\" or \"http://jenkins-sys.sh.intel.com/tools/ata/pybeanstalk-0.11.1.tar.gz\""
        echo "    tar -xvzf pybeanstalk-0.11.1.tar.gz"
        echo "    cd 0.11.1"
        echo "    python setup.py install --record pybeanstalk_files.txt"
...
function update_repo(){
    ...
    add-apt-repository "deb http://jenkins-sys.sh.intel.com/releases/latest/Ubuntu$OS_Ver/ /"
    sed -i '/deb-src http:\/\/jenkins-sys/d' /etc/apt/sources.list
    apt-get update
}
...
#    install_pip_pkg "nose" "'requests', 'uiautomator'" "If the symlink '/usr/local/man' is exists and broken, please remove it, and re-execute this script"
    echo "Installing oat-l dependence"
    install_testcases_deps "'igas-engine', 'beanstalkd', 'python-yaml'"
    echo "Installing pybeanstalk"
    install_pybeanstalk "'requests', 'uiautomator', 'nose', 'noserunner', 'oat-l'"
