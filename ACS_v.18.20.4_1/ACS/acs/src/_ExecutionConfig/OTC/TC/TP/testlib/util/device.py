...
class TestDevice(object):
    ...
    def adb_cmd_capture_msg_ext(self, cmdstr, time_out=None, callbk=None):
        ...
        adbcmdstr = "%s shell %s" % (self.adb_prefix, cmdstr)
        LOG.debug('Execute adb command: %s' % adbcmdstr)
        _, out, _ = shell_command_ext(adbcmdstr, timeout=time_out, callbk=callbk)
        return out.strip('\r\n')
    ...
    def sync_file_from_content_server(self, cmd, datapath):
        ...
        if os.path.exists(file_name):
            ...
            import urllib2
            try:
                downloader = fileDownloader.DownloadFile(data, file_name, timeout=3000)
    ...
    def getDevices(self, device_num=1):
        ...
        if len(serials) < device_num:
            raise Exception("Lack of devices: attached device %s, expected device %d" % (len(serials), device_num))
    ...
    def pull_file(self, local_path, remote_path):
        ...
        adbcmdstr = "%s pull %s %s" % (self.adb_prefix, remote_path, local_path)
        exit_code, ret = shell_command(adbcmdstr, timeout_second=300)
        if exit_code != 0:
            error = ret[0].strip('\r\n') if len(ret) else "adb shell timeout"
            LOG.info("[ Pull file \"%s\" failed, error: %s ]" % (remote_path, error))
            return False
        else:
            return True
    ...
    def push_file(self, local_path, remote_path):
        ...
        adbcmdstr = "%s push %s %s" % (self.adb_prefix, local_path, remote_path)
        exit_code, ret = shell_command(adbcmdstr, timeout_second=300)
        if exit_code != 0:
            error = ret[0].strip('\r\n') if len(ret) else "adb shell timeout"
            LOG.info("[ Push file \"%s\" failed, error: %s ]" % (local_path, error))
            return False
        else:
            return True
