...
class concurrent_orientation_ui(UIATestBase):
    ...
    def setUp(self):
        ...
        super(concurrent_orientation_ui, self).setUp()

    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(concurrent_orientation_ui, self).tearDown()

    def test_concurrent_orientation_ui(self):
        fileName = os.path.split(os.path.realpath(__file__))[1]
        freq_type = fileName.split('_')[2].split('.')[0].strip()

        cfg = self.config.read(self.cfg_file,'concurrent_orientation_ui')
        ...
        for loop in range(10):
            ...
            self.sensorImpl.logger.info("Loop %d/10 concurrent_orientation_ui" %(loop+1))
