...
class TestChangeReboot(UIATestBase):
    """
    Testing wifi enable
    """
    def setUp(self):
        super(TestChangeReboot, self).setUp()
    ...
    def tearDown(self):
        ...
        super(TestChangeReboot, self).tearDown()
        self.wifi = None

    def testChangeReboot(self):
        """
        ...
        print "Rate: %s" % rate
        print "Expect that pictures are similar"
        assert (rate >= 0.95)
        os.remove(base_path)
        os.remove(new_path)
