...
class list_rotation_vector(UIATestBase):
    ...
    def setUp(self):
        ...
        super(list_rotation_vector, self).setUp()

    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(list_rotation_vector, self).tearDown()

    def test_list_rotation_vector(self):
        '''
        check the basic info and list of rotation_vector
        '''
        fileName = os.path.split(os.path.realpath(__file__))[1]
        test_type = sensor_type = fileName.split('_')[0].strip()

        cfg = self.config.read(self.cfg_file,'list_rotation_vector')
        ...
        for loop in range(10):
            ...
            self.sensorImpl.logger.info("Loop %d/10 list_rotation_vector" %(loop+1))
