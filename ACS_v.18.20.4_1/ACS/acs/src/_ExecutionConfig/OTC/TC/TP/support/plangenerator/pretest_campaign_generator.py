...
def dump_testcase(file_path, module, script_path, case):
    ...
    params = {
        'TEST_DATA_ROOT':os.path.join(script_path, 'testplan', module),
        'TEST_CASE':case,
...
def convert_to_case_path(case_name, prefix):
    case_name = case_name.split('.')
    del case_name[-2]
    post_name = os.path.join(*case_name)
    return os.path.join(prefix, post_name)
...
def process_pretest(module, OPTIONS):
    ...
    pre_fix = os.path.join('..', 'TC')
    case_paths = [convert_to_case_path(case_name, pre_fix) for case_name in case_names]
    ...
    for case_full_name in case_full_names:
        full_path = os.path.join(OPTIONS.dest,
                                'TC',
                                os.path.join(*(case_full_name.split('.')[1:-2])),
                                case_full_name.split('.')[-1] + '.xml')
        dump_testcase(full_path, module, OPTIONS.script_path, case_full_name)
