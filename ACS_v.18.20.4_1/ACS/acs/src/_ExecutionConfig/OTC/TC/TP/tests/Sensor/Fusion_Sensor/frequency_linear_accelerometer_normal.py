...
class frequency_linear_accelerometer_normal(UIATestBase):
    ...
    def setUp(self):
        ...
        super(frequency_linear_accelerometer_normal, self).setUp()

    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(frequency_linear_accelerometer_normal, self).tearDown()

    def test_frequency_linear_accelerometer_normal(self):
        fileName = os.path.split(os.path.realpath(__file__))[1]
        freq_type = fileName.split('_')[3].split('.')[0].strip()

        cfg = self.config.read(self.cfg_file,'frequency_linear_accelerometer_normal')
        ...
        for loop in range(10):
            ...
            self.sensorImpl.logger.info("Loop %d/10 frequency_linear_accelerometer_normal" %(loop+1))
