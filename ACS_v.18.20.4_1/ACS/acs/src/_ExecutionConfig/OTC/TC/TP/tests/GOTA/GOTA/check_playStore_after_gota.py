...
'''
@summary: test playstore after gota
@since: 9/10/2015
...
from testlib.domains.gms_impl import PlayStoreImpl
class CheckPlayStoreAfterGOTA(UIATestBase):

    def setUp(self):
        super(CheckPlayStoreAfterGOTA, self).setUp()
        ...
        self.playstore=PlayStoreImpl()
        print "[Setup]: %s" % self._test_name
    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(CheckPlayStoreAfterGOTA, self).tearDown()
        self.cfg = None
    def testCheckPlayStoreAfterGOTA(self):
        print "[RunTest]: %s" % self.__str__()
        self.playstore.launch_playstore()
