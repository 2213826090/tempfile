...
class KeepImpl:
    ...
    class Locator(object):
        ...
        def btn_search(self):
            """ UI button search """
            return self.d(resourceId="com.google.android.keep:id/search")

        @property
        def btn_searchx(self):
            """ UI button search """
            return self.d(resourceId="com.google.android.keep:id/search_button")

        @property
        def btn_search_text(self):
            """ UI button search text """
            return self.d(
                resourceId="com.google.android.keep:id/search_src_text")
    ...
    def create_a_note(self):
        ...
        self._locator.btn_menu_delete.click.wait()
        time.sleep(2)
        self.quit_app()

    def search_in_keep(self, input_key):
        """ search in keep """
        print "[INFO] Test input in Keep"
        g_common_obj.launch_app_from_home_sc("Keep")
        #Skip set up screen while launch the music app in the first time
        time.sleep(5)
        if self._locator.btn_search.exists:
            self._locator.btn_search.click()
        if self._locator.btn_searchx.exists:
            self._locator.btn_searchx.click()
