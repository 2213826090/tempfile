...
'''
@summary: test email contacts after gota
@since: 13/10/2015
...
from testlib.mail.mail_impl import EmailImpl
class CheckEmailAfterGOTA(UIATestBase):

    def setUp(self):
        super(CheckEmailAfterGOTA, self).setUp()
        ...
        self.email=EmailImpl()
        self.d=g_common_obj.get_device()
        print "[Setup]: %s" % self._test_name
    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(CheckEmailAfterGOTA, self).tearDown()
        self.cfg = None
    def testCheckEmailAfterGOTA(self):
        print "[RunTest]: %s" % self.__str__()
        g_common_obj.back_home()
        self.d(description="Apps").click.wait()
        if self.d(text="Widgets").exists:
            self.d(text="Widgets").click.wait()
        if self.d(text="Email").exists:
            self.gota.startEmailApp()
