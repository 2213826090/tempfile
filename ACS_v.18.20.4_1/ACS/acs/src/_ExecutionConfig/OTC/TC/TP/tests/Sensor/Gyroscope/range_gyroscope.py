...
class range_gyroscope(UIATestBase):
    ...
    def tearDown(self):
        ...
        super(range_gyroscope, self).tearDown()

    def test_range_gyroscope(self):
        fileName = os.path.split(os.path.realpath(__file__))[1]
        sensor_type = fileName.split('_')[1].split('.')[0].strip()

        cfg = self.config.read(self.cfg_file,'range_gyroscope')
        ...
        for loop in range(10):
            ...
            self.sensorImpl.logger.info("Loop %d/10 range_gyroscope" %(loop+1))
