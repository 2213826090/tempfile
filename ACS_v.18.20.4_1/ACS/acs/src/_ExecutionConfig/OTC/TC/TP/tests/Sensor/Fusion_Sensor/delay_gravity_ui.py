...
class delay_gravity_ui(UIATestBase):
    ...
    def setUp(self):
        ...
        super(delay_gravity_ui, self).setUp()

    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(delay_gravity_ui, self).tearDown()

    def test_delay_gravity_ui(self):
        fileName = os.path.split(os.path.realpath(__file__))[1]
        freq_type = fileName.split('_')[2].split('.')[0].strip()

        cfg = self.config.read(self.cfg_file,'delay_gravity_ui')
        ...
        for loop in range(10):
            ...
            self.sensorImpl.logger.info("Loop %d/10 delay_gravity_ui" %(loop+1))
