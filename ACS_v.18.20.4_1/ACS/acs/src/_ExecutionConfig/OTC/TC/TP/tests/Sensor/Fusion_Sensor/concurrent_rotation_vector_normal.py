...
class concurrent_rotation_vector_normal(UIATestBase):
    ...
    def setUp(self):
        ...
        super(concurrent_rotation_vector_normal, self).setUp()

    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(concurrent_rotation_vector_normal, self).tearDown()

    def test_concurrent_rotation_vector_normal(self):
        '''
        test the freq and delay for concurrent normal mode rotation_vector
        '''
        fileName = os.path.split(os.path.realpath(__file__))[1]
        freq_type = fileName.split('_')[3].split('.')[0].strip()

        cfg = self.config.read(self.cfg_file,'concurrent_rotation_vector_normal')
        ...
        for loop in range(10):
            ...
            self.sensorImpl.logger.info("Loop %d/10 concurrent_rotation_vector_normal" %(loop+1))
