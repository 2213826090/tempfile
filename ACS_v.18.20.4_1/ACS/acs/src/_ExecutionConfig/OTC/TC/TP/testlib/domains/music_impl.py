...
#--------- begin locator -------------
    class Locator(object):
        ...
        @property
        def btn_search(self):
            """ UI button music search """
            return self.d(resourceId="com.google.android.music:id/search")
        @property
        def btn_navigate_up(self):
            """UI button navigate up"""
            return self.d(description="Navigate up")
        @property
        def btn_search_text(self):
            """ UI button music search """
            return self.d(resourceId="com.google.android.music:id/search_src_text")
    ...
    def music_play(self, music_section=None):
        ...
        if music_section==None:
            assert self._locator.btn_music.exists, \
            "ERROR: Not find music folder in list, Please run deploy.sh first"
            self._locator.btn_music.long_click()
        else:
    ...
    def search_in_music(self, input_key):
        """ search in music """
        print "[INFO] Test input in music"
        self.launch_app_from_home_sc("Play Music")
        #Skip set up screen while launch the music app in the first time
        time.sleep(5)
        if self._locator.btn_not_now.exists:
        ...
        self._locator.btn_navigate_up.click()
