...
class HalImpl:
    ...
    def init_environment(self):
        ...
        self.cfg_file = 'tests.hal.binary.conf'
        ...
        osversion = g_common_obj.adb_cmd_capture_msg('getprop | grep ro.build.version.release | cut -d ":" -f2 | cut -d "[" -f2 | cut -d "]" -f1 | cut -d "." -f1')
        if osversion == "M" or osversion == "6":
            print "osversion is M"
            binary_name = cfg.get("name_m")
        else:
    ...
    def run_case(self, case):
        ...
        cmd = str(cfg.get('cmd'))
        output = g_common_obj.adb_cmd_capture_msg('/data/app/' + self.file_name + ' ' + cmd)
        ret = output.find("PASSED")
        assert ret !=-1,"The test failed,output is %s"%output