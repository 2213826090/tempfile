...
'''
@summary: test hangouts function after gota
@since: 9/10/2015
...
class CheckHangoutsAfterGOTA(UIATestBase):

    def setUp(self):
        super(CheckHangoutsAfterGOTA, self).setUp()
	...
	self.hangouts=HangoutsImpl()
        print "[Setup]: %s" % self._test_name
    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(CheckHangoutsAfterGOTA, self).tearDown()
        self.cfg = None
    def testCheckHangoutsAfterGOTA(self):
        print "[RunTest]: %s" % self.__str__()
        print "Check basic features"
        self.hangouts.basic_test()
