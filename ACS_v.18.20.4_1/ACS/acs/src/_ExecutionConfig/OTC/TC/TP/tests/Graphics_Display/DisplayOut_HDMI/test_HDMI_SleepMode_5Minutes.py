...
class HDMITest(UIATestBase):
    ...
    def test_HDMI_SleepMode_5Minutes(self):
        """
        test_HDMI_SleepMode_5Minutes
        """
        print "[RunTest]: %s" % self.__str__()

        print """Launch photos and capture decode QRcodepicture."""
        self.HDMI_switch_card.switch_on()
        self.photosImpl.launch_photos_am()
        self.photosImpl.open_a_picture()
        print "press power key to Sleep mode."
        ...
        time.sleep(300)
        print "press power key to wake up device."
        g_common_obj.adb_cmd("input keyevent 26")
