...
class Game(RenderAppTestBase):
    ...
    def setUp(self):
        ...
        self._game = GameImpl()
        self._game.install_apk("Tower")

    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(Game, self).tearDown()
        self._game.return_normal_screen()
        self._game.uninstall_tower()

    def test_3dgame_crystalglasstower(self):
        ''' refer TC test_3DGame_CrystalGlassTower
        '''
        print "[RunTest]: %s" % self.__str__()
        self._game.launch_tower_am()
        self._game.play_tower()
        self._game.stop_tower_am()
