...
'''
@summary: Class for DebugMode operation
@since: 05/18/2015
@author: Xiangyi Zhao
'''
...
class DebugModeImpl:
    ...
    def set_secondary_displays_none(self):
        for i in range(0, 3):
            put_cmd = "settings put global overlay_display_devices null"
            g_common_obj.adb_cmd_capture_msg(repr(put_cmd))
            get_cmd = "settings get global overlay_display_devices"
            value = g_common_obj.adb_cmd_capture_msg(repr(get_cmd))
            if value == 'null':
                print "[Debug] DebugMode Simulate secondary displays is %s" % value
                break
    ...
    def change_simulate_480p(self):
        ...
        remsg1, remsg2, msg = self.check_resolution("720x480", "secure")
        assert  remsg1 != -1 and remsg2 == -1 , "dumpsys msg is %s" % msg
        self.set_secondary_displays_none()
