...
class rotation_vector_streaming(UIATestBase):
    ...
    def setUp(self):
        ...
        super(rotation_vector_streaming, self).setUp()

    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(rotation_vector_streaming, self).tearDown()

    def test_rotation_vector_streaming(self):
        fileName = os.path.split(os.path.realpath(__file__))[1]
        sensor_name = fileName.split('_')[0].strip() + '_' + fileName.split('_')[1].strip()
        sensor_type = fileName.split('_')[0].strip() + " vector"

        num = 0
        for loop in range(10):
            self.sensorImpl.logger.info("")
            self.sensorImpl.logger.info("Loop %d/10 test rotation_vector delay" %(loop+1))
            if self.sensorImpl.excute(fileName, "delay", sensor_name, str(10), "fastest", str(0), "fastest") is False:
                num += 1
        if num > 5:
            assert False

        self.sensorImpl.logger.info("test rotation_vector jitter--------^_^--------")
