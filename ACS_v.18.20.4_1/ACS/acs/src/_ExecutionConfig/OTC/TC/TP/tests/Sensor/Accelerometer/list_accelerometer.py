...
class list_accelerometer(UIATestBase):
    ...
    def setUp(self):
        ...
        super(list_accelerometer, self).setUp()

    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(list_accelerometer, self).tearDown()

    def test_list_accelerometer(self):
        fileName = os.path.split(os.path.realpath(__file__))[1]
        test_type = sensor_type = fileName.split('_')[0].strip()

        cfg = self.config.read(self.cfg_file,'list_accelerometer')
        ...
        for loop in range(10):
            ...
            self.sensorImpl.logger.info("Loop %d/10 list_accelerometer" %(loop+1))
