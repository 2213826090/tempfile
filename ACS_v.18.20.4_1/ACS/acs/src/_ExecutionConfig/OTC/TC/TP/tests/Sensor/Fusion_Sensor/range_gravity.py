...
class range_gravity(UIATestBase):
    ...
    def setUp(self):
        ...
        super(range_gravity, self).setUp()

    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(range_gravity, self).tearDown()

    def test_range_gravity(self):
        '''
        check the max range of gravity
        '''
        ...
        cfg = self.config.read(self.cfg_file,'range_gravity')
        ...
        for loop in range(10):
            ...
            self.sensorImpl.logger.info("Loop %d/10 range_gravity" %(loop+1))
