...
class frequency_gyroscope_normal(UIATestBase):
    ...
    def setUp(self):
        ...
        super(frequency_gyroscope_normal, self).setUp()

    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(frequency_gyroscope_normal, self).tearDown()

    def test_frequency_gyroscope_normal(self):
        ...
        if 'mrd' in product:
            cfg = self.config.read(self.cfg_file, 'frequency_gyroscope_normal_t3mrd')
        else:
            cfg = self.config.read(self.cfg_file, 'frequency_gyroscope_normal')
        ...
        for loop in range(10):
            ...
            self.sensorImpl.logger.info("Loop %d/10 frequency_gyroscope_normal" %(loop+1))
            if self.sensorImpl.excute(fileName, catagory, sensorName, compare_result, para1, para2, freq_type, sensor_type) is False:
                num += 1
        if num > 5:
            assert False
