...
class concurrent_gyroscope_normal(UIATestBase):
    ...
    def setUp(self):
        ...
        super(concurrent_gyroscope_normal, self).setUp()

    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(concurrent_gyroscope_normal, self).tearDown()

    def test_concurrent_gyroscope_normal(self):
        fileName = os.path.split(os.path.realpath(__file__))[1]
        freq_type = fileName.split('_')[2].split('.')[0].strip()

        cfg = self.config.read(self.cfg_file,'concurrent_gyroscope_normal')
        ...
        for loop in range(10):
            ...
            self.sensorImpl.logger.info("Loop %d/10 concurrent_gyroscope_normal" %(loop+1))
