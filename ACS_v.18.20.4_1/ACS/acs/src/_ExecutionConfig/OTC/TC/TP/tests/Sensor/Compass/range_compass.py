...
class range_compass(UIATestBase):
    ...
    def setUp(self):
        ...
        super(range_compass, self).setUp()

    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(range_compass, self).tearDown()

    def test_range_compass(self):
        fileName = os.path.split(os.path.realpath(__file__))[1]
        sensor_type = fileName.split('_')[1].split('.')[0].strip()

        cfg = self.config.read(self.cfg_file,'range_compass')
        ...
        for loop in range(10):
            ...
            self.sensorImpl.logger.info("Loop %d/10 range_compass" %(loop+1))
            if self.sensorImpl.excute(fileName, catagory, sensorName, compare_result, para1, para2, None, sensor_type) is False:
                num += 1
        if num > 5:
            assert False
