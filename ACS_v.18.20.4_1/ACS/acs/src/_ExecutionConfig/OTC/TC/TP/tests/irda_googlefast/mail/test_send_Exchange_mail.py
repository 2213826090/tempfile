...
class TestSendExchangeMail(UIATestBase):
    """
    @summary: Test send exchange email function
    """
    def setUp(self):
        """
        @summary: set up
        @return: None
        """
        super(TestSendExchangeMail, self).setUp()
        cfg_file = 'tests.tablet.google_fast.conf'
        self.mail = MailImpl()
        self.mailCfg = self.config.read(cfg_file, 'exchange_mail')
    ...
    def tearDown(self):
        ...
        @return: None
        """
        super(TestSendExchangeMail, self).tearDown()
        self.contacts = None

    def testSendExchangeMail(self):
        """
        This test used to test contacts app

        The test case spec is following:
        1. launch mail app
        2. send Exchange Mail
        """
