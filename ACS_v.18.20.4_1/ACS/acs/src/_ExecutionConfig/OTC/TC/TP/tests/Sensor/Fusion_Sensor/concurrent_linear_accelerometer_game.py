...
class concurrent_linear_accelerometer_game(UIATestBase):
    ...
    def setUp(self):
        ...
        super(concurrent_linear_accelerometer_game, self).setUp()

    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(concurrent_linear_accelerometer_game, self).tearDown()

    def test_concurrent_linear_accelerometer_game(self):
        '''
        test the freq and delay for concurrent game mode linear_accelerometer
        '''
        fileName = os.path.split(os.path.realpath(__file__))[1]
        freq_type = fileName.split('_')[3].split('.')[0].strip()

        cfg = self.config.read(self.cfg_file,'concurrent_linear_accelerometer_game')
        ...
        for loop in range(10):
            ...
            self.sensorImpl.logger.info("Loop %d/10 concurrent_linear_accelerometer_game" %(loop+1))
