...
def _install(device, pkg_path):
    cmd = 'adb install -r -g %s' % pkg_path
    os.system(cmd) # Fix apk not install issue
...
def _instrument(device, test_case, test_pkg, inst_componment):
    cmd = 'am instrument -w -r -e class %s %s/%s' % (test_case, test_pkg, inst_componment)
...
class InstrumentedBaseImpl():
    ...
    def instr_run(self, test_case):
        if self.test_pkg is None:
            return
        output = _instrument(self.device, test_case, self.test_pkg, self.inst_runner)
