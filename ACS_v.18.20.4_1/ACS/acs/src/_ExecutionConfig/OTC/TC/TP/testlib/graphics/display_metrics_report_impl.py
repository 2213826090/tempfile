...
class DisplayMetricsReportImpl(object):
    ...
    density_range = [120, 160, 213, 240, 280, 320, 360, 400, 480, 560, 640]
    flag = False
    ...
    def compare_dumpsys_size_with_real_size(self):
        ...
        assert dumpsys_hight == hight and dumpsys_hight >= 600, \
            "[FAILURE] dumpsys hight is not equal to real hight:dumpsys hight is %d,real hight is %d" % (dumpsys_hight, hight)
        assert dumpsys_width == width and dumpsys_width >= 600, \
            "[FAILURE] dumpsys width is not equal to  real width:dumpsys width is %d,real width is %d" % (dumpsys_width, width)

    def compare_dumpsys_density_with_real_density(self):
        """
        compare dumpsys density with real density
        """
        dumpsys_density = windows_info.get_dumpsys_density()
        ...
        assert dumpsys_density == density and dumpsys_density > 0, \
            "[FAILURE] dumpsys density is not equal to  real density:dumpsys density is %d,real density is %d" % (dumpsys_density, density)
    ...
    def judge_dpi(self):
        ...
        min = 0
        closest_density = 0
        dumpsys_dpi = windows_info.get_dumpsys_dpi()
        ...
        assert closest_density == dumpsys_density, \
            "[FAILURE]the dumpsys_density is not the closest to dumpsys dpi,the closest density is %s )" % closest_density
    ...
    def calculate_screen_aspect_ratio(self):
        width = windows_info.get_wm_width()
        hight = windows_info.get_wm_hight()
        if width > hight:
            ratio = float(width) / hight
        else:
            ratio = float(hight) / width
        assert ratio > 1.3333 and ratio < 1.86, "[FAILURE]The screen ratio is not right,it shold be between 1.3333 and 1.86,but now it is %s" % ratio
