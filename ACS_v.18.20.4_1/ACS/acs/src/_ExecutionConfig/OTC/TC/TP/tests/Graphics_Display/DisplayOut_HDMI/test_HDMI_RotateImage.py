...
class HDMITest(UIATestBase):
    ...
    def test_HDMI_RotateImage(self):
        """
        test_HDMI_RotateImage
        """
        ...
        for i in range(self.retry_num):
            ...
            self.d(resourceId="com.google.android.apps.photos:id/edit").click.wait()
            self.d(resourceId="com.google.android.apps.photos:id/cpe_crop_tool").click.wait()
            bounds = self.d(
                resourceId="com.google.android.apps.photos:id/cpe_crop_overlay").bounds
            snapshot_before_rotation = get_screenshot_region(bounds)
            # rotate the image to left
            for m in range(3):
                self.d(resourceId="com.google.android.apps.photos:id/cpe_rotate_90").click.wait()
            snapshot_after_rotation = get_screenshot_region(bounds)
            campare_result = ComparePicImpl().compare_pic(snapshot_before_rotation, snapshot_after_rotation)
            assert campare_result
            self.d(resourceId="com.google.android.apps.photos:id/cpe_rotate_90").click.wait()
            # move the 2 snapshots to self.tmpdir, then delete them in tear down
            os.system("mv " + snapshot_before_rotation + " " + self.tmpdir)
            os.system("mv " + snapshot_after_rotation + " " + self.tmpdir)
