...
class compass_streaming(UIATestBase):
    ...
    def setUp(self):
        ...
        super(compass_streaming, self).setUp()

    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(compass_streaming, self).tearDown()

    def test_compass_streaming(self):
        ...
        for loop in range(10):
            ...
            self.sensorImpl.logger.info("Loop %d/10 test compass delay" %(loop+1))
            if self.sensorImpl.excute(fileName, "delay", sensor_type, str(10), "fastest", str(0), "fastest") is False:
                num += 1
        if num > 5:
            assert False

        self.sensorImpl.logger.info("test compass jitter--------^_^--------")
        self.sensorImpl.sensor_log(self.config.read(self.cfg_file, "artifactory"), "Fastest", "magnetic field")
