...
class orientation_streaming(UIATestBase):
    ...
    def setUp(self):
        ...
        super(orientation_streaming, self).setUp()

    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(orientation_streaming, self).tearDown()

    def test_orientation_streaming(self):
        ...
        for loop in range(10):
            ...
            self.sensorImpl.logger.info("Loop %d/10 test orientation delay" %(loop+1))
            if self.sensorImpl.excute(fileName, "delay", sensor_type, str(10), "fastest", str(0), "fastest") is False:
                num += 1
        if num > 5:
            assert False

        self.sensorImpl.logger.info("test orientation jitter--------^_^--------")
