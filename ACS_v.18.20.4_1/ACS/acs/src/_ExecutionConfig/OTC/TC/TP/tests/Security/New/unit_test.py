...
class UnitTest(UIATestBase):
    def setUp(self):
        ...
        self.push_file = "unit_test_exists"
        print
    ...
    def auto_flash_eb_image(self):
        ...
        if "gordon_peak" in product:
            ...
            if "2G" == mem_size:
                option_file = "unit_omr1_2g"
            else:
                option_file = "unit_omr1"
        elif "bxt" in product:
            option_file = "unit_m"
        else:
        ...
        res_eb_image = self.flashImage.get_download_file_res(option_file, "unit_test-flashfiles")
    ...
    def test_eb_image_unit_test_secure_storage(self):
        # HP desc: need use userdebug image
        print "[RunTest]: %s" % self.__str__()
        self.securityImpl.logger.info("")
        self.whether_flash_eb_image()
        self.securityImpl.check_secure_storage_unit_test()

    def test_eb_image_unit_test_hwcrypto(self):
        print "[RunTest]: %s" % self.__str__()
        self.whether_flash_eb_image()
        self.securityImpl.check_hwcrypto_unit_test()

    def test_eb_image_unit_test_hwcrypto_app(self):
        print "[RunTest]: %s" % self.__str__()
        self.whether_flash_eb_image()
        self.securityImpl.check_hwcrypto_unit_test()
