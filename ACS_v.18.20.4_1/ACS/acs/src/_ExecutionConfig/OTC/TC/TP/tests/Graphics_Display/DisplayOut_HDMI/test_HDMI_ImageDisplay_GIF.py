...
class HDMITest(UIATestBase):
    ...
    def setUp(self):
        ...
        self.retry_num = self.config.read(cfg_file, 'rerun_times').get("hdmi_imagedisplay_gif")
        self.retry_num = int(self.retry_num)
        cfg = config.read(cfg_file, 'qrcode_marked_image')
        print self.qrcode
        self.qrcode = cfg.get("hdmi_gif_qrcode")
        print "[Debug] qrcode:%s" % (self.qrcode)
        arti = Artifactory(cfg_arti.get('location'))
        pic_name = cfg.get("hdmi_gif_image")
    ...
    def test_HDMI_ImageDisplay_GIF(self):
        """
        test_HDMI_ImageDisplay_GIF
        """
