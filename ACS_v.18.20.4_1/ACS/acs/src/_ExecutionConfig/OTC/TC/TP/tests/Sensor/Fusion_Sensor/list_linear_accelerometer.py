...
class list_linear_accelerometer(UIATestBase):
    ...
    def setUp(self):
        ...
        super(list_linear_accelerometer, self).setUp()

    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(list_linear_accelerometer, self).tearDown()

    def test_list_linear_accelerometer(self):
        '''
        check the basic info and list of linear_accelerometer
        '''
        fileName = os.path.split(os.path.realpath(__file__))[1]
        test_type = sensor_type = fileName.split('_')[0].strip()

        cfg = self.config.read(self.cfg_file,'list_linear_accelerometer')
        ...
        for loop in range(10):
            ...
            self.sensorImpl.logger.info("Loop %d/10 list_linear_accelerometer" %(loop+1))
            if self.sensorImpl.excute(fileName, catagory, sensorName, compare_result, para1, para2, None, None, test_type) is False:
                num += 1
        if num > 5:
            assert False
