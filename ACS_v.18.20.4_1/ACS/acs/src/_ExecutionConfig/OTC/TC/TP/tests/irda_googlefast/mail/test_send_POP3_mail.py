...
class TestSendPOP3Mail(UIATestBase):
    """
    @summary: Test send POP3 email function
    """
    def setUp(self):
        """
        @summary: set up
        @return: None
        """
        super(TestSendPOP3Mail, self).setUp()
        cfg_file = 'tests.tablet.google_fast.conf'
        self.mail = MailImpl()
        self.mailCfg = self.config.read(cfg_file, 'pop3_mail')
    ...
    def tearDown(self):
        ...
        @return: None
        """
        super(TestSendPOP3Mail, self).tearDown()
        self.contacts = None

    def testSendPOP3Mail(self):
        """
