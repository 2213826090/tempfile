...
class PhotosImpl(ImplCommon):
    ...
    def launchPhotos(self, push_folder=""):
        # am star used for photos version below 2.3
        # g_common_obj2.launch_app_am("com.google.android.apps.photos", \
        #                            "com.google.android.apps.photos.localmedia.ui.LocalFoldersActivity")
        # for photos version 2.4.0
        if push_folder == "":
            push_folder = "sdcard1"
        ...
        for _ in range(3):
            ...
            if self.d(description="Photos, selected, tab, 2 of 3").exists or self.d(textContains=test_file_folder).exists:
        ...
        for _ in range(3):
            ...
            # Photos' version above 2.5 , it's navigation dump changed as following
        ...
        #     time.sleep(3)
        #     assert not self.d(textContains="Get update").exists, "Photos app didn't upgrade!"
        #     assert False, "launch Photos app failed!!"
    ...
    def elementOpen(self, testElementFunc, x=1):
        ...
        uiChild2 = UiWindows(self.d(resourceId="com.google.android.apps.plus:id/grid").child(index=1).down().info[u"bounds"])
        print uiChild1
            ...
            if j == lineNum:
                ...
                if canRecogizeEnd:
                    ...
                    assert UiWindows(self.d(resourceId="com.google.android.apps.plus:id/grid")\
                        .child(index=nums/lineNum).info).getBottom()>self.d.info[u'displayHeight'], "Failed to find"
                j=0
            j=j+1
            print "open:", i
    ...
    def playback_video_un_QR_code_photoplus(self, playTime=0, videoIndex=0, flag=False, stoptime=-1):
        ...
        time.sleep(3)
        #Back to main interface if still in play interface
        ...
        assert self.d(resourceId="com.google.android.apps.plus:id/tile_row").child(index=videoIndex).exists or \
                self.d(resourceId="com.google.android.apps.plus:id/grid").child(index=videoIndex).exists, "There is no video in photo"
        try:
...
#         self.d(resourceId = "android:id/action_bar").child_by_description("More options").click()
            self.d(description="More options").click()
            self.d(text="Details").click()
            if self.d(text="Duration").exists:
                tt = self.d(resourceId="com.google.android.apps.plus:id/exif_detail_list").child(index=2).child(index=1).text
            else:
                tt = "00:10"
        ...
        time.sleep(1)
        #node = self.gallery.get("playback_page").get("play_area")
        ...
        while True:
            ...
            if ct and tt:
                ...
                print "Total play time is : " + str(tt) + " current play time is " + str(ct)  + " videoLength is " + str(videoLength)
                #assert not ((not flag) and ttInt != videoLength), "total time display failed"
            ...
            if time.time() - timeNow + 8 > useStopTime:
                ...
                break
#         assert self.d(resourceId="com.google.android.apps.plus:id/videoplayer").exists or \
#                 self.d(resourceId="com.google.android.apps.plus:id/photo_hashtag_fragment_container").exists, " video playback when finished failed"
        self.dut.press.back()
...
#         if img0:
#             w = otcImage.analysisImageToString(img0)
#         else:
#             w = None
#         if w:
#             words = w.replace("\n", "").replace(" ", "")
...
#         fileExsits = os.path.exists(fileName.replace(".png","_view.png"))
#         print "fileExsits is ",fileExsits
#         if not fileExsits:
#             print "screenshot path is ",fileName.replace(".png","_view.png")
#             img0.save(fileName.replace(".png","_view.png"))
#             return False,"File Not fileExsits"
#         img1 = Image.open(fileName.replace(".png","_view.png"))
    ...
    def view_image_detail(self, line=0, position=0):
        ...
        for i in range(5):
            try:
                key = self.d(resourceId="com.google.android.apps.plus:id/exif_detail_list").child(index=i).child(index=0).text
                value = self.d(resourceId="com.google.android.apps.plus:id/exif_detail_list").child(index=i).child(index=1).text
            except:
                key = None
                value = None
    ...
    def push_image_photo_plus(self, number):
        ...
        for count in range(1, number + 1):
            ...
            if orgFileName == 0:
                orgFileName = 50
            self.push_file(str(orgFileName) + ".png", "autotest/imageFile/num_image", str(count) + ".png", "/sdcard/otctest")
            fileMap[count] = orgFileName
            print str(count) + ".png ", orgFileName
        g_common_obj.adb_cmd_capture_msg("am broadcast -a android.intent.action.MEDIA_MOUNTED --ez read-only false -d file://sdcard")
        return fileMap
    ...
    def enter_menu_page(self):
        self.d(resourceId="android:id/action_bar").child_by_description("More options", className="android.widget.ImageButton").click()
    ...
    def zoom_in_out_image(self, ssim=0.99):
        ...
        assert not (words0 not in words1 and words1 not in words0), "double click,image zoom display failed ,before is " + str(words0) + " after values is " + str(words1)

    def exit_photo_plus(self):
        self.dut.press.back()
        time.sleep(3)
        assert not self.d(resourceId="com.google.android.apps.plus:id/tiles", packageName="com.google.android.apps.plus").exists, "exit photo plus to home failed"
