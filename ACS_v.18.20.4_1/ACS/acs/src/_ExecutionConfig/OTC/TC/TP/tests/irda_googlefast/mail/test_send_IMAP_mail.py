...
class TestSendIMAPMail(UIATestBase):
    """
    @summary: Test send IMAP mail function
    """
    def setUp(self):
        """
        @summary: set up
        @return: None
        """
        super(TestSendIMAPMail, self).setUp()
        cfg_file = 'tests.tablet.google_fast.conf'
        self.mail = MailImpl()
        self.mailCfg = self.config.read(cfg_file, 'imap_mail')
    ...
    def tearDown(self):
        ...
        @return: None
        """
        super(TestSendIMAPMail, self).tearDown()
        self.contacts = None

    def testSendIMAPMail(self):
        """
