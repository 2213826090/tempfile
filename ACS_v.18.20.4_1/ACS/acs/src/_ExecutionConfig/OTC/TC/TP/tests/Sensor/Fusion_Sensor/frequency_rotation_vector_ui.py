...
class frequency_rotation_vector_ui(UIATestBase):
    ...
    def setUp(self):
        ...
        super(frequency_rotation_vector_ui, self).setUp()

    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(frequency_rotation_vector_ui, self).tearDown()

    def test_frequency_rotation_vector_ui(self):
        fileName = os.path.split(os.path.realpath(__file__))[1]
        freq_type = fileName.split('_')[3].split('.')[0].strip()

        cfg = self.config.read(self.cfg_file,'frequency_rotation_vector_ui')
        ...
        for loop in range(10):
            ...
            self.sensorImpl.logger.info("Loop %d/10 frequency_rotation_vector_ui" %(loop+1))
