...
class list_gravity(UIATestBase):
    ...
    def setUp(self):
        ...
        super(list_gravity, self).setUp()

    def tearDown(self):
        print "[Teardown]: %s" % self._test_name
        super(list_gravity, self).tearDown()

    def test_list_gravity(self):
        '''
        check the basic info and list of gravity
        '''
        fileName = os.path.split(os.path.realpath(__file__))[1]
        test_type = sensor_type = fileName.split('_')[0].strip()

        cfg = self.config.read(self.cfg_file,'list_gravity')
        ...
        for loop in range(10):
            ...
            self.sensorImpl.logger.info("Loop %d/10 list_gravity" %(loop+1))
