...
class MultiMediaSetting:
    ...
    def install_apk(self, apk_name, times=5):
        ...
        if verify_apps(apk_path_config.get("package_name")) == 1:
            ...
            filepath = download_file(self.resource_file_path + apk_path_config.get("file_path"), apk_path_config.get("file_name"))
    ...
    def launch_apk(self, apk_name):
        if apk_name not in self.available_apk_dict.keys():
            ...
            assert package_name != None and activity_name != None, "Can't find package or activity name! apk_name=%s, package_name=%s, activity_name=%s" % (apk_name, package_name, activity_name)
    ...
    def download_file_to_host(self, src_path):
        ...
        if "http" not in src_path and "ftp" not in src_path:
            ...
            assert ret_file != None and os.path.exists(ret_file), "Download filed! ret_file=%s, src_path=%s" % (ret_file, src_path)
    ...
    def pressPowerKey(self, duration=2):
        logger.debug("press power key start")
        self.POWER_REALY = "power_relay"
        self.relay08_helper = Relay08Helper()
        self.relay08_helper.set_relay_NO(self.POWER_REALY)
        time.sleep(duration)
        self.relay08_helper.set_relay_NC(self.POWER_REALY)
    ...
    def get_mount_point(self):  #NOQA
        ...
        def get_mount_point_m():
            ...
            #         logger.debug(self.tag + "mount msg:" + l)
            #         return fuse_path
            # else:
            #     return None
    ...
    def get_sdcard_memory(self, path=""):
        if path == "":
        ...
        if len(result) == 3:
            ...
            used = result[1]
            free = result[2] 
            logger.debug("SDCard total "+str(total)+ "used "+ str(used)+ "free "+ str(free))
    ...
    def setAlarmTime(self, seconds):
        self.d(className="android.widget.EditText").set_text(seconds);
        time.sleep(1)
        if self.d(text="OK").exists:
            self.d(text="OK").click()
            logger.debug("set alarm %s seconds"% seconds)
...
class MultiMediaHandle:
    ...
    def launchVLCApp(self):
        ...
        for _ in range(3):
            ...
            if self.d(description="Open navigation drawer").exists:
                return
        assert self.d(resourceId="android:id/list").exists or self.d(description="Open navigation drawer").exists, "launch VLC app failed!"
    ...
    def get_activity(self):
        activity_msg = None
        if g_common_obj2.adb_cmd_capture_msg('dumpsys activity |grep mFocusedActivity'):
            activity_msg = g_common_obj2.adb_cmd_capture_msg('dumpsys activity |grep mResumedActivity')
        else:
            activity_msg = g_common_obj2.adb_cmd_capture_msg('dumpsys activity |grep mResumedActivity')
        activity_list= []
    ...
    def check_current_logcat_msg(self, msg, clear_log=False):
        ...
        #logger.debug("Get msg output :%s" % msg_output.split('/n')[0])
        if len(msg_output) > 0:
    ...
    def checkVideoPlayBack(self, s=30, done_flag=False, check_hang=True):
        ...
        if not play_view in self.get_activity():
    ...
    def checkVideoPlayBackWithPhotoApp(self, stoptime, bigfileskiptime=0, forceflag=0):
        ...
        while time.time() - start_time <= stoptime:
            for i in range(3):
                # if self.d(resourceId="com.google.android.apps.photos:id/list_photo_tile_view").exists:
                #     self.d(className="android.widget.ImageView").click()
                #Id changed in Phtotos(2.2.0135750339), update the video play Id.
                if self.d(resourceId="com.google.android.apps.photos:id/title").exists:
                    self.d(className="android.view.ViewGroup").click()
                #if media file not play, re-click the media file
...
class CheckPhotoApp:
    ...
    def check_photo_app(self):
        '''

        update Google Photos via Google Play

        '''
        logger.info("Start to check photos app")
        succeed = False
        for _ in range(3):
            try:
                ...
                logger("add google account exception : {0}".format(e))
        assert succeed, "add google failed : {0}".format(e)
        if len(self.get_photos_version()) > 1:
            logger.info("Photos has updated")
        else:
            logger.info("Started to update photos via Play store")
            app_name = "Google Photos"
            g_common_obj2.launch_app_am("com.android.vending", \
                                        "com.android.vending.AssetBrowserActivity")
            time.sleep(5)
            if self.d(textContains="Keep me up to date").exists:
                self.d(textContains="Keep me up to date").click()
                self.d(textContains="ACCEPT").click()
            t_flag = False
            for _ in range(3):
                if self.d(description="Search Google Play").exists:
                    self.d(description="Search Google Play").click()
                    t_flag = True
                elif self.d(className="android.widget.ImageView", description="Search").exists:
                    self.d(className="android.widget.ImageView", description="Search").click()
                    t_flag = True
                else:
                    time.sleep(3)
            assert t_flag == True, "Can't find search button!"
            self.d(className="android.widget.EditText").set_text(app_name)
            time.sleep(3)
            self.d.press.enter()
            time.sleep(3)
            for _ in range(5):
                if self.d(text='Google Photos').exists:
                    self.d(text='Google Photos').click()
                if self.d(resourceId='com.android.vending:id/update_button').exists:
                    logger.debug("start to update photos")
                    self.d(resourceId='com.android.vending:id/update_button').click()
                if self.d(textContains="Apps built for").exists:
                    self.d(resourceId="com.android.vending:id/positive_button").click()

            for _ in range(300):
                if self.d(text="UNINSTALL").exists:
                    break
                time.sleep(1)

        photos_versionNO = self.get_photos_version()
        assert photos_versionNO[1], 'update photos failed'
        logger.debug("original photos version is : %s" % photos_versionNO[1])
        logger.debug("updated photos version is : %s" % photos_versionNO[0])
        logger.debug("update photos success")

        self.init_updated_Photos()
        return True

    def get_photos_version(self):
        photos_versionNO_cmd = g_common_obj2.adb_cmd_common(
            'shell dumpsys package com.google.android.apps.photos |grep versionName')
        get_version_parttern = re.compile(r'versionName=(.*)')
        return get_version_parttern.findall(photos_versionNO_cmd)

    def init_updated_Photos(self):
        # init updated Photos
        g_common_obj2.launch_app_am(PHOTOS_PACKAGE_NAME, PHOTOS_ACTIVITY_NAME)
        for _ in range(3):
            if self.d(textContains='Google Photos uses face').exists and self.d(textContains='ON').exists:
                logger.debug("back up & sync display, try to init photos")
                self.d(textContains='on').click()
            if self.d(className='android.widget.FrameLayout').exists and self.d(textContains='done').exists:
                self.d(textContains='DONE').click()
                time.sleep(1)
            if self.d(textContains="Keep backup off").exists:
                self.d(textContains="Keep off").click()

            if self.d(description="Photos, selected, tab, 2 of 3").exists:
                logger.debug("init completed")
                break
        assert not self.d(textContains='Google Photos uses face').exists, logger.debug("init updated photos failed")
        return True
...
class MultiMediaBasicTestCase:
    ...
    def appPrepare(self, case_name, model=1):
        ...
        if model == 1:
            self.push_path = self.multimedia_setting.push_file(self.cfg_case.get("push_video"), self.cfg_case.get("datapath"))
            
        self.multimedia_setting.install_apk("video_apk")
        self.multimedia_setting.install_apk("alarm_apk")
        if not self.hardware in ['r2_cht_mrd']:
            self.multimedia_setting.install_apk("vlc_apk")
    ...
    def testVideoPlayBack(self, case_name, check_hang=True):
        ...
        ct, tt = self.multimedia_setting.get_play_time()
        if tt - ct >= 50:
            assert self.multimedia_handle.checkVideoPlayBackWithComparePicture(60,check_hang=check_hang), "Video play failed"
        else:
    ...
    def suspend_ressmue(self,sleep_time=30):
        logger.debug(self.tag+'suspend and then resume device')
    ...
    def streamingVideoPlayControlProcessLong(self, case_name):
        ...
        while time.time() - start_time < int(timeout):
            ...
            self.multimedia_handle.clickOtcVideoPlayer_Control(resourceId="android:id/pause")
            iteration += 1
            logger.info("{0} iteration control completed, run {1} seconds".format(iteration,\
                                                                                  str(time.time() - start_time).split('.')[0]))
