...
:description: This file contains Python UIAutomator functions for the Date & time
settings panel, Android L-dessert MR1 version
:since: 7/21/14
:author: mmaracix
"""
...
def enable_automatic_time():
    if not d(className="android.widget.Switch", instance=0).checked:
        d(text="Automatic date & time").click()
    else:
        pass
    if d(text="Set date").enabled or d(text="Set time").enabled or (
            not d(className="android.widget.Switch", instance=0).checked):
        return False
    else:
        return True
...
def disable_automatic_time():
    if not d(className="android.widget.Switch", instance=0).checked:
        ...
        d(text="Automatic date & time").click()
    if d(text="Set date").enabled and d(text="Set time").enabled and (
            not d(className="android.widget.Switch", instance=0).checked):
        return True
    else:
        return False
...
def set_time(time_string):
    ...
    time_to_be_set = ''
    if d(resourceId="android:id/ampm_layout").exists:
    ...
    if d(resourceId="android:id/ampm_layout").exists:
        if d(resourceId="android:id/pm_label", checked=True).exists:
            time_set += " " + "PM"
        elif d(resourceId="android:id/am_label", checked=True).exists:
            time_set += " " + "AM"
    ...
    if d(text="OK").exists:
        ...
        raise Exception, "OK button not found when setting time"
...
def set_timezone(new_timezone_name):
    ...
    if d(scrollable=True).scroll.to(text=new_timezone_name):
        ...
        raise Exception, 'Timezone %s not found' % (new_timezone_name)
    return tz_value
...
def get_date():
    ...
    try:
        ...
        raise SyntaxError("Unexpected format for date in Settings: %s." % date_str)
...
def set_date(date_string):
    ...
    if d(resourceId="android:id/date_picker_year").exists:
        d(resourceId="android:id/date_picker_year").click()
    else:
        raise Exception, "Year button not found in date picker"
    ...
    if d(descriptionContains=new_year).exists:
        d(descriptionContains=new_year).click()
    else:
        raise Exception, "%s year not found in year picker" % new_year

    if not d(resourceId="android:id/animator").child(descriptionContains=new_month).exists:
        i = 0
        while not d(resourceId="android:id/animator").child(descriptionContains="January").exists and not d(
                resourceId="android:id/animator").child(descriptionContains=new_month).exists and i < 12:
            d(scrollable=True).scroll.backward(steps=55)
            i += 1
        i = 0
    ...
    if d(descriptionContains=str(new_day) + ' ' + new_month + ' ' + str(new_year)).exists:
        d(descriptionContains=str(new_day) + ' ' + new_month + ' ' + str(new_year)).click()
    else:
        raise Exception, "%s date not found in date picker" % date_string
    ...
    if d(text="OK").exists:
        ...
        raise Exception, "OK button not found when setting date"
...
def get_time_format():
    ...
    try:
        datetime.datetime.strptime(time_format, "%H:%M")
        return_format = "24-hour"
    except ValueError:
        datetime.datetime.strptime(time_format, "%H:%M %p")
        return_format = "AM/PM"
...
def set_time_format():
    ...
    """
    if d(text="Use 24-hour format").right(className="android.widget.Switch").exists:
        if d(text="Use 24-hour format").right(className="android.widget.Switch").checked:
            d(text="Use 24-hour format").right(className="android.widget.Switch").click()
            if not d(text="Use 24-hour format").right(className="android.widget.Switch").checked:
                return True
            else:
                return False
        elif not d(text="Use 24-hour format").right(className="android.widget.Switch").checked:
            d(text="Use 24-hour format").right(className="android.widget.Switch").click()
            if d(text="Use 24-hour format").right(className="android.widget.Switch").checked:
                return True
            else:
