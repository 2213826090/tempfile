...
:summary: This file implements the AndroidDeviceBase reference phone for ACS Dev
:since: 11/10/2011
:author: jreynaud
"""
import imp
...
from UtilitiesFWK.Utilities import AcsConstants, Global, run_local_command, internal_shell_exec
from UtilitiesFWK.CommandLine import CommandLine
import UtilitiesFWK.DateUtilities as DateUtil
from Device.Model.AndroidDevice.Agent.Factory import get_acs_agent_instance
...
class AndroidDeviceBase(DeviceBase):
    ...
    def uiautomator(self):
        if not self.__uiautomator:
            from uiautomator import Device as UiAutomatorDevice
            self.__uiautomator = UiAutomatorDevice(self._serial_number)
            # TODO : raise an exception if None ?
    ...
    def get_screenshot_repo(self):
        """
        Return the path to store screenshots
        """
        return self._sc_repo
