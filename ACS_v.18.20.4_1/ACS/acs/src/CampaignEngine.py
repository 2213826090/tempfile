...
:summary: Campaign Engine, entry point of ACS Host Part
:author: cbresoli
...
import socket
import uuid
from UtilitiesFWK.Utilities import Obfuscated
...
from Core.CampaignMetrics import CampaignMetrics  # noqa
from Core.FileParsingManager import FileParsingManager  # noqa
# TestCaseManager requires Equipments, it shall be after adding acs_test_scripts to the system path
from Core.TestCaseManager import TestCaseManager  # noqa
from Core.CampaignGenerator.CampaignGeneratorFactory import CampaignGeneratorFactory  # noqa
from Core.CatalogParser.ParameterCatalogParser import ParameterCatalogParser  # noqa
from Core.CatalogParser.UseCaseCatalogParser import UseCaseCatalogParser  # noqa
from Core.CatalogParser. TestStepCatalogParser import TestStepCatalogParser  # noqa
from Device.DeviceManager import DeviceManager  # noqa
from ErrorHandling.AcsBaseException import AcsBaseException  # noqa
from ErrorHandling.DeviceException import DeviceException  # noqa
from Core.Report.ACSLogging import ACSLogging, LOGGER_FWK, LOGGER_FWK_STATS  # noqa
from Core.Report.CampaignReportTree import CampaignReportTree  # noqa
from Core.Report.DebugTestReport import DebugReport  # noqa
from Core.Report.Live.LiveReporting import LiveReporting  # noqa
from Core.Report.TestReport import Report  # noqa
from Core.Report.scripts.acsZipPush import ZipFolderUtilities  # noqa
from Core.PathManager import Paths  # noqa
from Core.PathManager import Files  # noqa
from UtilitiesFWK.UuidUtilities import is_uuid4  # noqa
from acs_test_scripts.Equipment.EquipmentManager import EquipmentManager  # noqa
from Core.Report.scripts.ComputeReportChecksum import ComputeReportChecksum  # noqa
import UtilitiesFWK.Utilities as Util  # noqa
from Core.ArgChecker import ArgChecker  # noqa
...
class CampaignEngine:
    ...
    def __init__(self, ignored_tc_lst=None, report_folder=None, stats_server="sdlunk.tl.intel.com", stats_port=514):

        ACSLogging.initialize(stats_server, stats_port)

        self.__ignored_tc_lst = ignored_tc_lst
    ...
    def __init_live_reporting(self, campaign_name, campaign_uuid, user_email, live_reporting_plugin, rerun):
        ...
        """
        tcr_dev_campaign = self.__global_config.campaignConfig.get("TCRReportEnvironment", "prod").lower() == "dev"
        self._tcr_reporting_on_rest_api = Util.str_to_bool(
            self.__global_config.campaignConfig.get("TCRReportViaRESTAPI", "False"))
        tcr_reporting_on_tsmb = Util.str_to_bool(self.__global_config.campaignConfig.get("ReportViaMsgBus", "False"))
        # if campaign parameters TCRReportViaRESTAPI = True and ReportViaMsgBus = True,
        # There is a conflict for pushing datas on TCR, so filling TCR via rest api is the default choice.
        if tcr_reporting_on_tsmb and self._tcr_reporting_on_rest_api:
            tcr_reporting_on_tsmb = False
        self._live_reporting_interface = LiveReporting.instance()
        self._live_reporting_interface.init(self.campaign_report_path,
                                            enable_tcr=self._tcr_reporting_on_rest_api,
                                            tcr_dev_report=tcr_dev_campaign,
                                            enable_tsmb=tcr_reporting_on_tsmb,
                                            campaign_uuid=campaign_uuid,
                                            live_reporting_plugin=live_reporting_plugin,
                                            rerun=rerun)
        self._live_reporting_interface.enable_live_reporting(True)
    ...
    def _log_acs_param_extra(self, acs_params):
        ...
        is_tcr_report_enabled = Util.str_to_bool(
            self.__global_config.campaignConfig.get("TCRReportViaRESTAPI", "False"))
        params += "tcr_report={0}".format(is_tcr_report_enabled)
        params += "; metacampaign_uuid={0}".format(self.__test_report.metacampaign_uuid)
    ...
    def execute(self, is_arg_checking=True, **kwargs):
        ...
        try:
            ...
            campaign_name = os.path.basename(campaign_name)
            ...
            if self._live_reporting_interface is not None:
                ...
                if self._tcr_reporting_on_rest_api:
                    self._live_reporting_interface.send_campaign_resource(
                        campaign_uuid=self.__test_report.metacampaign_uuid,
                        resource_info={
                            "campaign_report_path": self.campaign_report_path,
                            "campaign_elements": self._campaign_elements,
                            "user_email": user_email,
                            "dev_campaign": self.__global_config.campaignConfig.get("TCRReportEnvironment",
                                                                                    "prod").lower() == "dev"
                        })
            ...
            if self.campaign_report_path is not None:
                ...
                # Compute checksum
                ComputeReportChecksum.compute(self.campaign_report_path)
                ZipFolderUtilities.Zip(self.campaign_report_path, self.campaign_report_path)
