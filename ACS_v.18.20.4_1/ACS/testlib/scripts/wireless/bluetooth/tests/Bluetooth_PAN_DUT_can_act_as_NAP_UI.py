...
#######################################################################
#
# @description: DUT can act as NAP
# @note:        Connect to Wifi on DUT and close Wifi on DEV must be
...
try:
    ...
    bluetooth_steps.BtSetTethering(serial=serial, state="ON", tethering_settings_opened=False, version=DUT_VERSION)()
    # activate internet service on DEV
    bluetooth_steps.BtSetService(serial=serial_dev, paired_device_name=DUT_NAME, state=True,
                                 service="Internet access", version=DEV_VERSION)()
    bluetooth_steps.WaitForState(serial=serial_dev, device_name=DUT_NAME, connected=True, version=DEV_VERSION)()
    bluetooth_steps.CheckInternetAccessServiceState(serial=serial_dev, state="ON", max_checks=15, version=DEV_VERSION)()
    ...
    # DUT: turn off bt tethering, turn on BT if not already
    bluetooth_steps.StopPackage(serial=serial, critical=False)()
    bluetooth_steps.PressHome(serial=serial, critical=False)()
    bluetooth_steps.BtSetTethering(serial=serial, state="OFF", tethering_settings_opened=False, version=DUT_VERSION,
                                   critical=False)()
    ...
    bluetooth_steps.BtRemoveAllPairedDevices(serial=serial, critical=False, version=DUT_VERSION)()
    ...
    bluetooth_steps.BtRemoveAllPairedDevices(serial=serial_dev, critical=False, version=DEV_VERSION)()
