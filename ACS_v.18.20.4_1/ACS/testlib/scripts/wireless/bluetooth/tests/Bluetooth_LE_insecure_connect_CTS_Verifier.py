...
#######################################################################
#
# @description: scenarios for InsecureConnect via CTS app
# @usage:       Bluetooth_LE_insecure_connect_CTS_Verifier.py
...
class CtsVerifierInsecureClient(BtStep):
    """ Description:
            Makes sure that the CTS scanning progress is finished.
        Usage:
            CtsVerifierInsecureClient(serial=serial,
    ...
    def __init__(self, timeout_appear=5000, time_to_wait=60000, **kwargs):
        ...
        self.step_data = True
        self.set_passm("ctsverifier InsecureClient scanning ")
    ...
    def do(self):
        try:
            ...
            if not self.uidevice(text="Insecure Client").wait.exists(timeout=self.timeout):
                raise Exception("Insecure Client Test object not found in menu list")
            self.uidevice(text="Insecure Client").click()
...
class CtsVerifierInsecureServer(BtStep):
    """ Description:
            Makes sure that the CTS app in discoverable mode.
        Usage:
            CtsVerifierInsecureServer(serial=serial,
                                 timeout_appear=5000, time_to_wait=60000, version=version)()
    """
    ...
    def __init__(self, timeout_appear=5000, time_to_wait=60000, **kwargs):
        ...
        self.list = self.uidevice(packageName="com.android.cts.verifier")
        self.step_data = True
        self.set_passm("ctsverifier InsecureServer in Discoverable mode ")
    ...
    def do(self):
        try:
            ...
            if self.uidevice(text="OK").wait.exists(timeout=self.timeout):
                self.uidevice(text="OK").click()
            if not self.uidevice(text="Insecure Server").wait.exists(timeout=self.timeout):
                raise Exception("Insecure Server object not found in menu list")
            self.uidevice(text="Insecure Server").click()
            if self.uidevice(text="OK").wait.exists(timeout=self.timeout):
                self.uidevice(text="OK").click()
...
class CtsVerifierInsecureConnect(BtStep):
    """ Description:
    ...
    def __init__(self, dev_name, timeout_appear=5000, time_to_wait=60000, **kwargs):
        """
        :param timeout_appear: time to wait till the scanning progress bar appears
        :param time_to_wait: time to wait till the scanning progress bar is gone
        :param kwargs: serial, version, no_log and standard kwargs for base_step
...
try:
    ...
    CtsVerifierInsecureClient(serial=serial,version=DUT_VERSION)()
    CtsVerifierInsecureServer(serial=serial_dev, version=DEV_VERSION)()
    CtsVerifierInsecureConnect(serial=serial,dev_name=PAIRING_DEV_NAME)()
finally:
