...
#######################################################################
#
# @filename:    Bluetooth_OPP_DUT_Send_vCard_Accept.py
# @description: DUT sends vCard and smartphone accepts the request
...
DUT_NAME = 'DUT'
PAIRING_PAIRING_DEV_NAME = 'Smartphone'
...
uidevice = ui_device(serial = PAIRING_DEV, port = adb_server_port)
...
while uidut.exists(text = CONTACT_NAME):
...
ui_steps.press_home(serial = PAIRING_DEV)()
ui_steps.open_app_from_allapps(serial = PAIRING_DEV,
                               view_to_find = {'text': 'People'}
                              )()

#select a contact
if not uidevice(text = CONTACT_NAME).exists:
    ui_steps.click_button(serial = PAIRING_DEV,
                          port = adb_server_port,
                          view_to_find = {
                                'descriptionContains': 'Add Contact'
                                         },
    ...
    ui_steps.edit_text(serial = PAIRING_DEV,
                       port = adb_server_port,
                       view_to_find = {'text' : 'Name'},
                       value = CONTACT_NAME
                      )()
...
bluetooth_steps_old.bt_share_contact(serial = PAIRING_DEV,
                                 port = adb_server_port,
...
bluetooth_steps_old.bt_accept_contact(serial = serial,
                                  port = adb_server_port,
                                  contact_name = CONTACT_NAME
                                 )()
