...
for i in range(9):
    if i == 7:
        last_attempts = True
    telephony_steps.unlock_sim(serial = serial, puk = wrong_puk_code,
                                last_attempts = last_attempts, correct = False,
                                new_pin = sim_pin)()
telephony_steps.unlock_sim(serial = serial, puk = puk_code, new_pin = sim_pin)()
telephony_steps.check_pin_is_requested(serial = serial)()
