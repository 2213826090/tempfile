...
try:
	...
	system_corrupted_verity_metadata_path = config.get(image_platform, "system_corrupted_verity_metadata")
	system_corrupted_verity_metadata_name = system_corrupted_verity_metadata_path.split("/")[-1]
	fastboot_utils.download_file(url=fastboot_path+system_corrupted_verity_metadata_path, local_filename="./temp/image/n/img/"+system_corrupted_verity_metadata_name)

	adb_steps.reboot(command="fastboot", reboot_timeout=300, serial=serial)()
	fastboot_steps.flash_image(partition_name="system", file_name="./temp/image/n/img/"+system_corrupted_verity_metadata_name, serial=serial)()
