...
try:
    ...
    os.system("adb shell touch /data/temp.txt")
    adb_steps.reboot(command = "fastboot", reboot_timeout = 300, serial = serial)()
    fastboot_steps.flash_image(partition_name = partition_name, file_name = "./temp/image/n/flashfiles/"+file_name, unlock_dut = False, serial = serial)()
    fastboot_steps.continue_to_adb(serial=serial)()
    time.sleep(60)
    local_steps.wait_for_adb(timeout = 300, serial=serial)()

    file_exists = fastboot_utils.adb_command_file_exists(command = "adb shell ls /data/temp.txt")
    if not file_exists: raise Exception("The test result did not achieve the desired results")

    os.system("sudo rm -rf ./temp")
    os.system("adb shell rm /data/temp.txt")
