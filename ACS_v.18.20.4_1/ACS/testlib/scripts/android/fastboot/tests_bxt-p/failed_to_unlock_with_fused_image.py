...
try:
	...
	test_result = False
	fastboot_steps.command(command="flashing get_unlock_ability > ./temp/files/flashing_get_unlock_ability.txt 2>&1", serial=serial)()
	return_result = open("./temp/files/flashing_get_unlock_ability.txt").readlines()
	for line in return_result:
		if "The device class does not permit to unlock it." in line: test_result = True
	if not test_result:
