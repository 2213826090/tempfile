...
try:
	...
	fastboot_utils.download_file(url=fastboot_path+n_ota_zip_path, local_filename="./temp/image/n/"+n_ota_zip_name)

	adb_steps.reboot(command="fastboot", reboot_timeout=300, serial=serial)()
	fastboot_steps.unlock_device(serial=serial)()
	fastboot_steps.command(command="update ./temp/image/n/"+n_img_zip_name+" > ./temp/files/update_img_result.txt 2>&1", serial=serial)()
	time.sleep(120)
	local_steps.wait_for_adb(timeout=300, serial=serial)()

	return_result = fastboot_utils.fastboot_command_result(file_name="./temp/files/update_img_result.txt")
	if not return_result:
		raise Exception("The test result did not achieve the desired results")

	fastboot_utils.unpack_the_zip(file_name="./temp/image/n/"+n_ota_zip_name, temp_path=r"./temp/image/n/ota")
