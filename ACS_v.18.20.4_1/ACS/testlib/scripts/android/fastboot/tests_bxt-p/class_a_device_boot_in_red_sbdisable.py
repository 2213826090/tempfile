...
try:
	...
	user_signed_corrupted_boot_path = config.get(image_platform, "user_signed_corrupted_boot")
	user_signed_corrupted_boot_name = user_signed_corrupted_boot_path.split("/")[-1]
	fastboot_utils.download_file(url=fastboot_path+user_signed_corrupted_boot_path, local_filename="./temp/image/n/img/"+user_signed_corrupted_boot_name)
	...
	fastboot_steps.flash_image(partition_name="boot", file_name="./temp/image/n/img/"+user_signed_corrupted_boot_name, serial=serial)()
	...
	for line in return_result:
		if "chainloading boot image, boot state is red" in line: check_point1 = True
