...
try:
	...
	vendor_corrupted_verity_path = config.get(image_platform, "vendor_corrupted_verity")
	vendor_corrupted_verity_name = vendor_corrupted_verity_path.split("/")[-1]
	fastboot_utils.download_file(url = fastboot_path + vendor_corrupted_verity_path, local_filename = "./temp/image/n/img/" + vendor_corrupted_verity_name)

	adb_steps.reboot(command = "fastboot", reboot_timeout = 300, serial = serial)()
	fastboot_steps.flash_image(partition_name = "vendor", file_name = "./temp/image/n/img/"+vendor_corrupted_verity_name, lock_dut = True, serial = serial)()
