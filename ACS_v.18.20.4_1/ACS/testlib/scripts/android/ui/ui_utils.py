        ...
        def create_handler_function(self):
            ...
            def function_template(*args, **kwargs):
                if obj.uidevice(**obj.selector).exists:
                    print "[ {0} ]: Handler {1} has been triggered".format(str(obj.serial),
                                                                       obj.handler_name)
                    if obj.action == "click":
        ...
        for handler_name, handler_value in ui_statics.handler_list.iteritems():
            if ui_handler_group in handler_value['groups'] and\
               handler_name not in uidevice.handlers():
                ...
                print "[ {0} ]: Handler {1} has been registered".format(serial,
                                                                    handler_name)
...
def get_triggered_watchers(serial = None):
    ...
    if adb_connection.check_connected():
        try:
            ...
            if "RPC" not in str(e.message) and "not connected" not in str(e.message) and "not attached" not in str(e.message):
                print strftime("%d/%m %X.%S0 ", time.localtime()) + "Registered watchers query exception due to lack of adb connectivity: {0}".format(e.message)
                raise
            else:
                print strftime("%d/%m %X.%S0 ", time.localtime()) + "Watchers could not be queried due to exception: {}".format(e.message)
                ...
                if watcher_name not in uidevice.watchers:
                    try:
                        if watcher_value['action'] == "click":
                            ...
                            print strftime("%d/%m %X.%S0 ", time.localtime()) + "Watcher {0} has been  registered for {1}.".format(watcher_name, serial)
                        elif watcher_value['action'] == "press":
                            uidevice.watcher(watcher_name).\
                                        when(**watcher_value['selector']).press(watcher_value['action_view'])
                            print strftime("%d/%m %X.%S0 ", time.localtime()) + "Watcher {0} has been  registered for {1}.".format(watcher_name, serial)
                        else:
                            print strftime("%d/%m %X.%S0 ", time.localtime()) + "Invalid watcher action: {0}".format(watcher_value['action'])
                    except Exception as e:
                        if "RPC" not in str(e.message) and "not connected" not in str(e.message) and "not attached" not in str(e.message):
                            print strftime("%d/%m %X.%S0 ", time.localtime()) + "Watcher registration exception due to lack of adb connectivity: {0}".format(e.message)
                            raise
                        else:
                            print strftime("%d/%m %X.%S0 ", time.localtime()) + "Watcher {} could not be registered due to exception: {}".format(watcher_name,e.message)
    ...
    try:
        for watcher in uidevice.watchers:
            ...
            for ui_watcher_group in ui_watcher_groups:
                if ui_statics.watcher_list.has_key(watcher) and ui_watcher_group in ui_statics.watcher_list[watcher]['groups']:
                    break
                else:
                    found = False
            if not found:
        ...
        if "RPC" not in str(e.message) and "not connected" not in str(e.message) and "not attached" not in str(e.message):
            print "[ {0} ]: Remove non-group watchers due to lost adb connection exception: {1}".format(serial, e.message)
            raise
        else:
            print "[ {0} ]: Non group watchers could not be unregistered due to exception: {1}".format(serial, e.message)
...
def click_apps_entry(view_to_find, serial = None, app = True):
    ...
    while True:
        ...
        first_app = uidevice(className = "android.widget.TextView",
                                      instance = 3)
...
def is_switch_on(view_to_find, right_of = False, serial = None):
    ...
    if serial:
        ...
        uidevice = ui_device()
    if right_of:
        switch = uidevice(**view_to_find).right(
...
def is_text_visible(text_to_find, serial = None):
    ...
    """
    return is_view_visible(serial = serial,
                           view_to_find = {"text": text_to_find})
...
def is_text_visible_scroll_left(text_to_find, serial = None):
    ...
    """
    return is_view_visible_scroll_left(serial = serial,
                                       view_to_find = {"text": text_to_find})
...
def move_slider(view_to_find, position = 50, x_min_delta = 16, x_max_delta = 5):
...
def is_device_pin_locked(serial = None):
    ...
    if uidevice(resourceId = "android:id/message"):
        is_blocked = "You have incorrectly typed your PIN 5 times." in uidevice(resourceId = "android:id/message").info["text"]
        return is_blocked
...
def google_account_exists(serial = None,
    ...
    """
    return int(adb_utils.sqlite_count_query(serial = serial,
                                            db = db,
                                            table = table,
                                            where = where)) == 1
...
def is_homescreen(serial = None, sim_pin_enabled = False):
    ...
    if sim_pin_enabled:
        return  is_view_displayed(serial = serial,
