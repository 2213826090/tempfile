...
class click_button(ui_step):
    ...
    def __init__(self, view_to_find, view_to_check = None,
            view_presence = True, wait_time = 20000, wait_for_event_occurs = True,
            child_view_to_find = None, right_view_to_find = None, **kwargs):
        ui_step.__init__(self, **kwargs)
        self.view_to_find = view_to_find
        self.view_to_check = view_to_check
        self.view_presence = view_presence
        self.wait_time = wait_time
        self.wait_for_event_occurs = wait_for_event_occurs
        self.child_view_to_find = child_view_to_find
        self.right_view_to_find = right_view_to_find
        self.set_errorm("", "Could not click {0} checking {1}".format(view_to_find, view_to_check))
        self.set_passm("{0} clicked checking {1}".format(view_to_find, view_to_check))

    def do(self):
        wait_for_view(view_to_find = self.view_to_find,
                          serial = self.serial)()
        if self.child_view_to_find is not None:
            self.uidevice(**self.view_to_find).\
                    child(**self.child_view_to_find).click.wait()
            return
        if self.right_view_to_find is not None:
            self.uidevice(**self.view_to_find).\
                    right(**self.right_view_to_find).click.wait()
            return
        if self.wait_for_event_occurs:
            self.uidevice(**self.view_to_find).click.wait()
        else:
            self.uidevice(**self.view_to_find).click()

    def check_condition(self):
        if self.view_to_check == None:
            return True
        if self.view_presence:
            check_state = self.uidevice(**self.view_to_check).wait.exists(timeout = self.wait_time)
        else:
            check_state = self.uidevice(**self.view_to_check).wait.gone(timeout = self.wait_time)
        return check_state
...
class set_pin_screen_lock(ui_step):
    ...
    def do(self):
        ...
        click_button_with_scroll(serial = self.serial,
                    view_to_find = {"textContains": "Screen lock"})()
        if self.uidevice(text = "Confirm your PIN").wait.exists(timeout = self.wait_time):
            if self.device_info.confirm_pin_go_back is not None:
                click_button(serial = self.serial,
                    view_to_find = self.device_info.confirm_pin_go_back)()
            else:
            ...
            click_button(serial = self.serial,
                        view_to_find = {"textContains": "PIN"})()
            if self.uidevice(text = "Require PIN to start device").wait.exists(timeout = self.wait_time):
                if self.require_pin_to_start_device:
                    click_button(serial = self.serial,
                        view_to_find = {"textContains": "Require PIN to start device"})()
                    click_button_if_exists(serial = self.serial,
            ...
            if self.uidevice(text="Secure start-up").wait.exists(
                ...
                if self.require_pin_to_start_device:
                    click_button_if_exists(serial=self.serial,
                                               view_to_find={"text": "YES"})()
                else:
                    click_button_if_exists(serial=self.serial,
                                               view_to_find={"text": "NO"})()
            ...
            edit_text(serial = self.serial,
                      view_to_find = {"resourceId": "com.android.settings:id/password_entry"},
                                      ...
                                      is_password = True)()
            click_button(serial = self.serial,
                    view_to_find = {"resourceId":"com.android.settings:id/next_button"})()
            edit_text(serial = self.serial,
            ...
            click_button(serial = self.serial,
                    view_to_find = {"resourceId":"com.android.settings:id/next_button"})()
            click_button(serial = self.serial,
                    view_to_find = self.device_info.password_done_btn_id)()
    ...
    def check_condition(self):
        ...
        return self.uidevice(className="android.widget.TextView", resourceId="android:id/summary", text="PIN").wait.exists(timeout = self.wait_time)
...
class remove_pin_screen_lock(ui_step):
    ...
    def do(self):
        ...
        click_button_with_scroll(serial = self.serial,
                    view_to_find = {"textContains": "Screen lock"})()
        if self.uidevice(text = "Confirm your PIN").wait.exists(timeout = self.wait_time):
            edit_text(view_to_find = {"resourceId":
                    "com.android.settings:id/password_entry"},
                    value = self.dut_pin,
                    serial = self.serial,
                    is_password = True)()
        ...
        click_button(serial = self.serial,
                    view_to_find = {"textContains": self.new_mode})()
        ...
        if self.uidevice(textContains = self.device_info.remove_pin_confirm_desc).wait.exists(timeout = self.wait_time):
            click_button(serial = self.serial,
                    view_to_find = {"text":self.device_info.remove_pin_confirm_button})()

    def check_condition(self):
        if self.uidevice(className="android.widget.ListView", scrollable=True).wait.exists(timeout = self.wait_time):
            self.uidevice(scrollable = True).scroll.to(text="Screen lock")
        return self.uidevice(className="android.widget.TextView", resourceId="android:id/summary", text=self.new_mode).wait.exists(timeout = self.wait_time)
...
class open_security_settings(adb_step):
    ...
    def do(self):
        am_start_command(serial = self.serial,
...
class am_start_command(adb_step):
    ...
    def do(self):
        if self.package:
            ...
            self.process = self.adb_connection.run_cmd(
        ...
        if self.component:
            ...
            self.step_data = self.adb_connection.run_cmd(
    ...
    def check_condition(self):
        ...
        if self.view_to_check is None:
            ...
            return self.step_data
        if self.view_presence:
            self.step_data = self.uidevice(**self.view_to_check).wait.exists(
                timeout = self.timeout)
        else:
            self.step_data = self.uidevice(**self.view_to_check).wait.gone(
                timeout = self.timeout)
        return self.step_data
...
class am_stop_package(adb_step, ui_step):
    ...
    def __init__(self, package_name, view_to_check=None, view_presence=True,
                 timeout=5000, **kwargs):
        """
    ...
    def do(self):
        try:
            self.adb_connection.run_cmd("am force-stop " + str(self.package_name))
            self.step_data = True
        except Exception, e:
...
class click_view(ui_step):
    ...
    def __init__(self, view, view_to_check = None,
        ...
        self.view_presence = view_presence
        self.wait_time = wait_time
        self.set_errorm("", "Could not click view {0} checking {1}".format(view, view_to_check))
        self.set_passm("View {0} clicked checking {1}".format(view, view_to_check))
...
class open_notifications_menu(ui_step):
    ...
    def do(self):
        swipe(serial = self.serial, sx = self.x_center, sy = 1, ex = self.x_center, ey = self.y_center, steps = 10)()
        time.sleep(1)
        swipe(serial = self.serial, sx = self.x_center, sy = 1, ex = self.x_center, ey = self.y_center, steps = 10)()
        time.sleep(1)
...
class click_xy(ui_step):
    ...
    def __init__(self, x, y, view_to_check = None, **kwargs):
        ...
        self.set_errorm("", "Could not click coordinates ({0} x {1})".format(x, y))
...
class click_button_with_scroll(ui_step):
    ...
    def __init__(self, view_to_find, view_to_check = None, view_presence = True,
        ...
        self.view_presence = view_presence
        self.wait_time = wait_time
        self.set_errorm("", "Could not click {0} with scroll checking {1}".format(view_to_find, view_to_check))
        self.set_passm("{0} clicked with scroll checking {1}".format(view_to_find, view_to_check))

    def do(self):
        if not self.uidevice(**self.view_to_find).wait.exists(timeout = self.wait_time) and\
           self.uidevice(scrollable = True).wait.exists(timeout = self.wait_time):
...
class edit_text(ui_step):
    ...
    def __init__(self, view_to_find, value, is_password = False, scroll = False, clear_text = True, **kwargs):
        ...
        self.set_errorm("", "Could not edit {0} with {1}".format(view_to_find, value))

    def do(self):
        self.uidevice.wait.idle()
        if self.scroll and self.uidevice(className="android.widget.ScrollView", scrollable=True).wait.exists(timeout=1000):
            self.uidevice(scrollable = True).scroll.to(**self.view_to_find)
    ...
    def check_condition(self):
        ...
        if self.wait_time:
            if self.exists:
                ...
                self.uidevice(**self.view_to_check).wait.gone(timeout = self.wait_time)
        exists = self.uidevice(**self.view_to_check).wait.exists(timeout=1000)
        return exists if self.view_presence else not exists
...
class click_switch(ui_step):
    ...
    def __init__(self, view_to_find, state = "ON", click_to_close_popup = None,
        ...
        self.wait_time = wait_time
        self.set_passm("Set switch {0} to {1}".format(view_to_find, self.state))
        self.set_errorm("", "Could not set switch {0} to {1}".format(view_to_find, self.state))
    ...
    def check_condition(self):
        if self.right_of:
            ...
            self.switch = self.uidevice(**self.view_to_find).wait.exists(timeout=self.wait_time)
        return self.switch
...
class press_all_apps(ui_step):
    ...
    def do(self):
        if self.device_info.all_apps_icon != None:
            ...
            while not self.uidevice(text = "Calculator").wait.exists(timeout=1000):
                ...
                # swipe horizontally
                self.uidevice.swipe(1, int(maxy/2), int(maxx)-1, int(maxy/2))
                self.uidevice.swipe(int(maxx/2), int(maxy/2), int(maxx/2), int(maxy)-1)
    ...
    def check_condition(self):
        ...
        if self.dut_dessert == "M" and ro_name != "r0_bxtp_abl":
            return self.uidevice(resourceId = "com.android.launcher3:id/apps_list_view").\
                    wait.exists(timeout = 20000)
        elif self.dut_dessert == "L":
            return self.uidevice(descriptionContains = "Apps page 1 of").\
                    wait.exists(timeout = 20000)
...
class press_home(ui_step):
    ...
    def home_state(self):
        ...
        if self.uidevice(textContains="Google").wait.exists(timeout=100) or \
           ...
           self.uidevice(descriptionContains="Home screen", className="android.widget.LinearLayout").wait.exists(timeout=100) or \
           self.uidevice(resourceId="com.google.android.googlequicksearchbox:id/vertical_search_button").wait.exists(timeout=100) or \
           self.uidevice(resourceId="com.google.android.googlequicksearchbox:id/search_edit_frame").wait.exists(timeout=100) or \
...
class press_recent_apps(ui_step):
    ...
    def do(self):
        if not self.uidevice(resourceId="com.android.systemui:id/recents_view").wait.exists(timeout=1000) and\
               not self.uidevice(textContains="recent screens").wait.exists(timeout=1000):
            self.uidevice.press.recent()

    def check_condition(self):
        return self.uidevice(resourceId = "com.android.systemui:id/task_view_content").wait.exists(timeout=1000) or\
               self.uidevice(text = "Your recent screens appear here").wait.exists(timeout=1000)
...
class app_in_recent_apps(base_step):
    ...
    def __init__(self, app_name, target = "tablet", **kwargs):
        ...
        self.set_errorm("", "Could not find {0} in recent apps".format(self.app_name))
    ...
    def check_condition(self):
        ...
        if self.target == "tablet":
            return ui_utils.is_view_visible_scroll_left(view_to_find =
                                                        app_filter)
...
class open_app_from_recent_apps(android_step):
    ...
    def __init__(self, app_name, target = "tablet", **kwargs):
        ...
        self.set_errorm("", "Could not open {0} from recent apps".format(self.app_name))
...
class open_app_from_allapps(ui_step):
    ...
    def __init__(self, view_to_find, view_to_check = None,
        ...
        self.set_errorm("", "App {0} could not be opened applications page".format(view_to_find))
        self.set_passm("App {0} opened for applications page".format(view_to_find))

    def do(self):

        press_all_apps(serial = self.serial)()
        ui_utils.click_apps_entry(serial = self.serial,
...
class find_app_from_allapps(ui_step):
    ...
    def __init__(self, view_to_find, presence = True, **kwargs):
        ...
        self.set_errorm("", "App {0} was not found in applications page".format(view_to_find))
        self.set_passm("App {0} found in applications page".format(view_to_find))

    def do(self):
        press_all_apps(serial = self.serial)()

    def check_condition(self):
        return wait_for_view_with_scroll(serial = self.serial,
                                           view_to_find = self.view_to_find,
                                           iterations = 1,
...
class open_smart_lock_settings(ui_step):
    ...
    def do(self):
        ...
        self.uidevice.press("enter")
    def check_condition(self):
        return wait_for_view_with_scroll(serial = self.serial, timeout = 5000,
...
class open_settings(ui_step):
    ...
    """ description:
        ...
        tags:
            ui, android, press, click, settings, allapps
    """
    def __init__(self, intent = False, settings_check_point=".*?(S|s)ettings",
                 timeout=5000, **kwargs):
        self.intent = intent
    ...
    def check_condition(self):
        return wait_for_view_with_scroll(serial = self.serial, timeout = 5000,
                        view_to_find = {"textMatches":self.settings_check_point},
                        iterations = 1)()
...
class open_settings_app(ui_step):
    ...
    def __init__(self, view_to_find, view_to_check = None, view_presence = True,
        ...
        self.view_presence = view_presence
        self.wait_time = wait_time
        self.set_errorm("", "Could not open app {0} from settings checking {1}".format(view_to_find, view_to_check))
        self.set_passm("Open app {0} from settings checking {1}".format(view_to_find, view_to_check))

    def do(self):
        open_settings(serial = self.serial)()
        open_app_from_settings(serial = self.serial,
                               view_to_find = self.view_to_find)()
...
class open_app_from_settings(ui_step):
    ...
    def do(self):
        self.uidevice(scrollable = True).scroll.vert.toBeginning(steps=100, max_swipes=1000)
        self.uidevice(scrollable = True).scroll.to(**self.view_to_find)
        self.uidevice(**self.view_to_find).click.wait()
...
class open_playstore(ui_step):
    ...
    def check_condition(self):
        self.uidevice.wait.idle()
        return self.uidevice(text = "Add a Google Account").wait.exists(timeout=1000)\
               or self.uidevice(text="APPS",\
                        resourceId="com.android.vending:id/li_title").\
                        wait.exists(timeout = 20000)
...
class open_google_books(ui_step):
    ...
    def do(self):
        open_app_from_allapps(serial = self.serial,
                              view_to_find = {"text": "Play Books"})()

    def check_condition(self):
        self.uidevice.wait.idle()
        return self.uidevice(text = "Add a Google Account").wait.exists(timeout=100)\
               or self.uidevice(text = "Read Now").wait.exists(timeout=100)\
               or self.uidevice(text = "My Library").wait.exists(timeout=100)\
               or self.uidevice(text = "Settings").wait.exists(timeout=100)\
               or self.uidevice(text = "Help").wait.exists(timeout=100)
...
class add_google_account(ui_step):
    ...
    def do(self):
        ...
        if self.version == "L":
            click_button(serial = self.serial,
                         view_to_find = {"text" : "Accounts"},
                         view_to_check = {"text": "Add account"})()
            click_button(serial = self.serial,
                         view_to_find = {"text" : "Add account"},
        ...
        click_button(serial = self.serial,
                     ...
                     view_to_check= {"textContains":
                                     "Do you want to add an existing"})()

        click_button(serial = self.serial,
                     view_to_find = {"text":"Existing"},
        ...
        click_button(serial = self.serial,
                     view_to_find = {"descriptionContains":self.device_info.next_btn_id})()

        if self.version == "L":
            self.uidevice(textContains = "Set up payment").wait.\
                exists(timeout = 3000)
            click_button(serial = self.serial,
                         view_to_find = {"textContains":"Skip"},
                         wait_time = 3000,
                         view_to_check = {"text": "Google"})()
        elif self.version == "K":
            self.uidevice(serial = self.serial,
                          text = "Not now").wait.exists(timeout = 3000)
            click_button(serial = self.serial,
...
class add_app_from_all_apps_to_homescreen(ui_step):
    ...
    def do(self):
        ...
        self.x_coord = (app.info['bounds']['left'] + app.info['bounds'][
            'right']) / 2
        self.y_coord = (app.info['bounds']['bottom'] + app.info['bounds'][
            'top']) / 2
        swipe(serial = self.serial,
              sx=self.x_coord,
              sy=self.y_coord,
...
class uninstall_app_from_apps_settings(ui_step, base_step):
    ...
    def do(self):
        ...
        click_button(serial = self.serial,
                     view_to_find = {"textContains": "Uninstall"},
                     view_to_check = {"text": "OK"})()
        click_button(serial = self.serial,
                     view_to_find = {"text": "OK"})()
...
class open_display_from_settings(ui_step):
    ...
    def do(self):
        open_settings(serial = self.serial)()
        open_app_from_settings(serial = self.serial,
                               print_error = "Failed to open Display",
                               view_to_find = {"text":"Display"},
                               view_to_check = {"text":"Daydream"})()
...
class click_checkbox_button(ui_step):
    ...
    def __init__(self, view_to_find, state = "ON", confirm_view = None,
        ...
        self.wait_time = wait_time
        self.set_passm("Set the {0} checkbox to {1}".format(self.view_to_find, self.state))
        self.set_errorm("", "Could not set the {0} checkbox to {1}".format(self.view_to_find, self.state))
    ...
    def do(self):
        ...
        if (is_checked and self.state == "OFF") or (not is_checked and
            ...
            if self.confirm_view and self.uidevice(**self.confirm_view).wait.exists(timeout = self.wait_time):
                ...
                self.uidevice(**self.confirm_view).wait.gone(timeout = self.wait_time)
...
class enable_developer_options(ui_step):
    ...
    def do(self):
        if self.intent:
            ...
            open_settings(serial=self.serial)()
            click_button_common(serial=self.serial, view_to_find={
...
class disable_options_from_developer_options(ui_step):
    ...
    def __init__(self, developer_options, enabled = False, **kwargs):
        ...
        self.set_errorm("", "One or more options from {0} could not be disabled".format(developer_options))
    ...
    def do(self):
        if not self.enabled:
            ...
            click_button_common(serial = self.serial,
                         view_to_find = {"text": "Developer options"},
                         view_to_check = {"text": "Take bug report"})()
        version = adb_utils.get_android_version(serial = self.serial,
                                                full_version_name = True)
        is_switch = True
        if version == "5.0":
            is_switch = False
        for opt in self.dev_opts:
            click_checkbox_button(serial = self.serial,
                                  view_to_find = {"text": opt},
                                  state = "OFF",
                                  scrollable = True,
                                  is_switch = is_switch,
                                  relationship = "right")()
...
class  enable_options_from_developer_options(ui_step):
    ...
    def __init__(self, developer_options, enabled = False, confirm_view = "Enable", **kwargs):
        ...
        self.set_errorm("", "One or more options from {0} could not be enabled".format(developer_options))
    ...
    def do(self):
        ...
        for opt in self.dev_opts:
            click_checkbox_button(serial = self.serial,
                                  ...
                                  is_switch = is_switch,
                                  scrollable = True,
                                  confirm_view = {"text": self.confirm_view},
                                  relationship = "right")()
...
class enable_oem_unlock(ui_step):
    ...
    def do(self):
        ...
        click_checkbox_button(serial = self.serial,
                              view_to_find = {"text": oem_switch_text},
                              is_switch = is_switch,
                              scrollable = True,
                              confirm_view = self.device_info.oem_unlock_btn_id,
                              relationship = "right")()
...
class allow_unknown_sources(ui_step):
    ...
    def __init__(self, state = "ON", **kwargs):
        ...
        self.set_errorm("", "Allow unknown sources could not be set {0}".format(self.state))
    ...
    def do(self):
        ...
        if self.state == "ON":
            click_checkbox_button(serial = self.serial,
                                  view_to_find = {"text": "Unknown sources"},
                                  confirm_view = {"text": "OK"},
                                  state = self.state,
                                  is_switch = True,
                                  relationship = "right")()
        else:
    ...
    def check_condition(self):
        return ui_utils.is_checkbox_checked(serial = self.serial,
                                            view_to_find = {"text":
                                                            "Unknown sources"},
                                            is_switch = True,
                                            relationship = "right")
...
class put_device_into_sleep_mode(ui_step):
    ...
    def do(self):
        while adb_utils.is_power_state(serial = self.serial,
                                       state = "ON") and self.tries > 0:
            self.uidevice.sleep()
            time.sleep(3)
            self.tries -= 1

    def check_condition(self):
        return adb_utils.is_power_state(serial = self.serial,
                                        state = "OFF")
...
class unlock_device_pin(ui_step):
    ...
    def do(self):
        ...
        edit_text(serial = self.serial,
                  view_to_find = {"descriptionContains": "PIN area"},
                  is_password = True,
                  value = self.pin)()
        click_button(serial = self.serial,
                     view_to_find = {"descriptionContains": "Enter"})()
...
class unlock_device(ui_step):
    ...
    def do(self):
        ...
        if ui_utils.bxtp_car_locked(serial=self.serial):
            click_button(serial = self.serial, view_to_find={"resourceId": "com.android.systemui:id/user_name"})()
...
class scroll_to_text_from_scrollable(ui_step):
    ...
    def __init__(self, text_to_find = "United States",
        ...
        if direction == "down":
            self.start_x = int ((self.scrollable_view.info['bounds']['left'] + self.scrollable_view.info['bounds']['right'])/2)
            ...
            self.start_y = int ((self.scrollable_view.info['bounds']['top'] + self.scrollable_view.info['bounds']['bottom'])/2)
...
class perform_startup_wizard(ui_step):
    ...
    def do(self):
        ...
        if not self.uidevice(textContains = self.device_info.predefined_language_text_id).wait.exists(timeout = self.wait_time):
            if self.uidevice(resourceId = "android:id/numberpicker_input").wait.exists(timeout = self.wait_time):
                ...
                view_to_scroll = {"resourceId": "android:id/select_dialog_listview"}
            if not self.uidevice(textContains = self.device_info.predefined_language_text_id).wait.exists(timeout = self.wait_time):
                scroll_to_text_from_scrollable(text_to_find = self.device_info.predefined_language_text_id,
                                               serial = self.serial,
                                               view_to_scroll = view_to_scroll,
                                               iterations = 200,
                                               direction = "down",
                                               critical = False,
                                               blocking = False)()
                ...
                click_button(serial = self.serial,
                             view_to_find = {"textContains": "United State"})()
        if self.device_info.dessert == "M":
            click_button(serial = self.serial,
                             view_to_find = {"textContains": "United State"})()
        ...
        if self.uidevice(text = "Set up as new").wait.exists(timeout = self.wait_time):
            click_button(serial = self.serial,
                         view_to_find = {"text": "Set up as new"},
        ...
        if self.uidevice(textContains = "SIM").wait.exists(timeout = self.wait_time):
            if self.uidevice(textContains = "Skip").wait.exists(timeout = self.wait_time):
                ...
                click_button(serial = self.serial,
                         view_to_find = {"text": self.device_info.next_btn_id},
                         wait_time = self.wait_time,
                         view_to_check = {"textContains": "a SIM for calls"})()
                print "[ {0} ]: Selecting a SIM for calls".format(self.serial)
                click_button(serial = self.serial,
                         view_to_find = {"className": "android.widget.RadioButton", "instance": 1},
                         wait_time = self.wait_time,
                         view_to_check = {"textContains": "a SIM for calls"})()
                click_button(serial = self.serial,
                         view_to_find = {"text": self.device_info.next_btn_id},
                         wait_time = self.wait_time,
                         view_to_check = {"textContains": "a SIM for text messages"})()
        ...
        click_button(serial = self.serial,
                     view_to_find = self.device_info.skip_wifi_btn_id,
                     view_to_check = self.device_info.wifi_skip_anyway_btn_id)()
        click_button(serial = self.serial,
                     view_to_find = self.device_info.wifi_skip_anyway_btn_id)()
        ...
        if self.uidevice(textContains = "Checking connection").wait.exists(timeout = self.wait_time):
            ...
            wait_time = 0
            while not self.uidevice(descriptionContains = "Add your account").wait.exists(timeout=1000) and wait_time < timeout:
                wait_time += self.wait_time
                ...
                if self.uidevice(textContains = "Got another device").wait.exists(timeout = self.wait_time):
                    ...
                    click_button(serial = self.serial,
                                 view_to_find = {"text": self.device_info.next_btn_id})()
            print "[ {0} ]: Checking connection page disappeared in {1} seconds".format(self.serial, int(wait_time / 1000))
            if self.uidevice(descriptionContains = "Add your account").wait.exists(timeout=1000):
                ...
                wait_time = 0
                while not self.uidevice(descriptionContains = "Or create a new account").wait.exists(timeout=1000) and wait_time < timeout:
                    wait_time += self.wait_time
                    time.sleep(self.wait_time / 1000)
                    wake_up_device(serial = self.serial)()
                if wait_time < timeout:
                    print "[ {0} ]: 'Or create a new account' option appeared in {1} seconds".format(self.serial, int(wait_time / 1000))
                    wait_time = 0
                    ...
                    if wait_time < timeout:
                        ...
                        click_button(serial = self.serial,
                                     view_to_find = {"resourceId": "skip"},
                                     view_to_check = {"descriptionContains": "Skip account setup"})()

                        self.uidevice.press(61)
                        self.uidevice.press("enter")
                        self.uidevice(descriptionContains = "Skip account setup").wait.gone(timeout = self.wait_time)
                    else:
                        print "[ {0} ]: Skip option did not appear before the timeout of {1} seconds".format(self.serial, int(timeout))
                else:
                    print "[ {0} ]: 'Or create a new account' option did not appear before the timeout of {1} seconds".format(self.serial, int(timeout))
        ...
        if self.uidevice(textContains = "Google services").wait.exists(timeout = self.wait_time):
            click_button(serial=self.serial,
                         view_to_find={"resourceId": "com.google.android.gms:id/suw_navbar_more"},
                         view_to_check={"text": self.device_info.next_btn_id})()
            click_button(serial=self.serial,
                         view_to_find={"text": self.device_info.next_btn_id},
        ...
        if self.uidevice(resourceId = "com.google.android.setupwizard:id/lock_screen_intro_check_box").wait.exists(timeout = self.wait_time):
            click_button(serial = self.serial,
                         view_to_find = {"resourceId":
                                         "com.google.android.setupwizard:id/lock_screen_intro_check_box"},
                         view_to_check = self.device_info.skip_pin_btn_id)()
        click_button(serial = self.serial,
                     view_to_find = self.device_info.skip_pin_btn_id,
                     view_to_check = self.device_info.skip_anyway_btn_id)()
        click_button(serial = self.serial,
                     view_to_find = self.device_info.skip_anyway_btn_id)()
        ...
        if self.uidevice(description = "More").wait.exists(timeout = self.wait_time):
            click_button(serial = self.serial,
                     view_to_find = {"description": "More"},
                     view_to_check = {"text": self.device_info.next_btn_id})()
        print "[ {0} ]: Finish setup wizard".format(self.serial)
        click_button(serial = self.serial,
                     view_to_find = self.device_info.finish_startup_btn_id)()
...
class perform_startup_wizard_for_new_user(ui_step):
    ...
    def do(self):
        ...
        click_button(serial = self.serial,
                     view_to_find = {"text": "Skip"},
                     view_to_check = {"text": "Skip anyway"})()

        click_button(serial = self.serial,
                     view_to_find = {"text": "Skip anyway"},
                     view_to_check = {"text": "Name"})()
...
class set_orientation(ui_step):
    ...
    def do(self):
        self.uidevice.orientation =\
            self.__orientation[self.target][self.orientation]
        time.sleep(1)

    def check_condition(self):
        return self.uidevice.orientation ==\
                   self.__orientation[self.target][self.orientation]


class wait_for_view(ui_step):

    """ description:

        usage:
            wait_for_view(view_to_find = {"text":"Songs"}, timeout = 20,
                          serial = serial)
    ...
    def __init__(self, view_to_find, timeout = 10000, **kwargs):
        ui_step.__init__(self, **kwargs)
        self.view_to_find = view_to_find
        self.timeout = timeout
        self.set_errorm("", "{0} view did not appear in {1} seconds".format(view_to_find, timeout))
        self.set_passm("{0} view appeared in {1} seconds".format(view_to_find, timeout))

    def do(self):
        self.step_data =  self.uidevice(**self.view_to_find).wait.exists(timeout = self.timeout)

    def check_condition(self):
        return self.step_data


class wait_for_view_with_scroll(ui_step):

    """ description:

        usage:
            wait_for_view_with_scroll(view_to_find = {"text":"Songs"}, timeout = 20,
                          serial = serial)
    ...
    def __init__(self, view_to_find, timeout = 10000, iterations = 10,
                 presence = True, **kwargs):
        ui_step.__init__(self, **kwargs)
        self.view_to_find = view_to_find
        self.timeout = timeout
        self.set_errorm("", "Did not find view {0} after {1} miliseconds.".format(view_to_find, timeout))
        self.set_passm("Found view {0} in less than {1} milisecconds".format(view_to_find, timeout))
        self.iterations = iterations
        self.presence = presence

    def do(self):
        while self.iterations > 0 and not self.uidevice(**self.view_to_find).wait.exists(timeout = int(self.timeout/self.iterations)):
            if self.uidevice(scrollable = True).wait.exists(timeout=1000):
                try:
                    self.uidevice(scrollable = True).scroll.to(**self.view_to_find)
                except Exception as e:
                    self.logger.debug("scrollable exception:{0}".format(str(e.message)))
                    self.uidevice().scroll.to(**self.view_to_find)
            else:
                self.uidevice().scroll.to(**self.view_to_find)
            self.iterations -= 1

    def check_condition(self):
        self.step_data = self.uidevice(**self.view_to_find).wait.exists(timeout=1000)
...
class click_button_if_exists(ui_step):

    """ description:
            clicks a button identified by <view_to_check>
            if is is present on the screen

        usage:
            ui_steps.click_button_if_exists(view_to_find = {"resourceId":
                    "com.intel.TelemetrySetup:id/button_allow"})()
    ...
    def __init__(self, view_to_find, wait_time = 20000, view_to_check = None,
            view_presence = True, **kwargs):
        ui_step.__init__(self, **kwargs)
        self.view_to_find = view_to_find
        self.view_to_check = view_to_check
    ...
    def do(self):
        if self.uidevice(**self.view_to_find).wait.exists(timeout = self.wait_time):
            self.uidevice(**self.view_to_find).click.wait()
            self.step_data = True
    ...
    def check_condition(self):
        ...
        if self.step_data:
            self.set_passm("View {0} exists and was clicked: {1}".format(self.view_to_find, self.step_data))
            self.set_errorm("", "View {0} exists and was clicked, but {1} was not found: {2}".format(self.view_to_find, self.view_to_check, self.step_data))
            if self.view_to_check == None:
                return True
            if self.view_presence:
                check_state = self.uidevice(**self.view_to_check).wait.exists(timeout = self.wait_time)
...
class close_app_from_recent(ui_step):
    ...
    def do(self):
        press_home(serial = self.serial)()
        press_recent_apps(serial = self.serial)()
        if ui_utils.swipe_to_app_from_recent(serial = self.serial,
                                             view_to_find = self.view_to_find):
            self.uidevice(**self.view_to_find).swipe.right()

    def check_condition(self):
        return not ui_utils.swipe_to_app_from_recent(serial = self.serial,
                                                     view_to_find = self.view_to_find)
...
class open_widget_section(ui_step):
    ...
    def do(self):
        press_home(serial = self.serial)()
        page_indicator = self.uidevice(resourceId = "com.google.android.googlequicksearchbox:id/page_indicator")
        x = (page_indicator.info["bounds"]["left"] + page_indicator.info["bounds"]["right"]) / 2
        y = (page_indicator.info["bounds"]["top"] + page_indicator.info["bounds"]["bottom"]) / 2
        swipe(serial = self.serial,
              sx = x,
              sy = y,
              ex = x,
...
class add_widget_to_homescreen(ui_step):
    ...
    def do(self):
        ...
        if ui_utils.is_text_visible_scroll_left(serial = self.serial,
                                                text_to_find = self.widget_name):
            widget = self.uidevice(text = self.widget_name)
            x = (widget.info["bounds"]["left"] + widget.info["bounds"]["right"]) / 2
            y = (widget.info["bounds"]["top"] + widget.info["bounds"]["bottom"]) / 2
            swipe(serial = self.serial,
                  sx = x,
                  sy = y,
                  ex = x,
...
class add_google_account_for_L(ui_step, adb_step):

    """ description:
            adds google account <account> from settings page

        usage:
            ui_steps.add_google_accout(account = "account_email",
                                      paswword = "account_password")()
    ...
    def __init__(self,
                 account = "intelchat002@gmail.com",
                 password = "intel002",
                 open_from_settings = True,
                 sync = None, from_gmail = False,
                 **kwargs):
        self.account = account
        self.password = password
        self.open_from_settings = open_from_settings
        self.sync = sync
    ...
    def do(self):
        if self.open_from_settings:
            open_add_google_account_wizard(serial = self.serial)()
        ...
        if self.uidevice(description="ACCEPT").wait.exists(timeout = 30000):
        # get object coords
            x, y = [c for c in ui_utils.get_center_coords(self.\
                            uidevice(description = "ACCEPT").\
                            info["bounds"])]
            for f in range(1,10):
                    cmd = "input tap {0} {1}".format(x,y)
                    self.adb_connection.run_cmd(cmd, timeout = 6)
                    if not self.uidevice(description = "ACCEPT").wait.exists(timeout=1000):
                        break

        self.uidevice(textContains="Checking info").wait.gone(timeout = 60000)

        if self.open_from_settings:
            if (self.acc_no == 0):
                click_button(serial = self.serial,
                     view_to_find = {"text":self.device_info.next_btn_id},
                     view_to_check ={"textContains":"Enter your billing"})()
            self.uidevice(text = "Remind me later").wait.exists(timeout = 5000)
            click_button(serial = self.serial,
                     view_to_find = {"text":"Remind me later"},
                     view_to_check ={"textContains":"Enter your billing"})()

            if (self.acc_no > 0):
                    click_button(serial = self.serial,
                         view_to_find = {"text":self.device_info.next_btn_id},
                         view_to_check ={"text":"Sync your account"})()
            click_button(serial = self.serial,
                         view_to_find = {"text":self.device_info.next_btn_id},
                         view_to_check ={"text":"Google"})()
            click_button(serial = self.serial,
            ...
            if (self.sync != None):
                for f in range(1,10):
                    if self.uidevice(textContains = "Last synced").wait.exists(timeout = 5000):
                        self.uidevice(textContains = "Last synced").click.wait()
                        break
        else:
            self.uidevice(text = self.device_info.next_btn_id).wait.exists(timeout = 20000)
            click_button(serial = self.serial,
                     view_to_find = {"text":self.device_info.next_btn_id})()

            if self.from_gmail:
                self.uidevice(text = self.account).wait.exists(timeout = 20000)
                click_button(serial = self.serial,
                     view_to_find = {"text":self.account})()

                click_button(serial = self.serial,
                     view_to_find = {"text":"Take me to Gmail"},
                     view_to_check = {"resourceId": "com.google.android.gm:id/compose_button"})()
                if not self.uidevice(text = "Primary").wait.exists(timeout = 5000):
                    for f in range(1,10):
                        if self.uidevice(text = "Primary").wait.exists(timeout = 5000):
                            self.uidevice(text = "Primary").click.wait()
                            break

    def check_condition(self):
        if self.open_from_settings:
            self.uidevice.wait.update()
            stat = self.uidevice(text = self.account).wait.exists(timeout=1000)
            if (self.sync != None):
                stat = self.uidevice(textContains = "Last synced").wait.exists(timeout = 5000)
            return stat
...
class open_google_account_for_edit(ui_step):
    ...
    def do(self):
        ...
        click_button(serial = self.serial,
                         view_to_find = {"text" : "Google"},
                         view_to_check = {"resourceId":"android:id/title",
                                        "text":"Accounts"})()
...
class remove_google_account(ui_step):
    ...
    def do(self):
        ...
        click_button(serial = self.serial,
                     view_to_find = {"textContains":self.account},
        ...
        click_button(serial = self.serial,
                     view_to_find = {"text":"Remove account"},
    ...
    def check_condition(self):
        ...
        close_app_from_recent(serial = self.serial,view_to_find=\
            {"text": "Settings"})()
...
class remove_all_google_accounts(ui_step):
    ...
    def do(self):
        while gms_utils.get_google_account_number(serial = self.serial) > 0:
            remove_google_account(serial = self.serial,
                                    account = "gmail.com")()
...
class show_as_list(ui_step):
    ...
    def do(self):
        ...
        self.uidevice.wait.idle()
        if not click_button_if_exists(serial = self.serial,
                        view_to_find = {"text":"List view"})():
            self.uidevice.press.back()
...
class close_all_app_from_recent(ui_step):
    ...
    def do(self):
        ...
        self.uidevice(resourceId =\
            "com.android.systemui:id/task_view_content").wait.exists(timeout = 20000)


        while (self.uidevice(resourceId =\
            "com.android.systemui:id/task_view_content").count > 0):
                self.uidevice(resourceId=\
                    "com.android.systemui:id/task_view_content").swipe.right()

    def check_condition(self):
        press_home(serial = self.serial)()
        press_recent_apps(serial = self.serial)()
        exist_stat = self.uidevice(text =\
            "Your recent screens appear here").wait.exists(timeout=1000)
        press_home(serial = self.serial)()
        return exist_stat
...
class set_timezone_from_settings(ui_step):
    ...
    def do(self):
        ...
        if not self.uidevice(text = self.timezone).wait.exists(timeout=1000):
            scroll_up_to_view(serial = self.serial,
                            view_to_check = {"text": self.timezone},
                                    ey=100, critical = False)()
        if not self.uidevice(text = self.timezone).wait.exists(timeout=1000):
            scroll_up_to_view(serial = self.serial,
                            view_to_check = {"text": self.timezone},
                            ey=500,
                            iterations = 20 )()
        click_button(serial = self.serial,
                            view_to_find = {"text": self.timezone},
                            view_to_check = {"text": "Select time zone"})()
    def check_condition(self):
...
class sync_google_account(ui_step):
    ...
    def do(self):
        try:
            while self.step_data != True and self.max_attempts > 0:
                ...
                if self.uidevice(text="Sync now").wait.exists(timeout = 60000):
                    ...
                    if self.uidevice(textContains="experiencing problems").wait.exists(timeout=1000):
                        self.step_data = False
                        handle_google_action_required(serial = self.serial,
    ...
    def do(self):
        try:
            ...
            if self.uidevice(text="Account Action Required").wait.exists(timeout = 20000):
                click_button(serial = self.serial,
                    view_to_find = {"text":"Account Action Required"},
                    view_to_check = {"text":"Try again"})()
                click_button(serial = self.serial,
                    view_to_find = {"text":"Try again"},
...
class set_orientation_vertical(ui_step):
    ...
    def do(self):
        ...
        set_orientation(serial = self.serial,
                     orientation = "portrait",
                     target = "phone")()
        # set the orientation to 'portrait'
        self.device_type = adb_utils.get_device_orientation_type(serial = self.serial)
        set_orientation(serial = self.serial,
...
class block_device(ui_step):
    ...
    def do(self):
        ...
        unlock_device_swipe(serial = self.serial)()
        for i in range(0, 5):
            unlock_device_pin(serial = self.serial, pin = self.pin, wrong_pin = True)()
...
class create_new_user(ui_step):
    ...
    def do(self):
        ...
        click_button(serial = self.serial,
                     view_to_find = {"textContains": "Users have their own"},
...
class remove_user(ui_step):
    ...
    def do(self):
        ...
        if not wait_for_view_common(serial=self.serial,
            ...
            return
        if not click_button_common(serial=self.serial,
                     view_to_find={"text": self.user_name},
                     second_view_to_find={"descriptionContains": "Delete user"},
...
class set_up_user(ui_step):
    ...
    def do(self):
        ...
        perform_startup_wizard_for_new_user(serial = self.serial, user_name = "New user")()
...
class add_trusted_location(ui_step):
    ...
    def do(self):
        open_smart_lock_settings(serial = self.serial,
                                 pin = self.pin)()
        click_button(serial = self.serial,
                view_to_find = {"text": "Trusted places"},
                view_to_check = {"text": "Add trusted place"})()
        click_button(serial = self.serial,
                view_to_find = {"text": "Add trusted place"})()
        ...
        if self.uidevice(text = "Select this location").wait.exists(timeout = self.wait_time):
            click_button(serial = self.serial,
                    view_to_find = {"text": "Select this location"},
                    view_to_check = {"resourceId": "com.google.android.gms:id/trusted_place_name"})()
        edit_text(serial = self.serial,
                  view_to_find = {"resourceId": "com.google.android.gms:id/trusted_place_name"},
...
class remove_trusted_location(ui_step):
    ...
    def do(self):
        ...
        if self.device_info.dessert == "M":
            click_button(serial = self.serial,
                    view_to_find = {"text": self.location_name},
                    view_to_check = {"resourceId": "com.google.android.gms:id/trusted_places_custom_places_menu_delete_this_location"})()
            click_button(serial = self.serial,
                         view_to_find = {"resourceId": "com.google.android.gms:id/trusted_places_custom_places_menu_delete_this_location"},
                         view_to_check = {"text": "Add trusted place"})()
        elif self.device_info.dessert == "L":
            click_button(serial = self.serial,
                    view_to_find = {"text": self.location_name},
            ...
            if ui_utils.is_display_direction_landscape(serial = self.serial):
                click_xy(serial = self.serial,
                         x = self.uidevice.info["displayWidth"] / 2,
                         y = self.uidevice.info["displayHeight"] *
                             self.device_info.remove_trusted_location_horizontal_percentage)()
            else:
                click_xy(serial = self.serial,
                         x = self.uidevice.info["displayWidth"] / 2,
                         y = self.uidevice.info["displayHeight"] *
                             self.device_info.remove_trusted_location_vertical_percentage)()
    def check_condition(self):
        # Check if user switched
        open_smart_lock_settings(serial = self.serial,
                                 pin = self.pin)()
...
class add_trusted_device(ui_step):
    ...
    def do(self):
        ...
        click_button(serial=self.serial,
                     view_to_find={"text": self.device_name},
                     view_to_check={"text": "YES, ADD"})()
        click_button(serial=self.serial,
                view_to_find = {"text": "YES, ADD"},
                view_to_check={"text": "Connected"})()
...
class set_date_and_time(ui_step):
    ...
    def do(self):
        ...
        if self.uidevice(resourceId = "android:id/switchWidget", instance = 1).info["text"] == "ON":
            click_button(serial = self.serial,
                             view_to_find = {"text": "Automatic time zone"},
...
class enable_disable_auto_time_date(ui_step):
    ...
    def __init__(self, serial, enable=True, **kwargs):
        ...
        self.set_errorm("", "Could not {0} auto time and date".format("enable" if self.enable else "disable"))
        self.set_passm("Successfully {0} auto time and date".format("enabled" if self.enable else "disabled"))
    ...
    def do(self):
        ...
        # If the option is enabled and the parameter is set to false, click to disable
    ...
    def check_condition(self):
        # If the option should be enabled and it isn't, return false (step fails)
        if self.enable and not self.auto_time_checkbox.info["checked"]:
            return False
...
class enable_disable_auto_timezone(ui_step):
    ...
    def __init__(self, time_zone_switch_value, **kwargs):
        ...
        self.set_errorm("", "Could not {0} auto timezone".format("enable" if self.time_zone_switch_value else "disable"))
        self.set_passm("Successfully {0} auto timezone".format("enabled" if self.time_zone_switch_value else "disabled"))
...
class press_map(ui_step):
    """ description:
        ...
        """
    def check_view(self):
        if self.uidevice(**self.view_to_check).wait.exists(
            timeout=self.timeout):
            return True
        if self.uidevice(**{"text":"Google Maps",
    ...
    def __init__(self, view_to_check={
            "resourceId":"com.android.support.car.lenspicker:id/stream_card",
            "packageName":"com.android.support.car.lenspicker"},
                         timeout = 5000, **kwargs):
        ui_step.__init__(self, **kwargs)
        self.view_to_check = view_to_check
        self.timeout = timeout
    ...
    def do(self):
        if self.device_info.platform not in ["gordon_peak"]:
            ...
            map_x_coordinate = activity_bar_single_element_width * 2 - \
...
class press_dialer(ui_step):
    ...
    def do(self):
        if self.device_info.platform not in ["gordon_peak"]:
            ...
            # dialer resides at 3 position and click has to be done at the center
            dialer_x_coordinate = activity_bar_single_element_width * 3 - \
                               activity_bar_single_element_width / 2
    ...
    def check_condition(self):
        if self.step_data == True:
           self.step_data = wait_for_view(serial=self.serial, view_to_find={
...
class press_media(ui_step):
    ...
    def __init__(self, view_to_check={
        "resourceId":"com.android.support.car.lenspicker:id/stream_card",
        "packageName":"com.android.support.car.lenspicker"}, timeout=5000,
                 **kwargs):
        ui_step.__init__(self, **kwargs)
        self.view_to_check = view_to_check
        self.timeout = timeout
    ...
    def do(self):
        if self.device_info.platform not in ["gordon_peak"]:
            ...
            # media resides at 5 position and click has to be done at the center
            media_x_coordinate = activity_bar_single_element_width * 5 - \
                               activity_bar_single_element_width / 2
...
class press_car(ui_step):
    ...
    def __init__(self, view_to_check={"resourceId":
                     #"com.android.support.car.lenspicker:id/scrollbar_thumb",
                     "com.android.support.car.lenspicker:id/stream_card",
                     "packageName":"com.android.support.car.lenspicker"},
                     timeout=5000, **kwargs):
        ui_step.__init__(self, **kwargs)
        self.view_to_check = view_to_check
    ...
    def do(self):
        if self.device_info.platform not in ["gordon_peak"]:
            ...
            car_x_coordinate = activity_bar_single_element_width * 6 - \
                              activity_bar_single_element_width / 2
...
class wait_for_view_common(ui_step):
    """ description:
            ...
            By default, <retrieve_info> is False. long_click=False to retrieve
            info about <view_to_find/second_view_to_find>
