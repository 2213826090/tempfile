...
#######################################################################
#
# @filename:    adb_steps.py
# @description: adb test steps
...
class connect_device(adb_step):
    ...
    def __init__(self, **kwargs):
        ...
        if self.adb_connection.serial:
            self.set_errorm("", "adb connection could not be established for device: {0}".format(self.adb_connection.serial))
...
class root_connect_device(adb_step):
    ...
    def __init__(self, **kwargs):
        adb_step.__init__(self, **kwargs)
        self.set_errorm("",
                        "adb root connection could not be established for device: {0}".format(self.adb_connection.serial))
...
class kill_process(adb_step):
    ...
    def check_condition(self):
        if self.with_reboot:
            ...
            self.set_errorm("", "Process {0} killed, but the device was not rebooted in {1}!".format(self.process_name, self.reboot_timeout))
            return self.reboot_timeout > time_passed
        process_info = self.adb_connection \
                                .parse_cmd_output(cmd = "ps",
                                                  grep_for = self.process_name)
        if self.process_pid:
            self.step_data = not self.process_pid in process_info
...
class reboot(adb_step, local_step):
    ...
    def check_condition(self):
        ...
        if not self.ip_enabled and len(self.command) == 0:
            ...
            if self.pin:
                ...
                wait_for_ui(serial = self.serial,
                            disable_uiautomator = self.disable_uiautomator,
                            boot_to_Android = self.boot_to_Android,
                            pin = self.pin)()
            else:
                ui_ok = wait_for_ui(serial = self.serial,
                                    disable_uiautomator = self.disable_uiautomator)()
        ...
        try:
            self.local_connection.wait_for_ping(self.ip,
...
class reboot_recovery(adb_step):
    ...
    def do(self):
        self.step_data = self.adb_connection.reboot_device(reboot_params = "recovery",
...
class wait_for_ui_processes(adb_step):
    ...
    def __init__(self,
        ...
        self.wait_step = 5
        adb_step.__init__(self, **kwargs)
        self.set_passm("UI processes on the device {0} are alive in less than {1} seconds".format(self.serial, self.timeout))
        self.set_errorm("", "UI processes on the device {0} ware not alive in less than {1} seconds".format(self.serial, self.timeout))
        ...
        while waiting < self.timeout:
            try:
                if local_utils.has_adb_serial(serial = self.serial) and\
                   ...
                   adb_utils.is_prop_set(serial = self.serial,
                                         prop = "dev.bootcomplete",
                                         value = "1") and \
                   adb_utils.is_prop_set(serial = self.serial,
                                         prop = "sys.boot_completed",
                                         value = "1") and \
                   adb_utils.is_prop_set(serial = self.serial,
                                         prop = "init.svc.bootanim",
                                         value = "stopped"):
                    break
            except Exception, e:
...
class wait_for_ui(adb_step):
    ...
    def __init__(self,
        ...
        self.set_errorm("", "UI on the device {0} was not up in less than {1} seconds".format(self.serial, self.timeout))
    ...
    def do(self):
        ...
        if self.pin and self.boot_to_Android:
            enter_pin_to_start_android_if_necesary(serial = self.serial,
                                                   pin = self.pin,
                                                   timeout = self.timeout,
    ...
    def check_condition(self):
        try:
            ui_loaded = ui_utils.is_homescreen(serial = self.serial,
                                               sim_pin_enabled = self.sim_pin_enabled)
            if self.disable_uiautomator:
                ...
                for jar in __jar_files:
                    if not adb_utils.file_exists(serial = self.serial,
                                                 file_path = os.path.join(resource_path, jar)):
                        jar_not_found = True
                if jar_not_found == False:
                    kill_process(serial = self.serial,
                                 process_name = "uiautomator")()
                for apk in __apk_files:
                    if adb_utils.is_apk_installed(serial = self.serial,
                                                  apk_path = os.path.join(resource_path, apk)):
                        uninstall_apk(serial = self.serial,
                                      apk_path = os.path.join(resource_path, apk),
                                      uninstall_time = 10)()
...
class enter_pin_to_start_android_if_necesary(adb_step):
    ...
    def check_condition(self):
        if self.step_data == "Booting to Android":
            waiting = 0
            while waiting < self.timeout:
               if adb_utils.is_prop_set(serial = self.serial,
                                        prop = "init.svc.bootanim",
                                        value = "running"):
                   return True
               waiting += self.wait_step
...
class open_users_settings(adb_step):
    ...
    def do(self):
       am_start_command(serial = self.serial,
                        component = "com.android.settings/.UsersSettings")()
...
class push_file(adb_step):
    ...
    def check_condition(self):
        ...
        self.step_data =\
            self.adb_connection.parse_cmd_output(cmd = "ls {0}".format(self.remote),
                                                 grep_for = file_name)
...
class pull_file(adb_step, local_step):
    ...
    def do(self):
        self.adb_connection.get_file(local = self.local,
                                     remote = self.remote,
                                     timeout = self.timeout)
...
class wake_up_device(adb_step):
    ...
    def do(self):
        while not adb_utils.is_power_state(serial = self.serial,
...
class delete_folder_content(adb_step):
    ...
    def do(self):
        command(serial = self.serial,
                command = "rm -rf {0}/*".format(self.folder))()
...
class create_folder(adb_step):
    ...
    def do(self):
        command(serial = self.serial,
                command = "mkdir {0}/{1}".format(self.path, self.folder))()
...
class check_folders_exist(adb_step):
    ...
    def do(self):
        ...
        for folder in self.folder_list:
            self.step_data = self.step_data and adb_utils.folder_exists(serial = self.serial,
                                                                        folder = folder)
...
class check_package_installed(adb_step):
    ...
    def do(self):
        self.step_data = adb_utils.is_package_installed(package_name = self.package,
                                                        serial = self.serial)
...
class command(adb_step):
    ...
    def __init__(self, command, stdout_grep = None, stdout_not_grep = None,
                 stderr_grep = None, stderr_not_grep = None, timeout = 20,
                 mode = "sync", ignore_error = False, **kwargs):
        self.command = command
    ...
    def do(self):
        self.step_data = self.adb_connection.run_cmd(command = self.command,
                                                     timeout = self.timeout,
                                                     mode = self.mode,
        ...
        if self.mode == "async":
            if self.stdout_grep:
                if self.stdout_grep in self.step_data.stdout.read():
                    ...
                    return False
            return True
        if self.stdout_grep or self.stdout_not_grep or self.stderr_grep \
           or self.stderr_not_grep:
...
class am_start_command(adb_step):
    ...
    def do(self):
        if self.package:
            command(command = "pm clear {0}".format(self.package),
                    serial = self.serial,
                    timeout = self.timeout)()
        if self.component:
            self.step_data = command(command = "am start -n {0}".format(self.component),
                                     serial = self.serial,
                                     timeout = self.timeout)()
...
class check_buildstamp(adb_step):
    ...
    def do(self):
        self.result = self.build_number in \
            self.adb_connection.parse_file("/etc/buildstamp",
                                           grep_for = self.build_number)
...
class sqlite_replace_query(adb_step):
    ...
    """
    def __init__(self, db, table, columns, values, where_columns, where_values,
            **kwargs):
        self.db = db
        self.table = table
    ...
    def do(self):
        query = Sqlite.generate_update_query(db = self.db,
                                             table = self.table,
    ...
    def check_condition(self):
        sqlite_select_query(serial = self.serial,
                            db = self.db,
                            table = self.table,
                            columns = self.columns,
                            where_columns = self.columns,
                            where_values = self.values,
                            values = self.values)()
...
class sqlite_select_query(adb_step):
    ...
    def do(self):
        ...
        self.set_passm(query)
        self.set_errorm("", query)
        process = self.adb_connection.run_cmd(serial = self.serial,
                                              command = query)
...
class set_prop(adb_step):
    ...
    def check_condition(self):
        ...
        return self.value == self.adb_connection.parse_cmd_output(
                                 cmd = command,
                                 grep_for = self.value)
    ...
    def do(self):
        if self.install_time:
            self.adb_connection.install_apk(apk = self.apk_path,
                                            timeout = self.install_time)
        else:
            self.adb_connection.install_apk(apk = self.apk_path)

    def check_condition(self):
        return adb_utils.is_apk_installed(serial = self.serial,
    ...
    def do(self):
        if self.uninstall_time:
            self.adb_connection.uninstall_apk(package = self.package_name,
                                              timeout = self.uninstall_time)
        else:
            self.adb_connection.uninstall_apk(package = self.package_name)

    def check_condition(self):
        return not adb_utils.is_apk_installed(serial = self.serial,
                                              apk_path = self.apk_path)
...
class take_screenshot_given_path(adb_step):
    ...
    def do(self):
        ...
        command(serial = self.serial,
                command = "screencap -p {0}".format(remote_path),
                timeout = 60)()
        self.step_data = pull_file(serial = self.serial,
...
class perform_startup_wizard_with_tap(adb_step):
    ...
    def do(self):
        ...
        command (serial = self.serial,
                 command = "input tap 50 50")()
        time.sleep(1)
        command (serial = self.serial,
                 command = "input tap {0} 50".format(width - 50))()
        time.sleep(1)
        command (serial = self.serial,
                 command = "input tap {0} {1}".format(width - 50, height - 50))()
        time.sleep(1)
        command (serial = self.serial,
                 command = "input tap 50 {0}".format(height - 50))()
...
class enable_uiautomator_service(adb_step, ui_step):
    ...
    def do(self):
        ...
        if self.apks:
            ...
            bundle_jar_path = os.path.join(jars_path, "bundle.jar")
            push_file(serial = self.serial,
                      local = bundle_jar_path,
                      remote = "/data/local/tmp")()
            uiautomator_stub_jar_path = os.path.join(jars_path, "uiautomator-stub.jar")
            push_file(serial = self.serial,
...
class do_provision(adb_step):
    ...
    def do(self):
        ...
        install_apk(serial = self.serial,
                    apk_path = self.exoPlayerDemo_path)
        install_apk(serial = self.serial,
                    apk_path = self.widevineSamplePlayer_path)
        push_file(serial = self.serial,
                  local= self.keybox_path,
                  remote = self.remote_keybox_path)()
        push_file(serial = self.serial,
                  local= self.txei_sec_tools_path,
                  remote = self.remote_txei_sec_tools_path)()
        command(serial = self.serial,
                command = self.set_txei_sec_tools_exec_cmd)()
        command(serial = self.serial,
                command = self.input_cmd)()
...
class enable_package_from_pm(adb_step, ui_step):
    ...
    def do(self):
        root_connect_device(serial = self.serial)()
        command(serial = self.serial,
                command = "pm enable {0}".format(self.package_name))()
    ...
    def check_condition(self):
        ...
        return ui_utils.is_text_visible_scroll_left(serial = self.serial,
...
class check_gvb_state(adb_step):
    ...
    def check_condition(self):
       return adb_utils.is_prop_set(serial = self.serial,
...
class change_permissions(adb_step):
    ...
    def do(self):
        command(serial = self.serial,
                command = "chmod " + str(self.perm) + " " + self.file_name)()
    ...
    def do(self):
        ...
        if "kb_provisioning" in self.device_info.provisioning_type:
            ...
            remount(serial = self.serial,
                    should_remount = True)()
            push_file(serial = self.serial,
                 local = self.wvkeyboxtool,
                 remote = self.remote_wvkeyboxtool)()
            push_file(serial = self.serial,
                 local = self.keybox,
                 remote = self.remote_keybox)()
            change_permissions(serial = self.serial,
                               file_name = self.remote_wvkeyboxtool,
                               perm = 777)()
            command(serial = self.serial,
...
class swipe(adb_step):
    ...
    def check_condition(self):
        ...
        exists = adb_utils.is_view_displayed(view_to_find=self.view_to_check,
                                    serial=self.serial,
