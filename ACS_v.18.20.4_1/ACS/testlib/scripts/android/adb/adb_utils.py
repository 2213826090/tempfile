...
class Sqlite:
    ...
    @staticmethod
    def generate_update_query(db , table, columns, values, where_columns,
            where_values):
    ...
    @staticmethod
    def generate_select_query(db, table, columns, where_columns,
            where_values):
...
def is_power_state(serial = None, state = "ON"):
    ...
    return power_state in adb_connection.parse_cmd_output(
                                    cmd = power_cmd,
                                    grep_for = power_state)
...
def get_running_activities(serial = None):
    ...
    activities = adb_connection.parse_cmd_output(cmd = "dumpsys activity",
                                                 grep_for = "Run #0").split("\n")
...
def folder_exists(folder, serial = None):
    ...
    return "No such file or directory" not in\
            adb_connection.parse_cmd_output(cmd = "ls {0}".format(folder),
                                            grep_for = folder)
...
def is_apk_installed(apk_path, serial = None):
    ...
    out = adb_connection.parse_cmd_output(cmd = command,
                                          grep_for = package_name)
...
def check_download_completed(value, name, download_path = "/storage/sdcard0/Download/", polling_time = 10, max_time = 600, serial = None):
    if serial:
        adb_connection = connection_adb(serial=serial)
    else:
        adb_connection = connection_adb()
...
def check_download_progess(value, name,
        download_path="/storage/sdcard0/Download/", polling_time=10, serial=None):
    if serial:
        adb_connection = connection_adb(serial=serial)
    else:
        adb_connection = connection_adb()
...
def is_virtual_keyboard_on(serial = None):
    ...
    if serial:
        ...
        adb_connection = connection_adb()
    return len(adb_connection.parse_cmd_output(cmd = "dumpsys input_method",
                                          grep_for = "mInputShown=true")) > 0
...
def get_battery_prop(prop = None, serial = None):
    ...
    if serial:
        ...
        adb_connection = connection_adb()
    return adb_connection.parse_cmd_output(cmd = "dumpsys battery",
                                          grep_for = prop,
                                          timeout = 5).strip()
...
def is_text_displayed(text_to_find, serial=None,  wait_time=100, exists=True):
    ...
    while current_time < wait_time:
        ...
        p = adb_connection.get_file(remote=remote_file,
                                    local=local_file)
