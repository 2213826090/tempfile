...
class AutomatorServer(object):
    ...
    __jar_files = {
        ...
        "uiautomator-stub.jar": "uiautomator-stub.jar"
    }

    __apk_files = ["app-uiautomator.apk", "app-uiautomator-test.apk"]
    __apk_packages = {"app-uiautomator.apk" : "package:com.github.uiautomator",
                      "app-uiautomator-test.apk" : "package:com.github.uiautomator.test"}
    ...
    def push(self):
        ...
        for jar, url in self.__jar_files.items():
            ...
            if not os.path.isfile(filename):
                if "uiautomator_jars_path" in os.environ:
                    filename = os.path.join(os.environ['uiautomator_jars_path'], url)
                elif "RESOURCES_FOLDER" in os.environ:
                    filename = os.path.join(os.environ['RESOURCES_FOLDER'], url)
    ...
    def install(self):
        ...
        for apk in self.__apk_files:
            filename = os.path.join(base_dir, apk)
            if not os.path.isfile(filename):
                if "uiautomator_jars_path" in os.environ:
                    filename = os.path.join(os.environ['uiautomator_jars_path'], apk)
                elif "RESOURCES_FOLDER" in os.environ:
                    filename = os.path.join(os.environ['RESOURCES_FOLDER'], apk)
            (stdout, stderr) = self.adb.cmd("shell", "pm", "list", "packages").communicate(input = None)
            already_installed = False
            installed_packages = stdout.splitlines()
            for item in installed_packages:
                if self.__apk_packages[apk] in item:
                    already_installed = True
            print "[ {0} ]: Installing apk {1} if needed".format(self.adb.default_serial, filename)
            if not already_installed:
                (stdout, stderr) = self.adb.cmd("install", "-t", filename).communicate(input = None)
            else:
                print ("[ {0} ]: Apk {1} already installed".format(self.adb.default_serial, filename))
    ...
    def jsonrpc_wrap(self, timeout):
        ...
        def _JsonRPCMethod(url, method, timeout, restart=True):
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    is_True = _method_obj(*args, **kwargs)
                    if method == "waitForExists" and not is_True:
                        raise JsonRPCError(code = ERROR_CODE_BASE - 2,
                                           message = "Wait not found for handlers")
                    return is_True
    ...
    def uiautomator_process_exists(self):
        cmd = ["shell", "ps"]
        (out, err) = self.adb.cmd(*cmd).communicate()
        return "com.github.uiautomator" in out

    def start(self, timeout = 30):
        print "[ {0} ]: Starting uiautomator".format(self.adb.default_serial)
        if self.sdk_version() < 18:
            ...
            # check if uiautomator process is running
            if not self.uiautomator_process_exists():
                # allow unknown apps to be installed
                cmd = ["shell", "settings", "get", "secure", "install_non_market_apps"]
                (unknown_apps_state, err) = self.adb.cmd(*cmd).communicate()
                unknown_apps_state = unknown_apps_state.strip()
                cmd = ["shell", "settings", "get", "global", "package_verifier_enable"]
                (package_verifier_state, err) = self.adb.cmd(*cmd).communicate()
        ...
        while tries < 5:
            tries += 1
            print "[ {0} ]: Running {1}".format(self.adb.default_serial, cmd)
            self.uiautomator_process = self.adb.cmd(*cmd)
            self.adb.forward(self.local_port, self.device_port)
            print "[ {0} ]: Process {1}".format(self.adb.default_serial, self.uiautomator_process)
            ...
            print "[ {0} ]: UIAutomator online".format(self.adb.default_serial)
            try:
                self.jsonrpc.deviceInfo()
                return
            except JsonRPCError as e:
                time.sleep(1)
                pass
        raise Exception("Could not start uiautomator process")
    ...
    def rpc_uri(self):
        if not self.local_port:
            port_not_allocated = True
            while port_not_allocated:
                port_not_allocated = True
...
class AutomatorDevice(object):
    ...
    def handlers(self):
        ...
        class Handlers(object):
            ...
            def __call__(self):
                return [x.__name__ for x in obj.server.handlers['handlers']]
