...
class Adb(connection.Connection):
    ...
    def run_cmd(self, command, mode="sync", soutfile = None, dont_split=False,
            timeout = 10, env = {}, liveprint = True, ignore_error = False,
            cmd_type = None):
        ...
        return self.run_cmd_linux(cmd, mode = mode, soutfile = soutfile,
            timeout = timeout, env = env, liveprint = liveprint,
            ignore_error = ignore_error)


    def run_cmd_linux(self, command, mode = "sync", soutfile = None,
            timeout = 10, env = {}, liveprint = True, ignore_error = False,
            with_accept = False):
    ...
    def get_file(self, remote, local, timeout = 60):
        ...
        p = self.run_cmd_linux(cmd, timeout = timeout, ignore_error = True,
            liveprint = False)
        err = p.stderr.read()
        out = p.stdout.read()
    ...
    def install_apk(self, apk, timeout = 60):
        ...
        assert "Success" in out or "ALREADY_EXISTS" in out, \
                    "Could: not install apk {0}\Stdout: {1}\nStderr: {2}".format(apk, out, err )
    ...
    def uninstall_apk(self, package, timeout = 60):
        ...
        assert "Success" in out, \
                    "Could: not uninstall package {0}\nStdout: {1}\nStderr: {2}\n".format(package, out, err)
...
cat /dev/urandom > /dev/null & cat /dev/urandom > /dev/null"
    ...
    def parse_cmd_output(self, cmd, grep_for = None, multiple_grep = None,
                            left_separator = None, right_separator = None,
        ...
        tmp_file_name = "tmp_{0}_{1}_{2}".format(
                "5037" if self.port == None else str(self.port),
                self.serial.split(":")[0],
        ...
        with open(tmp_file_name, 'r') as f:
            ...
            string = base_utils.parse_string(string, \
                        grep_for = grep_for, multiple_grep = multiple_grep, \
                        left_separator = left_separator, \
                        right_separator = right_separator, strip = strip)
        os.remove(tmp_file_name)
        return string

    def parse_logcat(self, grep_for = None, left_separator = None,
                        right_separator = None, strip = False):
        """parses logcat output"""
        cmd = "logcat -d"
        return self.parse_cmd_output(cmd, grep_for = grep_for,
                        left_separator = left_separator,
                        right_separator = right_separator, strip = strip)

    def parse_dmesg(self, grep_for = None, left_separator = None,
                        right_separator = None, strip = False):
        """parses dmesg output"""
        cmd = "dmesg"
        return self.parse_cmd_output(cmd, grep_for = grep_for,
    ...
    def parse_file(self, file_name, grep_for = None, left_separator = None,
                        right_separator = None, strip = False):
