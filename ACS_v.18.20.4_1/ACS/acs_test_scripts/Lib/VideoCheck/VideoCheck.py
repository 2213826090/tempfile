...
:summary: Methods for video check
:since: 03/02/2014
:author: mmorchex
"""
...
def check_video_corrupt(video_uri):
    """
...
def check_video_brightness(video_uri):
    """
...
def check_video_resolution(video_uri):
    """
...
def compare_videos_size(first_video_uri, second_video_uri):
    """
...
def get_percent_diff(frame1, frame2, percent_threshold=0.10):
    """
...
def match_template_on_video_frame(frame1, frame2, threshold=0.99):
    """
...
def extract_frame_of_video(video_uri, box_crop_top, box_crop_bottom, box_crop_left, box_crop_right, size=(128, 128),
                           crop=False):
    """
...
def get_frame_time(num_research_frame, video_frame, video_time):
    """
...
def compare_two_video_frame_by_frame(video1, video2, video1_time, max_consecutive_missing_frame=5, crop=False,
    ...
    video_frame = extract_frame_of_video(video1, crop=crop, box_crop_top=box_crop_top, box_crop_bottom=box_crop_bottom,
                                         box_crop_left=box_crop_left, box_crop_right=box_crop_right)

    #Extract frames from second video
    video_frame2 = extract_frame_of_video(video2, crop=crop, box_crop_top=box_crop_top, box_crop_bottom=box_crop_bottom,
                                          box_crop_left=box_crop_left, box_crop_right=box_crop_right)

    if video_frame == [] or video_frame2 == []:
        raise AcsConfigException(AcsConfigException.READ_PARAMETER_ERROR, "One of video is missed.")
    ...
    for i in range(len(video_frame)):
        ...
        if missing_consecutive_frame_counter == max_consecutive_missing_frame:
            ...
            msg = "Record and reference file are not identical, doesn't match " + str(max_consecutive_missing_frame) + \
                  " consecutive frame at " + str(get_frame_time(i, video_frame, video1_time)) + " s."
            break
        for iter in range(num_frame, len(video_frame2)):
            #Use first comparison algorithm match_template
            match, diff_size_match_template = match_template_on_video_frame(video_frame[i], video_frame2[iter],
                                                                            match_template_threshold)
            if match:
                #Use second algorithm percent diff
                diff_per, match_per, diff_mode, diff_size = get_percent_diff(video_frame[i], video_frame2[iter],
                ...
                if match_per:
                    ...
                    if ((lag_frame > 0) and (i > 5)):
                        ...
                        warning_msg += str(lag_frame) + " frame skipped at " + \
                                       str(get_frame_time(i, video_frame, video1_time)) + " s, "
