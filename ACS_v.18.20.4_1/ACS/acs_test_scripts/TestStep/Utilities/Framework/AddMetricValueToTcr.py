...
class AddMetricValueToTcr(TestStepBase):
    """
    ...
    def run(self, context):
        ...
        metrics = context.get_info(self._pars.metrics_node)
        node_to_add = {"name": self._pars.name, "value": self._pars.data, "unit": self._pars.unit}

        if metrics is not None:
            metrics["metrics"].append(node_to_add)
        else:
            metrics = {"metrics": [node_to_add]}

        LiveReporting.instance().update_running_tc_info(test_info=metrics,
                                                        iteration=iteration)
        context.set_info(self._pars.metrics_node, metrics)
