...
class StopTcrTcIteration(TestStepBase):
    """
    ...
    def run(self, context):
        ...
        if self._tc_parameters.get_b2b_iteration() > 1:
            self._logger.debug("Sending stop iteration info to TCR")
            LiveReporting.instance().send_stop_tc_info(verdict=self._pars.verdict,
                                                       execution_nb=1,
                                                       success_counter=1,
                                                       max_attempt=1,
                                                       acceptance_nb=1,
