...
:summary: This file implements a Test Step to read any KeyCode events
:since:21/09/2015
:author: mmaraci
"""
...
class ReadKeyCodeEvents(DeviceTestStepBase):
    """
    Reads KeyCode events
    """
    ...
    def run(self, context):
        ...
        self._logger.info("Waiting for the following KEYCODE events: %s for %d seconds" % (str(events), self._pars.timeout))

        if not self._api.check_hid_events_generic(self._pars.input_device, events,
                                                  self._device.retrieve_serial_number(), self._pars.timeout):
    ...
    def _raise_error_if_hid_device_is_invalid(self):
        ...
        """
        if not self._pars.input_device:
            self._raise_config_exception("INPUT_DEVICE must be valid")
