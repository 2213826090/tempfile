...
:summary: This file implements a Test Step for a simple ping
:since 26/03/2014
:author: floeselx
"""
...
class Ping(DataXferBase):
    ...
    def run(self, context):
        ...
        blocking = bool(self._pars.blocking)

        # Optional parameters
        source_ip = str(self._pars.source_ip)
        if "none" in source_ip.lower():
            source_ip = None

        # Where the information will be stored into the context
        save_as = str(self._pars.save_as)

        self._logger.info("Ping the IP address %s ..." % destination_ip)

        # Check if IPV4 (mean 3 dots)
        # url contains 3 dots max (ex www.google.com, mysite.com, www.other.com/test.html)
        if destination_ip.count('.') <= 3:
            packet_loss = self._networking_api.ping(server_ip_address=destination_ip,
            ...
            packet_loss = self._networking_api.ping6(ip_address=destination_ip,
                                                     packet_size=packet_size,
                                                     packet_count=packet_count,
                                                     flood_mode=flood_mode,
                                                     blocking=blocking,
