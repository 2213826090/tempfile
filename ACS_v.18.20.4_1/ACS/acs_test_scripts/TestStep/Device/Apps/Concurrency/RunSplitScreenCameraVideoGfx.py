"""
@summary: This file implements a Test Step to run VideoCapture_VideoPlayback_GFX.apk on the device.  This
    application was created by the DSV FV (functional validation) team, and can be
    found in Artifactory under acs_test_artifacts/CONCURRENCY/TESTS/camera_video_gfx_conc.
...
class RunSplitScreenCameraVideoGfx(DeviceTestStepBase):
    ...
    def run(self, context):
        ...
        self._logger.info("CameraVideoGfx: Run")
        ...
        #- videocapture_videoplayback_gfx application
        #- OpenGLRender
        #- OMX Codec
        ...
        conc_start_messages = self._device.get_app_start_messages(package_name="com.example.videocapture_videoplayback_gfx", app_name="JB_Video_StillImage_GFxActivity_v2")
        # [12/17/2014 JONG] : Commented out out dated start messages (WW50 Android releases)
        #conc_start_messages = [
        #    self._device.get_app_start_messages(package_name="com.example.videocapture_videoplayback_gfx", app_name="JB_Video_StillImage_GFxActivity_v2"),
        ...
        self.app_api.launch_app(intent=pkg_activity, extras="-e vpath {0} -e time {1} -e gfxactive {2}".format(video_file, duration, enable_gfx))
        ...
        self._logger.info("CameraVideoGfx: Waiting %d minutes before starting verification"%duration)
        time.sleep(duration * 60)

        self._logger.info("CameraVideoGfx: Done")
