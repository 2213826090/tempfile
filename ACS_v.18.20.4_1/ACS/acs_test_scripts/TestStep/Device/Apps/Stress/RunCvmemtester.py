...
a memory stress test from the CV team that's called "memtester".
Prerequisite: the executable called "memtester" must be in directory specified by SCRIPTS_PATH.
...
class RunCvmemtester(DeviceTestStepBase):
    def run(self, context):
        ...
        self._logger.info("RunCvmemtester: Run")
        ...
        #If anyone enables this in Windows, I assume they will create a binary with the .exe extension.
        #Add to this list if required for any other OS.
        possible_extensions = ["", ".exe"]
        for ext in possible_extensions:
            ...
            if found:
                break
        if not found:
            msg = "RunCvmemtester: Could not find a " + basename + ".* script."
            self._logger.error(msg)
            raise DeviceException(DeviceException.OPERATION_FAILED, msg)

        args = "-t {0} -s {1}".format(self._pars.duration, self._pars.mem_size)

        """run_shell_executable doesn't return anything, but will raise an exception if it fails"""
        self.system_api.run_shell_executable(script, args, io_redirection=2, timeout=self._pars.duration*60 + 300)

        self._logger.info("RunCvmemtester: Done")
