"""
@summary: Verify S state residency from the result which was created by SOCwatch.
PREREQUISITES:
...
class VerifySOCWatchSstate(DeviceTestStepBase):
    def run(self, context):
        ...
        states_title, states_info = self.residency_api.parse_socwatch_sstates_info(result_file_fullpath)
        ...
        for index, title in enumerate(states_title):
            ...
            if ret_val:
                self._logger.info("Success: Reported {0} residency is {1}%. It is {2} {3} ".format(title, states_info[index], converted_target_criteria[index], target_value[index]))
            else:
                msg = "Reported {0} residency is {1}%. Different from expectation, it is not {2} {3}".format(title, states_info[index], converted_target_criteria[index], target_value[index])
                self._logger.error(msg)
                raise DeviceException(DeviceException.OPERATION_FAILED, msg)
