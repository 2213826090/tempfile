...
:summary: This file implements a Test Step to power Cellular
:since 30/10/2014
:author: emarchan
"""
...
class CellularSetPower(CellularBase):
    """
    ...
    def _wait_until_state_is_reached(self, expected):
        ...
        if self._time_out_reached() and not self._state_reached:
            self._raise_device_exception("Set CELLULAR %s failure" % expected)
    ...
    def _is_state_reached(self, expected):
        ...
        state = str_to_bool_ex(self._modem_api.get_modem_power_status())
        if state:
