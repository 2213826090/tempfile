...
class GetWifiDirectDevAddressFromName(WifiDirectBase):
    """
    ...
    def run(self, context):
        ...
        wifi_direct_name = self._pars.wifi_direct_name
        assert wifi_direct_name != "", "Value (%s) is invalid for wifi_direct_name" % wifi_direct_name

        value = self._api.get_wifi_direct_dev_address_from_name(wifi_direct_name)

        if is_valid_mac_address(value) is not True:
            self._raise_device_exception("Invalid value detected for %s => %s" % (wifi_direct_name, value))
