...
:summary: This script implements the TestStep in which we check that when a GATT Descriptor is being read, the
...
class BleGattServerReadGattDescriptorCallback(BleBase):
    """
    ...
    def run(self, context):
        ...
        if (advertise_mode is not False) and (tx_power_level is not False):
            output = self._api.ble_gatt_server_read_descriptor_callback(tx_power_level, advertise_mode,
