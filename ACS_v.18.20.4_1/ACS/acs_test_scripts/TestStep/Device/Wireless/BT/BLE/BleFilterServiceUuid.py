...
:summary: This script implements the TestStep filtering a BLE Advertisement by the field Service Uuid(s)
:since: 7/30/15
:author: mmaraci
"""
...
class BleFilterServiceUuid(BleBase):
    """
    ...
    def run(self, context):
        ...
        if scan_mode is not False:
            bluetooth_device = self._api.ble_scan_filter_service_uuid(self._pars.service_uuid, scan_mode,
                                                                      self._pars.timeout)
        else:
        ...
        self._logger.debug("Found device: " + str(bluetooth_device.name) + " address: " + str(bluetooth_device.address))