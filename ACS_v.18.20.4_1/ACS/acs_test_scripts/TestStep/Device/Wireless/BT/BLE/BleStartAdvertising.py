...
:summary: This script implements the TestStep for starting a BLE Advertisement on an enabled Android
        device
:since: 7/30/15
:author: mmaraci
"""
...
class BleStartAdvertising(BleBase):
    """
    This script implements the TestStep for starting a BLE Advertisement on an enabled Android device
    """
    ...
    def run(self, context):
        ...
        if (advertise_mode is not False) and (tx_power_level is not False):
            self._api.ble_start_advertising(tx_power_level, advertise_mode, advertise_code, self._pars.is_connectable,
