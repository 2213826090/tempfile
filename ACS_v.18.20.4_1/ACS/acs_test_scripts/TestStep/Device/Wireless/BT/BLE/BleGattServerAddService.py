...
:summary: This script implements the TestStep for adding a service to our GATT Server
:since: 8/28/15:author: mmaraci
"""
...
class BleGattServerAddService(BleBase):
    """
    ...
    def run(self, context):
        """
        If the service cannot be added, the the operation has failed
        :param context:
        :return:
        """
        ...
        if (advertise_mode is not False) and (tx_power_level is not False):
            output = self._api.ble_gatt_server_add_service(tx_power_level, advertise_mode,
