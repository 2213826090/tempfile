...
:summary: This script implements the TestStep for adding a characteristic to our GATT Server
:since: 8/28/15
:author: mmaraci
"""
...
class BleGattServerInteractNoAdvertising(BleBase):
    """
    ...
    def run(self, context):
        ...
        gatt_server_operation = BleBase.check_gatt_server_operation(self, self._pars.operation)

        BleBase.run(self, context)

        if gatt_server_operation is not False:
            output = self._api.ble_gatt_server_interaction_no_advertising(gatt_server_operation)
