...
:summary: This script implements the TestStep in which we check that when a GATT Characteristic is being read, the
...
class BleGattServerReadGattCharacteristicCallback(BleBase):
    """
    ...
    def run(self, context):
        ...
        if (advertise_mode is not False) and (tx_power_level is not False):
            output = self._api.ble_gatt_server_read_characteristic_callback(tx_power_level, advertise_mode,
