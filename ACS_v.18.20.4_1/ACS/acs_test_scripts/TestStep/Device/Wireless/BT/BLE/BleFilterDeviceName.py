...
:summary: This script implements the TestStep filtering a BLE Advertisement by the emitting device's Bluetooth name
:since: 7/30/15
:author: mmaraci
"""
...
class BleFilterDeviceName(BleBase):
    """
    ...
    def run(self, context):
        ...
        if scan_mode is not False:
            bluetooth_device = self._api.ble_scan_filter_name(self._pars.remote_name, scan_mode, self._pars.timeout)
        else:
        ...
        if not bluetooth_device:
            msg = "No device was found using the name filter: " + str(self._pars.remote_name)
            self._logger.error(msg)
            raise DeviceException(DeviceException.OPERATION_FAILED, msg)

        self._logger.debug("Found device: " + str(bluetooth_device.name) + " " + str(bluetooth_device.address))