...
:summary: This file implements a Test Step to power bluetooth
:since:15/03/2013
:author: fbongiax
"""
...
class BtSetPower(BtBase):
    ...
    def run(self, context):
        ...
        for current in power_sequence:
            ...
            if (current == TestConst.STR_ON) and (state_bt != str(BT_STATE.STATE_ON)):
                self._raise_device_exception("set BT ON failure")

            if (current == TestConst.STR_OFF) and (state_bt != str(BT_STATE.STATE_OFF)):
