...
:summary: This file implements a Test Step to get WiFi channel
:since 12/12/2014
:author: jfranchx
"""
...
class WifiGetChannel(WifiBase):
    """
    Implements the get channel test step for Wifi
    """
    ...
    def run(self, context):
        ...
        self._logger.info("Getting the WiFi Channel...")

        # Where the information will be stored into the context?
        variable_to_set = self._pars.save_wifi_channel

        # WiFi must be On to use this function
        if self._api.get_wifi_power_status() == 0:
            msg = "Can't get WiFi channel, WiFi is currently OFF"
            self._logger.error(msg)
            raise DeviceException(DeviceException.OPERATION_FAILED, msg)

        current_bandwidth = self._api.get_wifi_channel(self._pars.interface)
