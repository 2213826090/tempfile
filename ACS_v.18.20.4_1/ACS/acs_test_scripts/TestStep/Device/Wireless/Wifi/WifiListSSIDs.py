...
:summary: This file implements a Test Step for Wifi listing SSID
:since 12/06/2014
:author: floeselx
"""
...
class WifiListSSIDs(WifiBase):
    """
    Implements the SSIDs gathering test step for Wifi
    """
    ...
    def run(self, context):
        ...
        self._logger.info("Gather SSIDs list from the Wifi network...")

        # Where the information will be stored into the context?
        variable_to_set = self._pars.save_as

        ssids = self._api.list_ssids()
        if ssids:
            msg = "%d SSIDs found" % len(ssids)
            self._logger.debug(msg)
        else:
            msg = "No SSIDs found after scan !"
