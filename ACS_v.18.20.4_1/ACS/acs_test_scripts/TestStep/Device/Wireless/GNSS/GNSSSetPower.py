...
@summary: This file implements a Test Step to power GPS
@since 04/04/2014
@author: obouzain
"""

from UtilitiesFWK.Utilities import split_and_strip, TestConst, str_to_bool_ex
from acs_test_scripts.TestStep.Device.Wireless.GNSS.GNSSBase import GNSSBase
import time

class GNSSSetPower(GNSSBase):
    """
    Implements the Power test step for GPS
    """
    ...
    def __init__(self, tc_conf, global_conf, ts_conf, factory):
        GNSSBase.__init__(self, tc_conf, global_conf, ts_conf, factory)
    ...
    def run(self, context):
        ...
        for current in power_sequence:
            ...
            self._logger.info("Powering %s the GPS interface" % current)

            self._api.set_gps_power(current)
    ...
    def _wait_until_state_is_reached(self, expected):
        ...
        if self._time_out_reached() and not self._state_reached:
            self._raise_device_exception("Set GPS %s failure" % expected)
    ...
    def _is_state_reached(self, expected):
        ...
        state = str_to_bool_ex(self._api.get_gps_power_status())
        if state:
