...
:organization: CWS
:summary: Disconnect wired headset implementation Test Step
:since: 02/04/2015
:author: mcarriex
"""
...
class DisconnectHeadset(EquipmentTestStepBase):
    """
    ...
    def run(self, context):
        ...
        if "WiredHeadset" in self._global_conf.benchConfig.get_parameters_name():
            ...
            io_card.disable_line(ground_line)
            io_card.disable_line(left_line)
            io_card.disable_line(right_line)
            io_card.disable_line(jack_det_n)
        else:
