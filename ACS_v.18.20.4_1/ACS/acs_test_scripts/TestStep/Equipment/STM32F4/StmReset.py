...
:summary: This step will reset a given STM board equipment
:since: 3/10/16
:author: mmaraci
"""
...
class StmReset(EquipmentTestStepBase):
    """
    Class responsible for calling the reset method of the STM32F4xx board equipment
    """

    def run(self, context):
        EquipmentTestStepBase.run(self, context)
        self._api = self._equipment_manager.get_stm32f4(self._pars.eqt)
        output, err_output, return_code = self._api.reset_board(self._pars.binary_path)
        if return_code != 0:
