...
:summary: This file implements a Test Step for getting a network simulator parameter
:author: jduran4x
:since 12/09/2014
...
class NsGetParam(EquipmentTestStepBase):
    def run(self, context):
        ...
        command = command + '?'
        value = net_sim.get_interface().query(command)
        self.ts_verdict_msg = "VERDICT: %s stored as {0}".format(value) % self._pars.value
        context.set_info(self._pars.value, value)
