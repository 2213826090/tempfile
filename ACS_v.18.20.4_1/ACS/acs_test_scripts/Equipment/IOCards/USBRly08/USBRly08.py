...
import acs_test_scripts.Equipment.IOCards.USBRly08.Wrapper.WUSBRly08 as W
from ErrorHandling.TestEquipmentException import TestEquipmentException
from acs_test_scripts.Equipment.IOCards.USBRly08.USBRly08Rpc import USBRlyRpcServer, USBRlyRpcThread
import xmlrpclib
import time
import socket
...
class USBRly08(EquipmentBase, IIOCard):
    ...
    def usb_connector(self, plug):
        ...
        """
        if self.__selected_usb_device == USBRly08.USB_HOST_PC:
    ...
    def usb_device_selector(self, usb_device):
        ...
        if usb_device in USBRly08.SUPPORTED_DEVICE_TYPE:
    ...
    def simulate_insertion(self, device_type):
        ...
        if device_type in USBRly08.SUPPORTED_DEVICE_TYPE:
            relay_mapping = {self.USB_HOST_PC: self.__usb_host_pc_connect_relay,
    ...
    def remove_cable(self, cable_type):
        ...
        """
        if cable_type not in USBRly08.SUPPORTED_DEVICE_TYPE:
    ...
    def lamp(self, on):
        ...
        handle lamp
        :type on: boolean
        :param on: action to be done:
            - True  => Turn on light
            - False => Turn off light
        """
        if self.__lamp_relay is None:
            self.get_logger().warning("Lamp relay not configured. No action taken!")
            return False
        if on:
            self.get_logger().info("Turn on light")
            self.enable_line(self.__lamp_relay)
        else:
            self.get_logger().info("Turn off light")
            self.disable_line(self.__lamp_relay)
        return True
    ...
    def press_key_combo(self, button_list, press_duration, push_delay=RELAY_PUSH_DELAY, release_delay=RELAY_RELEASE_DELAY):
        """
