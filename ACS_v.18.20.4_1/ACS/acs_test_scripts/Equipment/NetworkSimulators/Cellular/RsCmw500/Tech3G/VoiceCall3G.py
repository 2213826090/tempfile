...
:summary: voice call 3G implementation for RS CMUW500 cellular network simulator
:since: 09/09/2014
:author: fbelvezx
"""
...
class VoiceCall3G(IVoiceCall3G, VoiceCall, AudioCommon):
    """
    ...
    def mt_originate_call(self):
        ...
        self._visa.send_command("CALL:WCDMa:SIGN:CSWitched:ACTion CONNect")
    ...
    def voice_call_network_release(self):
        ...
        self._visa.send_command("CALL:WCDMa:SIGN:CSWitched:ACTion DISConnect")
    ...
    def check_call_state(self, state, timeout=0, blocking=True):
        ...
        timer = timeout
        current_state = self._visa.query_command("FETCh:WCDMa:SIGN:CSWitched:STATe?")
    ...
    def set_audio_codec(self, codec):
        ...
        if "NB" in codec:
            self._visa.send_command("CONFigure:WCDMa:SIGN:CONNection:VOICe:CODec NB")
            self._visa.send_command("CONFigure:WCDMa:SIGN:CONNection:VOICe:AMR:NARRow %s" % codec_id)
        elif "WB" in codec:
            self._visa.send_command("CONFigure:WCDMa:SIGN:CONNection:VOICe:CODec WB")
            self._visa.send_command("CONFigure:WCDMa:SIGN:CONNection:VOICe:AMR:WIDE %s" % codec_id)
    ...
    def set_speech_configuration(self, audio_source):
        ...
        self.set_audio_scenario("WCDMA Sig1")

        self.get_logger().info("Set speech source to %s" % audio_source)
        self._visa.send_command("CONFigure:WCDMa:SIGN:CONNection:VOICe:SOURce %s" % audio_source)
