...
:summary: cell 3G implementation for Agilent 8960
:since: 01/04/2011
:author: ymorel
"""
import random
...
from acs_test_scripts.Equipment.NetworkSimulators.Cellular.Agilent8960.Wrapper.Tech3G import WCell3G as W
from acs_test_scripts.Equipment.NetworkSimulators.Cellular.Agilent8960.Wrapper import WBase
...
class Cell3G(ICell3G):
    ...
    def set_random_lac(self, lac_list=None):
        ...
        if not isinstance(lac_list, list):
            # Set to default value if not a list
            lac_list = []
    ...
    def set_ps_data_ergch_information_state(self, state):
        ...
        """
        (err, msg) = WBase.SendCommand(self.get_root(),
                                       "CALL:HSUP:SERV:PSD:ERGC:INF %s" % state)
        self._error_check(err, msg)
    ...
    def set_drx_cycle_utran(self, drx_cycle_length):
        ...
        """
        (err, msg) = WBase.SendCommand(self.get_root(),
                                       "CALL:DRX:CLENgth:UTRan %s" % drx_cycle_length)
        self._error_check(err, msg)
    ...
    def set_ctch_allocation_period(self, allocation_period):
        ...
        """
        (err, msg) = WBase.SendCommand(self.get_root(),
                                       "CALL:CTCHannel:APERiod %d" % allocation_period)
        self._error_check(err, msg)
    ...
    def set_rac(self, rac):
        ...
        self._root.send_command("CALL:CELL:RACode %i" % rac)
    ...
    def get_rac(self):
        ...
        """
        rac = int(self._root.query_command("CALL:CELL:RACode?"))
        return rac
    ...
    def set_page_response(self, pr):
        ...
        self._root.send_command("CALL:DCH:UPR %s" % pr)
    ...
    def get_mcc(self):
        ...
        """
        mcc = int(self._root.query_command("CALL:CELL:MCCode?"))
        return mcc
    ...
    def get_mnc(self):
        ...
        """
        mnc = int(self._root.query_command("CALL:CELL:MNCode?"))
        return mnc
    ...
    def set_red_ie_inclusion_state(self, red):
        ...
        self._root.send_command("CALL:HAND:RRC:CREL:RED %s" % red)
    ...
    def set_eutra_earfcn(self, eutra_earfcn):
        ...
        self._root.send_command("CALL:HAND:RRC:CREL:RED:EUTR:EARF %s" % eutra_earfcn)
    ...
    def set_external_epc_connection(self, ns_lte_ip_lan1, ns_lte_dl_earfcn):
        ...
        self._root.connect_to_external_epc(ns_lte_ip_lan1)

        # Verify ethernet connection before timeout
        # 5 seconds using the PXT active IP address
        self._root.check_external_epc_connection(ns_lte_ip_lan1, 5)
    ...
    def set_equivalent_plmn_list_points(self, mcc, mnc):
        ...
        self._root.send_command("CALL:PLMNetwork %d, %d, 0" % (mcc, mnc))
    ...
    def get_equivalent_plmn_list_points(self):
        ...
        output = self._root.send_command("CALL:PLMNetwork:POINts?")
    ...
    def set_rrc_state_transition(self, state):
        ...
        self._root.send_command("CALL:SERVice:PSData:RRC:TRANsition:%s" % state)
    ...
    def set_geran_neighbor_cells(self,
        ...
        self._root.send_command("CALL:BCCHannel:GSMSystem:STATe %s" % cell_list_str)
        self._root.send_command("CALL:BCCHannel:GSMSystem:NCC %s" % ncc_list_str)
        self._root.send_command("CALL:BCCHannel:GSMSystem:BCC %s" % bcc_list_str)
        self._root.send_command("CALL:BCCHannel:GSMSystem:BAND %s" % band_list_str)
        self._root.send_command("CALL:BCCHannel:GSMSystem:BCHannel %s" % arfcn_list_str)
    ...
    def set_compress_mode_gap_config(self,
        ...
        """
        self.get_logger().info("Set gap %d compress mode parameters to purpose %s ,slot number %d ,pattern length %d ,gap length 1&2 %d %d,\
                                relative CFN %d, pattern repetition count %d, delta SIR1 %d, delta SIR1 after %d" % (gap_number,
                                                                                                                     tgmpurpose,
                                                                                                                     tgsnumber,
                                                                                                                     tgplength,
                                                                                                                     tglength,
                                                                                                                     ...
                                                                                                                     dsir1,
                                                                                                                     dsir1_after))
        self._root.send_command("CALL:COMP:TGPS%d:STATE ON" % gap_number)
        self._root.send_command("CALL:COMP:TGPS%d:TGMP %s" % (gap_number,
                                                                tgmpurpose))
        self._root.send_command("CALL:COMP:TGPS%d:TGSN %d" % (gap_number,
                                                                tgsnumber))
        self._root.send_command("CALL:COMP:TGPS%d:TGPL %d" % (gap_number,
                                                                tgplength))
        self._root.send_command("CALL:COMP:TGPS%d:TGL %d" % (gap_number,
                                                                tglength))
        self._root.send_command("CALL:COMP:TGPS%d:TGL2 %d" % (gap_number,
                                                                tglength2))
        self._root.send_command("CALL:COMP:TGPS%d:TGCF:REL %d" % (gap_number,
                                                                tgcfn))
        self._root.send_command("CALL:COMP:TGPS%d:TGPRC %d" % (gap_number,
                                                                tgprc))
        self._root.send_command("CALL:COMP:TGPS%d:DSIR1 %d" % (gap_number,
                                                                dsir1))
        self._root.send_command("CALL:COMP:TGPS%d:DSIR1:AFT %d" % (gap_number,
                                                                dsir1_after))
        self._root.send_command("CALL:COMPressed:TGPSequence%d:TGDistance %d" % (gap_number,
                                                                tgd))
        self._root.send_command("CALL:COMPressed:TGPSequence:DFSType ATYPe")
    ...
    def set_compress_mode_channelization_codes_preset_for_384k_dl_dpch(self):
        ...
        see wireless.agilent.com/rfcomms/refdocs/wcdma/wcdma_gen_bse_dl_codes_lvls.html table Compressed Mode with 384k DL DPCH(Non-HSDPA)

        """
        self.get_logger().info("Set DL Channel code preset configuration for compressed mode when 384k DL DPCH is active (non HSDPA)")
        # Set S-CCPCH channelization code
        self._root.send_command("CALL:CCPChannel:SECondary:CCODe:CODE 7")
        # Set PICH channelization code
        self._root.send_command("CALL:PICHannel:CCODe:CODE 16")
        # Set AICH channelization code
        self._root.send_command("CALL:AICHannel:CCODe:CODE 10")
        # Sets the WCDMA channel code OVSF256
        self._root.send_command("CALL:DPCHannel:KSPS15:CODE 12")
        # Sets the WCDMA channel code OVSF128
        self._root.send_command("CALL:DPCHannel:KSPS30:CODE 9")
        # Sets the WCDMA channel code OVSF8
        self._root.send_command("CALL:DPCHannel:KSPS480:CODE 6")
        # Sets the WCDMA channel code OVSF32
        self._root.send_command("CALL:DPCHannel:KSPS120:CODE 25")
        # Sets the Channelization codes for WCDMA orthogonal channel noise simulator (OCNS) 1 to 16.
        self._root.send_command("CALL:OCNSource:CCODe:CODE 2,11,17,23,31,38,47,55,62,69,78,85,94,92,88,90")
    ...
    def set_compress_mode_state(self, state):
        ...
        self._root.send_command("CALL:COMP:ENABLE %d" % state)
    ...
    def get_gsm_neighbor_cells_rssi_measurements(self):
        ...
        arfcn = self._root.query_command("CALL:MS:REPorted:GSMSystem:BCHannel?").split(",")
        ...
        rssi = self._root.query_command("CALL:MS:REPorted:GSMSystem:RSSI?").split(",")
        # return of this GPIB command is the following format:
    ...
    def set_compress_mode_measurement_config(self,
        ...
        self._root.send_command("CALL:COMPressed:MEASurement:STATe ON")
        self._root.send_command("CALL:COMPressed:MEASurement:CONFig %s" % meas_type)
    ...
    def set_rau_fop_control(self, fop_control, fop_accept_mode):
        ...
        self._root.send_command("CALL:SERVice:GPRS:RAUAccept:FOProceed:CONTROL:AUTO %d" % fop_control)
        self._root.send_command("CALL:SERVice:GPRS:RAUAccept:FOProceed:MANual %d" % fop_accept_mode)
    ...
    def set_gsm_cell_reselection_min_rx_level(self, minrxlevel):
        ...
        self._root.send_command("CALL:BCCHannel:GSMSystem:CRESelection:RLMinimum %s" % min_rx_level_list_str)

    def set_barred_state(self,
                                barred):
        """
        Sets cell as barred or not barred.
        :type barred: int or string
        :param barred: int (0,1) or ('ON','OFF')
        """
        self._root.send_command("CALL:CELL:BCCHannel:UPDAtepage AUTO")
        time.sleep(5)

        if barred in(1,"ON"):
            self.get_logger().info("Set the cell as barred")
            barred = 1
        elif barred in(0,"OFF"):
            self.get_logger().info("Set the cell as NOT barred")
            barred = 0
        else:
            msg = "barred should  be int (0,1) or ('ON','OFF')! received: %s" %str(barred)
            self.get_logger().error(msg)
            raise TestEquipmentException(TestEquipmentException.INVALID_PARAMETER, msg)

        self._root.send_command("CALL:CELL:AREStriction:BARRed:STATe %d" %barred)

    def get_barred_state(self):
        """
        Gets cell barred state.
        :rtype: str
        :return: the bar state. Possible returned values:
            0, 1
        """
        self.get_logger().info("GET the BARRED status")
        barred_status = self._root.query_command("CALL:CELL:AREStriction:BARRed:STATe?")
        self.get_logger().info("BARRED STATUS = " + str(barred_status))
        return barred_status
