...
:summary: data 3G implementation of Agilent 8960 cellular network simulator
:since: 22/09/2014
:author: jduran4x
"""
...
class Data3G(IData3G, VisaObject):
    ...
    def clear_all_dut_ip_addresses(self):
        ...
        #     self.__error_check(err, msg)
        #     self.get_logger().info("the network simulator ip_addresses are temporary cleared,
    ...
    def set_gprs_radio_access_bearer(self, ul_rab, dl_rab):
        ...
        if ul_rab == "64k" and dl_rab == "64k":
            rab_str = "GPRSRAB1"
        elif ul_rab == "64k" and dl_rab == "384k":
            rab_str = "GPRSRAB3"
        elif ul_rab == "128k" and dl_rab == "384k":
            rab_str = "GPRSRAB4"
        elif ul_rab == "384k" and dl_rab == "384k":
            rab_str = "GPRSRAB5"
        elif ul_rab == "64k" and dl_rab == "HSDPA":
            rab_str = "PSDHsdpa64"
        elif ul_rab == "128k" and dl_rab == "HSDPA":
            rab_str = "PSDHsdpa128"
        elif ul_rab == "384k" and dl_rab == "HSDPA":
            rab_str = "PSDHsdpa384"
        elif ul_rab == "HSUPA" and dl_rab == "HSDPA":
            rab_str = "PHSPa"
        else:
            raise TestEquipmentException(TestEquipmentException.INVALID_PARAMETER,
                                         "Wrong RAB configuration: ul %s dl %s" % (ul_rab, dl_rab))

        self.get_logger().info("Set Cell RAB: ul %s dl %s" % (ul_rab, dl_rab))
        self._visa.send_command("CALL:SERV:GPRS:RAB %s", rab_str)
    ...
    def check_data_connection_state(self, state, timeout=0, blocking=True, cell_id=None):
        ...
        #  % (current_state, int(timeout) - int(timer)))
        #     return True
        raise TestEquipmentException(TestEquipmentException.FEATURE_NOT_AVAILABLE, "Not implemented!")
    ...
    def set_edch_cell_capability(self, state):
        ...
        if state == "ON":
            ...
            self._visa.send_command("CALL:EDCH:CIND CAP")
        elif state == "OFF":
            self.get_logger().info("Set E-DCH Cell capability Off")
            self._visa.send_command("CALL:EDCH:CIND CNIN")
        else:
            # Failed to reach desired state
    ...
    def set_hsdpa_cell_capability(self, state):
        ...
        if state == "ON":
            ...
            self._visa.send_command("CALL:HSDP:CIND CAP")
        elif state == "OFF":
            self.get_logger().info("Set HSDPA Cell capability Off")
            self._visa.send_command("CALL:HSDP:CIND CNIN")
        else:
            # Failed to reach desired state
    ...
    def set_rrc_idle_with_scri(self, state):
        ...
        self.get_logger().info("Set RRC IDLE with SCRI to "
                               "%s" % (state.lower()))
        self._visa.send_command("CALL:SERVice:PSData:RRC:TRANsition:SCRI:RRCIdle %s" % (state.lower()))
    ...
    def set_sccpch_prach(self, state):
        ...
        """
        gpib_parameters = {"signaling": "SIGN", "straffic": "STR"}
        state = state.lower()
        if state not in gpib_parameters:
            raise TestEquipmentException(TestEquipmentException.INVALID_PARAMETER,
                                         "The SCCPCH_PRACH configuration"
                                         "can only be set to signaling or"
                                         " straffic")

        self.get_logger().info("Set SCCPCH PRACH to %s" % state)
        self._visa.send_command("CALL:CCPChannel:SECondary:CONFigure %s" % gpib_parameters[state])
    ...
    def set_auto_rrc_state_transition_control(self, state):
        ...
        self.get_logger().info("Set automatic RRC state transition to %s" % state)
        self._visa.send_command("CALL:SERVice:PSData:RRC:TRANsition:AUTO %s" % gpib_parameters.index(state))
    ...
    def set_fast_dormancy_state(self, state):
        ...
        self.get_logger().info("Set fast dormancy state to %s" % (state.lower()))
        self._visa.send_command("CALL:SERVice:PSData:RRC:TRANsition:SCRI:FDORmancy %s" % gpib_parameters.index(state))
    ...
    def set_fast_dormancy_destination(self, state):
        ...
        """
        state = state.lower()
        if state not in ("pch", "upch", "fach", "idle"):
            raise TestEquipmentException(TestEquipmentException.INVALID_PARAMETER,
                                         "The Fast Dormancy Dest RRC State for CELL_DCH can only be set to "
                                         "PCH|UPCH|FACH|IDLE")

        self.get_logger().info("Set Fast Dormancy Dest RRC State for CELL_DCH to %s" % state)
        self._visa.send_command("CALL:SERVice:PSData:RRC:TRANsition:SCRI:FDORmancy:DCH:DESTination %s" % state)
    ...
    def set_dut_ip_version(self, ip_num, ip_protocol):
        ...
        if ip_protocol.upper() in ("IP4", "IP6", "DUAL"):
            ...
            self._visa.send_command("%s %s" % (gpib_command, ip_protocol.upper()))
        else:
    ...
    def get_cpc_state(self):
        ...
        """
        cpc_state = ["OFF", "ON"]
        cpc_value = int(self._visa.query_command("CALL:CPC:STAT?"))
        return cpc_state[cpc_value]
    ...
    def get_current_drx_activated_state(self):
        ...
        """
        current_drx_activated_state = ["OFF", "ON"]
        current_drx_activated_value = int(self._visa.query_command("CALL:STATus:CPC:DRX:ACTivated:STATe?"))

        self.get_logger().info("Current DRX Activated state reported is : %s",
                               current_drx_activated_state[current_drx_activated_value])

        return current_drx_activated_state[current_drx_activated_value]
    ...
    def get_current_dtx_activated_state(self):
        ...
        """
        current_dtx_activated_state = ["OFF", "ON"]
        current_dtx_activated_value = int(self._visa.query_command("CALL:STATus:CPC:DTX:ACTivated:STATe?"))

        self.get_logger().info("Current DTX Activated state reported is : %s",
                               current_dtx_activated_state[current_dtx_activated_value])

        return current_dtx_activated_state[current_dtx_activated_value]
    ...
    def send_hsscch_order(self):
        ...
        self._visa.send_command("CALL:CPC:DRX:ORDer 0")
        # Sets the DTX Order Info (deactivation on UL DTX activated state)
        self._visa.send_command("CALL:CPC:DTX:ORDer 0")
        # Perform the Send HS-SCCH Order action.
        self._visa.send_command("CALL:CPC:HSSCchannel:ORDer:SEND")
    ...
    def set_fast_dormancy_timer(self, rrc_inactivity_timer):
        ...
        self._visa.send_command("CALL:SERV:PSD:RRC:TRAN:ITIMer:DCH:DEST IDLE")

        # Increase inactivity timer length to make sure fast dormancy is
        # requested by UE via SCRI timer
        self._visa.send_command("CALL:SERVice:PSData:RRC:TRANsition:ITIMer:DCH:VALue %s" % str(rrc_inactivity_timer))
        ...
        self._visa.send_command("CALL:SERV:PSD:RRC:TRAN:SCRI:FDOR:FACH:DEST IDLE")
        # Increase inactivity timer length to make sure fast dormancy is
        # requested by UE via SCRI timer
        # Set "CELL_FACH Inactivity Timer Length" to the defined value in test case
        self._visa.send_command("CALL:SERVice:PSData:RRC:TRANsition:ITIMer:FACH:VALue %s" % str(rrc_inactivity_timer))

        # Set "SCRI with Fast FDORmancy state" to on
        self._visa.send_command("CALL:SERVice:PSData:RRC:TRANsition:SCRI:FDOR ON")

        # set Even 4B off to RRC so DCH directly goes to IDLE at the expiration of inactive timer
        # instead of going to FACH
        self._visa.send_command("CALL:SERVice:PSData:RRC:TRANsition:TVOL:TRIG:STAT:EVent4B OFF")
        ...
        self._visa.send_command("CALL:BCCH:T323 %s" % str(rrc_inactivity_timer + 10))
    ...
    def get_rrc_states(self):
        ...
        """
        rrc_states = self._visa.query_command("CALL:STATus:RRC:STATe?")
        return rrc_states
    ...
    def get_information_bit_throughput(self):
        ...
        """
        self._visa.send_command("INIT:HBL")
        time.sleep(5)
        measurement_fetch_flag = False
        timer = 20
        while timer >= 0:
            output = self._visa.query_command("INIT:DONE?")
        ...
        if measurement_fetch_flag:
            throughput = round(float(self._visa.query_command("FETC:HBL:IBTH?")), 2)
        else:
    ...
    def set_ip6_network_parameters(self, server_ipv6_address):
        ...
        self._visa.send_command("SYSTem:COMMunicate:LAN:ADDRess:IP6:PREFix '%s'" % str(ipv6_prefix))
        self.get_logger().info("Set IPv6 Default Router Interface ID to %s" % ipv6_interface_id)
        self._visa.send_command("SYSTem:COMMunicate:LAN:DGATeway:IP6:IID '%s'" % str(ipv6_interface_id))
    ...
    def set_dut_ip6_network_parameters(self, server_ipv6_address):
        ...
        self._visa.send_command("CALL:MS:IP:ADDRess:IP6:PREFix '%s'" % str(ipv6_prefix))
        self.get_logger().info("Set DUT IPv6 DNS to %s" %
                               (str(ipv6_prefix) + ':' + str(ipv6_interface_id)))
        self._visa.send_command("CALL:MS:DNSServer:PRIMary:IP:ADDRess:IP6 '%s'" %
                                (str(ipv6_prefix) + ':' + str(ipv6_interface_id)))
    ...
    def configure_ims_reject(self, apn_name):
        ...
        self._visa.send_command("CALL:SERVice:PSData:RAPName:STATe ON, OFF, OFF, OFF, OFF, OFF")
        # Set PDP reject cause to 33 (Requested Service Opt Not Suscribed)
        self._visa.send_command("CALL:SERVice:PSData:RAPName:REJect:CAUSe 33, 33, 33, 33, 33, 33")
        # Set First APN name
        self._visa.send_command("CALL:SERVice:PSData:RAPName1:VAlue CUST")
        self._visa.send_command("CALL:SERVice:PSData:RAPName1:CUSTom '%s'" % apn_name)
