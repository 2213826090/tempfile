...
:summary: cell 2G implementation of Agilent 8960 cellular network simulator
:since: 22/09/2014
:author: jduran4x
"""
...
from acs_test_scripts.Equipment.NetworkSimulators.Cellular.Agilent8960.Tech2G.Data2GVisa import Data2G



class Cell2G(CellCommon, ICell2G):
    ...
    def set_mnc(self, code):
        ...
        band = self._visa.query_command("CALL:BAND?")
        ...
        if band == "PCS":
            ...
            gpib = "MNCode"
            max_code = 99
        if int(code) in range(max_code + 1):
            self._visa.send_command("CALL:%s %d" % (gpib, int(code)))
        else:
            msg = "Invalid mobile network code"
    ...
    def set_band(self, band):
        ...
        self._visa.send_command("CALL:BAND %s" % band)
    ...
    def set_bcch_arfcn(self, arfcn):
        ...
        """
        self._visa.send_command("CALL:BCH %d" % arfcn)
    ...
    def set_cell_service(self, service):
        ...
        :param service: the cell service to set. Possible values:
            ...
            - "EGPRS"
        """
        if service in ("GSM", "GPRS", "EGPRS"):
            self._visa.send_command("CALL:BCH:SCEL %s" % service)
        else:
            msg = "Invalid Cell Service"
            self.get_logger().error(msg)
            raise TestEquipmentException(TestEquipmentException.SPECIFIC_EQT_ERROR, msg)
    ...
    def set_paging_multiframe(self, mrf):
        ...
        """
        if mrf in range(2, 10):
            self._visa.send_command("CALL:PAGING:MFRAMES %d" % mrf)
        else:
            msg = "Invalid multiframe range"
            self.get_logger().error(msg)
            raise TestEquipmentException(TestEquipmentException.SPECIFIC_EQT_ERROR, msg)
    ...
    def set_frequency_hopping_state(self, state):
        ...
        """
        if state in ("ON", "OFF"):
            self._visa.send_command("CALL:TCH:FHOP %s" % state)
        else:
            msg = "Invalid frequency hopping state"
            self.get_logger().error(msg)
            raise TestEquipmentException(TestEquipmentException.SPECIFIC_EQT_ERROR, msg)
    ...
    def set_ms_tx_level(self, tx_level):
        ...
        """
        if tx_level in range(32):
            self._visa.send_command("CALL:MS:TXL %d" % tx_level)
        else:
            msg = "MS Tx Level Out of Range"
            self.get_logger().error(msg)
            raise TestEquipmentException(TestEquipmentException.SPECIFIC_EQT_ERROR, msg)
    ...
    def set_channel_mode(self, chan_mode):
        ...
        :param chan_mode: channel mode to use. Possible values are:
            ...
            - "HRSP"
        """
        if chan_mode in ("FRSP", "EFRS", "HRSP"):
            self._visa.send_command("CALL:TCH:CMOD %s" % chan_mode)
        else:
            msg = "Out of range TCH mode"
            self.get_logger().error(msg)
            raise TestEquipmentException(TestEquipmentException.SPECIFIC_EQT_ERROR, msg)
    ...
    def set_periodic_location_update_timer(self, period):
        ...
        """
        if period in range(256):
            self._visa.send_command("CALL:PPR:LAU:T3212 %d" % period)
        else:
            msg = "Invalid location update period"
            self.get_logger().error(msg)
            raise TestEquipmentException(TestEquipmentException.SPECIFIC_EQT_ERROR, msg)
    ...
    def set_ba_table(self, ba_table, bch_band):
        ...
        if bch_band in self.AGILENT_8960_BANDS.keys() and self.AGILENT_8960_BANDS[bch_band] != "UNSUPPORTED":
            self._visa.send_command("CALL:BA:TABL:%s %s" % (bch_band, ",".join(ba_table)))
        else:
            msg = "Out of range TCH mode"
            self.get_logger().error(msg)
            raise TestEquipmentException(TestEquipmentException.SPECIFIC_EQT_ERROR, msg)
    ...
    def set_ca_table(self, ca_table):
        ...
        """
        self._visa.send_command("CALL:CA:TABL " + ",".join(ca_table))
    ...
    def set_tch_arfcn(self, arfcn):
        ...
        """
        self._visa.send_command("CALL:TCH %d" % arfcn)
    ...
    def set_pdtch_arfcn(self, arfcn):
        ...
        """
        self._visa.send_command("CALL:PDTCH %d" % arfcn)
    ...
    def get_tx_power(self):
        ...
        """
        return self._visa.query_command("FETC:ORFS:POW?")

    def init_orfs_measurement(self):
        """
        Inits ORFS measurement
        """
        self._visa.send_command("INIT:ORFS:ON")
    ...
    def is_dut_registered(self, dut_imsi):
        ...
        dut_imsi = dut_imsi.replace('"', '')
        imsi = self._visa.query_command("CALL:MS:REP:IMSI?")
    ...
    def set_dtm_state(self, state):
        ...
        """
        if state in ("ON", "OFF"):
            self._visa.send_command("CALL:DTM %s" % state)
        else:
            msg = "Invalid dual transfer mode state"
            self.get_logger().error(msg)
            raise TestEquipmentException(TestEquipmentException.SPECIFIC_EQT_ERROR, msg)
    ...
    def execute_external_handover(self):
        ...
        """
        self._visa.send_command("CALL:HAND:EXT")
    ...
    def set_page_response(self, pr):
        ...
        self._visa.send_command("CALL:BCH:UPR %s" % pr)
    ...
    def set_neighbour_parameters(self, earfcn, bd_incld, msbd, prio_incld, prio,
                                 thrsh_high, thrsh_low_incld, thrsh_low, qrx_incld, qrx):
        """
        Specify parameters for neighbour

        :param earfcn: earfcn (0 to 65535)
        :param bd_incld: Bandwidth inclusion state.: 0 (exclude), 1 (include)
        :param msbd: Measurement Bandwidth :0 (6RBs), 1 (15RBs), 2 (25RBs), 3 (50 RBs), 4 (75RBs), 5 (100RBs)
        :param prio_incld: Priority inclusion state: 0 (exclude), 1 (include)
        :param prio: Priority (0 to 7)
        :param thrsh_high: THRESH High (0 to 62)
        :param thrsh_low_incld: THRESH Low inclusion state: 0 (exclude), 1 (include)
        :param thrsh_low: THRESH Low (0 to 62)
        :param qrx_incld: QRXLEVMIN inclusion state: 0 (exclude), 1 (include)
        :param qrx: QRXLEVMIN (-140 to -78)
        """
        self.get_logger().info("Specify and set the neighbour parameters")
        self._visa.send_command("CALL:CELL:EUTRan:TABLe %i,%i,%i,%i,%i,%i,%i,%i,%i,%i "
                                % (earfcn, bd_incld, msbd, prio_incld, prio, thrsh_high,
                                   thrsh_low_incld, thrsh_low, qrx_incld, qrx))
    ...
    def set_equivalent_plmn_list_points(self, mcc, mnc):
        ...
        self._visa.send_command("CALL:PPRocedure:PLMNetwork %d, %d" % (mcc, mnc))
    ...
    def get_equivalent_plmn_list_points(self):
        ...
        output = self._visa.query_command("CALL:PPRocedure:PLMNetwork:POINts?")
    ...
    def set_ps_handover_state(self, ps_state):
        ...
        When the PS handover state is set to ON, the handover of a DUT in packet transfer
        state will maintain packet resources during the handover procedure.
        ...
        self._visa.send_command("CALL:HANDoff:EXTernal:PSWitched %d" % ps_state)
    ...
    def get_utran_neighbor_cell_list(self):
        ...
        cell = self._visa.query_command("CALL:UTRan:FDDuplex:TABLe?").split(",")
        cell_list = [int(float(cell[i])) for i in enumerate(cell)]
    ...
    def set_utran_neighbor_cell_list(self, cell_list):
        ...
        self._visa.send_command("CALL:UTRan:FDDuplex:TABLe %s" % cell_list)
    ...
    def set_utran_neighbor_cell(self,
        ...
        self.get_logger().info("Set 3G Neighbour cell %d parameters:  uarfcn %d, sci %d, sc %d, div %d"
                               % (utran_cell_id, uarfcn, sci, sc, div))
        self._visa.send_command("CALL:UTRan:FDDuplex:TABLe  %d,%d,%d,%d" % (uarfcn, sci, sc, div))

    def set_bcch_network_channel_order(self, ncorder):
        """
        This command sets the Network Control Order parameter.

        The Network Control Order parameter sets the level of control the network has to control measurement reporting
        and cell reselection.
        There are three settings:

            NC0 - allows the mobile station (MS) to perform autonomous cell reselections,
                  which is normal mobile station control.
            ...
            NC2 - The MS sends measurement reports to the network and does not perform autonomous
                cell reselection unless triggered downlink signaling failure or random access failure.
        Network Control Order determines how the MS should perform cell reselections.
        If the Network Control Order has been set to NC0 or NC1 at the test set to which the MS is GPRS attached
        has the Network Control Order set to NC2, autonomous cell reselection may not be performed by the MS as
        a result of invalid BCH configuration.
        :type ncorder: int
        :param ncorder: the Network Channel Order (0,1,2)
        """
        self.get_logger().info("Set Network Control Order to %d" % ncorder)
        self._visa.send_command("CALL:CELL:BCHannel:NCORder %d" % ncorder)
    ...
    def remove_all_gsm_pbcch_neighbor_cells(self):
        ...
        for i in range(1, 33):
            self._visa.send_command(gpib + "%s OFF" % i)
    ...
    def set_lau_reject_gmm_cause(self, cause_id):
        ...
        if cause_id in range(256):
            ...
            self._visa.send_command(gpib + str(cause_id))
        else:
            raise AcsBaseException(AcsBaseException.INVALID_PARAMETER,
    ...
    def set_lau_reject_state(self, state):
        ...
        if state.upper() in ("OFF", "ON"):
            ...
            self.get_logger().info("Setting the LAU reject state to %s"
                                   % state)
            self._visa.send_command(gpib + state.upper())
        else:
            raise AcsBaseException(AcsBaseException.INVALID_PARAMETER,
    ...
    def set_network_mode_of_operation(self, mode):
        ...
        if mode in range(1, 4):
            ...
            self.get_logger().info("Setting the network mode of operation to"
                                   "%s" % str(mode))
            self._visa.send_command(gpib + str(mode))
        else:
            raise AcsBaseException(AcsBaseException.INVALID_PARAMETER,
