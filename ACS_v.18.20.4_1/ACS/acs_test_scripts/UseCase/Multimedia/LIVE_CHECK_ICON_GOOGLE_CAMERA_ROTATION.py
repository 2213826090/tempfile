...
:summary: This module implements check of icon in google camera application with rotation
:since: 29/09/14
:author: jcoutox
"""
...
class LiveCheckIconGoogleCameraRotation(UseCaseBase):
    """
    ...
    def __init__(self, tc_name, global_config):
        ...
        self.__icon_dict_text = str_to_dict(self._tc_parameters.get_param_value("ICON_DICT_TEXT"))
        self.__icon_dict_id = str_to_dict(self._tc_parameters.get_param_value("ICON_DICT_ID"))
    ...
    def run_test(self):
        ...
        for icon in self.__icon_dict_text:
            verdict = self._multimedia_api.check_screen(self.__icon_dict_text[icon], "text")
            time.sleep(self._wait_btwn_cmd)

            if not verdict:
                msg += "Doesn't match " + str(icon) + " in UI for first orientation,"

        for icon in self.__icon_dict_id:
            verdict = self._multimedia_api.check_screen(self.__icon_dict_id[icon], "id")
            time.sleep(self._wait_btwn_cmd)
        ...
        self._display_api.set_display_orientation(self.__rotation)
        time.sleep(self._wait_btwn_cmd)
        ...
        for icon in self.__icon_dict_text:
            ...
            if not verdict:
                msg += "Doesn't match " + str(icon) + " in UI for second orientation,"
