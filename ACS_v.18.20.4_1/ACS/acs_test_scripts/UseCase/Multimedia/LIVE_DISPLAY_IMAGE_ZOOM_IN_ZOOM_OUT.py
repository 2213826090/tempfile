...
:summary: This file implements use case to display image and zoom in then zoom out
:since: 12/02/2014
:author: mmorchex
"""
...
class LiveDisplayImageZoomInZoomOut(UseCaseBase):

    """
    Class Display Image Zoom In Zoom Out.
    """
    ...
    def __init__(self, tc_name, global_config):
        ...
        # get REFERENCE_SWIPE from TC parameters
        self.__ref_image_swipe = self._tc_parameters.get_param_value("REFERENCE_IMAGE_SWIPE")
...
#------------------------------------------------------------------------------
    def run_test(self):
        ...
        screen_shot_before_zoom_in_uri = self.__image_api.take_screenshot_and_pull_on_host("first_screen_shot",
                                                                                           os.getcwd())
        ...
        self.__key_event_api.swipe((screen_shot_width / 2), (screen_shot_height / 2), 500, 1)
        time.sleep(self.__time_for_display)

        # TaKe a screen shot after swipe
        self._logger.info("Taking screen shot after swipe...")
        screen_shot_after_swipe_uri = self.__image_api.take_screenshot_and_pull_on_host("screen_shot_after_swipe",
                                                                                    os.getcwd())
        ...
        self._logger.info("Taking screen shot ...")
        zoom_out_screen_shot_uri = self.__image_api.take_screenshot_and_pull_on_host("zoom_out_screen_shot",
                                                                                     os.getcwd())
        ...
        result = Imagecheck.match_template_in_image(screen_shot_before_zoom_in_uri, zoom_out_screen_shot_uri, 0.9)
        if not result:
            verdict = Global.FAILURE
            msg += "Zoom out did not match, "

        # Check that picture after swipe is correctly displayed
        result = Imagecheck.match_template_in_image(self.__dic_image[self.__ref_image_swipe],
