...
from acs_test_scripts.Utilities.IPerfUtilities import compute_iperf_verdict, get_iperf_configuration, \
    retrieve_parameters_from_tc_params, get_random_iperf_port, parse_iperf_options
...
class LabHspaIperf(LabHspaBase):
...
# ------------------------------------------------------------------------------
    ...
    def set_up(self):
        ...
        if self._tune_options:
            self._iperf_settings.update(get_iperf_configuration(self._throughput_targets, self._iperf_direction))
        else:
    ...
    def run_test(self):
        ...
        try:
            ...
            if self._kpi_test:
                ...
                result_msg = "KPI test Iteration: %d " % (self._current_iteration + 1) + result_msg_tmp
            else:
