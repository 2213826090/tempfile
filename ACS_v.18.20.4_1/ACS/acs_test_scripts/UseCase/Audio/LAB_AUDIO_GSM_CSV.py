...
class LabAudioGsmCsv(LabAudioGsmVcBase):
    """
    Lab Audio Routing Gsm Voice Call Mobile Originated/Terminated class.
    """
    ...
    def run_test(self):
        ...
        while self._elapsed_time < self._call_duration:
            ...
            # Set voice codec using CODEC parameter (To ensure the reliability of the measurement with CMU200)
            self._vc_2g.set_audio_codec(self._codec)
            ...
            if self._audio_analyzer_node.get_param_value("Model") in "RS_UPV":
                ...
                if self._keep_record:
                    ...
                    self._host_trace_file_path_dl = self._host_trace_file_path_dl.strip(
                        self._host_trace_file_path_dl.split('_')[-1]) + \
                                                    str(
                                                        self._tc_parameters.get_b2b_iteration() - self._tc_iteration_count + 1) \
                                                    + ".trc"
                    self._host_trace_file_path_ul = self._host_trace_file_path_ul.strip(
                        self._host_trace_file_path_ul.split('_')[-1]) + \
                                                    str(
                                                        self._tc_parameters.get_b2b_iteration() - self._tc_iteration_count + 1) \
                                                    + ".trc"
                else:
            ...
            self._vc_2g.is_voice_call_connected(self._wait_between_measure)
