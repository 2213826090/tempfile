...
class LabAudioQualitySwapCall(UseCaseBase):
    """
    AudioComms Audio Swap Call class.
    """
    def __compute_test_verdict__(self, audio_analyzer_result_ul, audio_analyzer_result_dl):

        audio_analyzer_result = audio_analyzer_result_ul or audio_analyzer_result_dl
        if audio_analyzer_result_ul != 0 and audio_analyzer_result_dl != 0:
            self._signal_tested_direction = "ul and dl"
        if audio_analyzer_result_ul != 0:
            self._signal_tested_direction = "ul"
        if audio_analyzer_result_dl != 0:
            self._signal_tested_direction = "dl"
    ...
    def run_test(self):
        ...
        call_type_audio_analyser = self._call_type_list_audio_analyser_run.pop()
        if call_type_audio_analyser == "VOIP":
            self._system_api2.adjust_specified_stream_volume("VoiceCall",
                                                            (self._call_stream_volume_ref +
        ...
        l_audio_analyzer_result_ul = self._audio_analyzer.run(call_type_audio_analyser,
                                                              self._acc_type,
                                                              "ul")

        l_audio_analyzer_result_dl = self._audio_analyzer.run(call_type_audio_analyser,
                                                              self._acc_type,
                                                              "dl")

        # Compute test verdict
        self.__compute_test_verdict__(l_audio_analyzer_result_ul, l_audio_analyzer_result_dl)
        ...
        while self._number_of_swap > 0:
            ...
            self._previous_call_type_save = self._call_type_save
            self._call_type_save = call_type_audio_analyser
        ...
        # For resume the call
        end_call = self._call_end_type_list.pop()
...
#        time.sleep(self._wait_btwn_cmd)
        ...
        if self._previous_call_type_save == "VOIP" and self._call_type_save == "VOIP":
            self._ui_api.run_operation_set("hold_call")
            self._ui_api2.run_operation_set("hold_call")
        elif end_call == "MR":
            self._ui_api2.run_operation_set("hold_call")
        else:
            self._ui_api.run_operation_set("hold_call")
        ...
        if self._audio_analyzer_result_save == 0:
            self._verdict_comment = ("Audio Quality test success with %s call sequence"
                                     % (str(self._tc_parameters.get_param_value("CALL_SEQUENCE"))))
        elif self._audio_analyzer_result_save == 1:
            self._verdict_comment = ("Audio Quality test fail: Quality audio "
            ...
            self._verdict_comment = ("Audio Quality test fail: Exception in executable "
                                     "in %s with %s call sequence"
                                     % (self._signal_tested_direction,
                                        self._failed_transition_no_audio))
        else:
            self._verdict_comment = ("Audio Quality test fail: Unknow problems detected "
