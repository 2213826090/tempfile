...
class CameraTimeToPreview(UseCaseBase):
    ...
    def run_test(self):
        ...
        res, out = self._device.run_cmd("adb shell uiautomator runtest Performance.jar -c other.StartCamera", 120)

        out = self.runBashCommand("adb logcat -d -v time | grep \"START\ u0\|startDevice\"")
        rawtimestamps = []
        ...
        timestampsPair = []
        timestampsPair.append(rawtimestamps[0])
        timestampsPair.append(rawtimestamps[-1])


        numericTimestamp = re.findall( r'[0-9]+', timestampsPair[0])
        ...
        numericTimestamp = re.findall( r'[0-9]+', timestampsPair[1])
        ...
        avg = (finalTimestamp - initialTimestamp)

        self._device.get_logger().info("Result Camera to preview launch time : %s MEDIAN", str(float(avg)/1000))
