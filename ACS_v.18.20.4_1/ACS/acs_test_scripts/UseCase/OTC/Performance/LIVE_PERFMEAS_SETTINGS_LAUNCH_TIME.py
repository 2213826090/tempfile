...
class SettingsLaunchTime(UseCaseBase):
    ...
    def get_score(self, stat_type="MEDIAN"):
        ...
        for key in keys:
            ...
            if key == "score":
                self._device.get_logger().info("Result Settings score : %s (%s)"% (result, stat_type))
        return xmlresult
    ...
    def run_test(self):
        ...
        :return: tuple of Verdict and comment
        """
        #Launch Settings as user
        self._device.run_cmd("adb shell uiautomator runtest Performance.jar -c other.SettingsLaunch", 120)

        #Filter the output
        res, out = self._device.run_cmd("adb logcat -d | \
                        grep \"Displayed com.android.settings/.Settings\" | cut -d ' ' -f5-", 5)
