...
class ExecApiTests(UseCaseBase):
    ...
    def __internal_exec(self, command, timeout, expected_result):
        ...
        try:
            ...
            test_script_dir = os.path.normpath(os.path.join(working_dir, os.path.dirname(args[1])))
            test_script_path = os.path.normpath(os.path.join(working_dir, args[1]))

            self._test_script_logger.info("changing working dir to: " + test_script_dir)
            os.chdir(test_script_dir)
            sys.path.append(test_script_dir)

            self._test_script_logger.info("will execute test script: " + test_script_path)

            def exec_test_script(msg_queue):
                try:
                    # arg[1] is the python file path
                    # ACS_LOGGER to be passed to PyUiApiTests through execfile()
                    test_outcome = False
                    ACS_LOGGER = self._test_script_logger
                    test_locals = {"ACS_LOGGER": ACS_LOGGER}
                    execfile(test_script_path, test_locals)
                    if "test_outcome" in test_locals.keys():
                        test_outcome = test_locals["test_outcome"]
                    ACS_LOGGER.info("test outcome is: " + str(test_outcome))
                    msg_queue.put(test_outcome)
                finally:
                    os.chdir(current_dir)

            test_proc = Process(target=exec_test_script, args=(msg_queue,))
            test_proc.start()
            timeout = float(timeout)
            while timeout > 0 and test_proc.is_alive():
            ...
            # Get return code
            return_code = test_proc.exitcode

            if timeout > 0:
                test_passed = msg_queue.get(timeout=3)
                self._test_script_logger.info("test_passed is: " + str(test_passed))
            # Get stdout & stderr
            # TO DO:
            # f_stdout.seek(0)
            # stdout = proc
