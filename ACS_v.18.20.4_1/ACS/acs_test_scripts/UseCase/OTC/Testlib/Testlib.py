...
class Testlib_UseCase(UseCaseBase):
    ...
    def initialize(self):
        ...
        os.environ['uiautomator_jars_path'] = os.path.normpath(os.path.join(Paths.TEST_SCRIPTS, "Lib", "PythonUiautomator"))
    ...
    def _run_process(self, test, serial, timeout,  **kwargs):
        self.testlib_path = '/'.join(Paths.TEST_SCRIPTS.split('/')[0:-1])

        if not os.environ.has_key("LOG_PATH"):
        ...
        if os.environ.has_key("PYTHONPATH") and os.environ['PYTHONPATH']:
        ...
        # as mentioned below, the spaces in net_app_ssid were replaced with three underscores (net_ap_ssid = net_ap_ssid.replace(" ", "___"))
        # in order to prevent the name from being split and incorrectly introduced in the command script args
        # at this stage we reconstruct the command with script args so that ___ is replaced back with white space
        subp = subprocess.Popen(command_list, stdout = subprocess.PIPE,
    ...
    def run_test(self):
        ...
        adb_server_port = self._tc_parameters.get_param_value("adb-server-port",
                                                              default_value = None)
        adb_local_port = self._tc_parameters.get_param_value("adb-local-port",
                                                             default_value = None)
        ...
        try:
            ...
            self._logger.warning("Invalid parameter for AccessPoint in Bench Config. "
        ...
        try:
            ...
            self._logger.warning("'IP' or 'username' parameter missing for 'AccessPoint. Please update the AP Equipment with: "
            ...
            if net_ap_password:
                if script_args:
                    if "net_ap_password" in script_args:
                        ...
                        script_args = re.sub(r"net_ap_password=[.^\s]+", r"net_ap_password=" + net_ap_password, script_args)
                    else:
            ...
            if net_ap_security:
                if script_args:
                    if "net_ap_security" in script_args:
                        ...
                        script_args = re.sub(r"net_ap_security=[-\w]+", r"net_ap_security=" + net_ap_security, script_args)
                    else:
        ...
        if android_build_top:
            ...
            if script_args:
                if "android_build_top" in script_args:
                    script_args = re.sub(r"android_build_top=[-\w]+", r"android_build_top={}".format(android_build_top), script_args)
                else :
