...
class LabDutRebootMbEnumeration(UseCaseBase):
...
#------------------------------------------------------------------------------
    ...
    def run_test(self):
        ...
        rebooted = self._device.reboot(self._settledown_duration+30 )
        if rebooted:
        ...
        if mode_before_reboot == current_mode:
                self._logger.info ("Board rebooted successfully and Mobile Broadband before reboot is %s and after reboot is %s" % (mode_before_reboot, current_mode))
                return_code = Global.SUCCESS
                return_message = "Board rebooted successfully and Mobile Broadband status is detected after reboot is %s" % current_mode
