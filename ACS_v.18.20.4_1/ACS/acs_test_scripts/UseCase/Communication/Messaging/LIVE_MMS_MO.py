...
:summary: Use Case Live MMS Mobile Originated
:since: 13/03/2013
:author: rbertolx
"""
...
class LiveMmsMo(LiveMessagingBase):
    ...
    def set_up(self):
        ...
        if self._mms_type.upper() in MMS_ATTACH_TYPE_LIST:
            # Checks if the attachment file exist.
            self._phonesystem_api.check_file_exist_from_shell(self._attachment_file)
    ...
    def run_test(self):
        ...
        # Wait before sending the MMS.
        time.sleep(self._wait_btwn_cmd)

        self._mms_api.check_mms_sent(self._destination_number, self._send_mms_timeout)

        return (Global.SUCCESS, "MMS send in %s secs."
                % self._mms_api.time_to_send_mms)
