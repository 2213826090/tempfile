...
from acs_test_scripts.TestStep.Device.System.Logs.SysDebugReportToContext import SysDebugReportToContext

DEFAULT_SAVE_AS = "my_var"
class test_SysDebugInit(test_SysDebugBase):
    ...
    def test_init_call_ok(self):
        self._create_sut({"SYSDEBUG_REPORT_DEST": DEFAULT_SAVE_AS})
        self._sut.run(self._context)
        self._sut._sysdebug_apis.report.assert_called_once_with()

    def _create_sut(self, test_step_pars=None):
        """
        Create the SUT with only test step pars
        """
        self._sut = SysDebugReportToContext(None, None, test_step_pars, mock.MagicMock())
