...
from acs_test_scripts.TestStep.Device.Wireless.Wifi.WifiSetPower import WifiSetPower
from UtilitiesFWK.Utilities import TestConst

GET_PWR_ON = 1
GET_PWR_OFF = 0
POWER_PARAM = "POWER"
class WifiSetPowerTest(UTestTestStepBase):
    """
    SetPower test cases
    """
    ...
    def test_set_power_on_ok(self):
        ...
        sut = self._create_sut({POWER_PARAM: power, "MYPARAM2":"mirko"})
    ...
    def test_set_power_on_ko(self):
        ...
        self._assert_run_throw_device_exception(sut, "Set WIFI %s failure" % power)
    ...
    def test_set_power_on_off_on_ok(self):
        ...
        self._track_set_wifi_power_calls(sut)
    ...
    def _track_set_wifi_power_calls(self, sut):
        """
        Save all the parameters set_wifi_power was called with in order to check every calls went well.
        """
        sut._api.set_wifi_power.side_effect = lambda x:self.calls.append(x)
    ...
    def _simulate_get_power_returning(self, sut, set_power, expected_power):
        ...
        sut._api.get_wifi_power_status.side_effect = self._side_effect_get_power_response
        return sut

    def _assert_run_returned(self, sut, expected):
        sut.run(None)
        sut._api.set_wifi_power.assert_called_once_with(expected)
    ...
    def _create_sut(self, test_step_pars=None):
        ...
        """
        sut = WifiSetPower(None, None, test_step_pars, mock.Mock())
        sut._timeout = 0
        return sut
