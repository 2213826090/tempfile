...
from acs_test_scripts.TestStep.Device.Wireless.Cellular.CellularSetPower import CellularSetPower
from UtilitiesFWK.Utilities import TestConst

GET_PWR_ON = 1
GET_PWR_OFF = 0
POWER_PARAM = "POWER"
class CellularSetPowerTest(UTestTestStepBase):
    """
    SetPower test cases
    """
    ...
    def test_set_power_off_afer_several_times_ok(self):
        power = TestConst.STR_OFF
        sut = self._create_sut({POWER_PARAM: power})
        self._simulate_get_power_returning(sut, power, [GET_PWR_ON, GET_PWR_ON, GET_PWR_OFF])
        sut._timeout = 5
        self._assert_run_returned(sut, power)
    ...
    def test_set_power_on_ko(self):
        ...
        self._assert_run_throw_device_exception(sut, "Set CELLULAR %s failure" % power)
    ...
    def test_set_power_on_off_on_ok(self):
        ...
        self._track_set_cellular_power_calls(sut)
    ...
    def _track_set_cellular_power_calls(self, sut):
        """
        Save all the parameters set_cellular_power was called with in order to check every calls went well.
        """
        sut._modem_api.set_modem_power.side_effect = lambda x:self.calls.append(x)
    ...
    def _simulate_get_power_returning(self, sut, set_power, expected_power):
        ...
        sut._modem_api.get_modem_power_status.side_effect = self._side_effect_get_power_response
        return sut

    def _assert_run_returned(self, sut, expected):
        sut.run(None)
        sut._modem_api.set_modem_power.assert_called_once_with(expected)
    ...
    def _create_sut(self, test_step_pars=None):
        ...
        """
        sut = CellularSetPower(None, None, test_step_pars, mock.Mock())
        sut._timeout = 0
        return sut
