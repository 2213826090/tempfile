...
class ReplayAction(UiAction):
    ...
    def _do_process_parameters(self):
        ...
        """
        path = self._parameters[0]
        if not os.path.isfile(path):
            path = os.path.normpath(os.path.join(os.getcwd(),
                                                 "_ExecutionConfig",
                                                 path))

        self.__scritp_path = path
    ...
    def _do(self):
        ...
        if os.path.isfile(self.__scritp_path):
            # Upload file to /sdcard
            embedded_path = "/sdcard/replay/"
            cmd = "adb shell mkdir %s" % embedded_path
            self.__device_instance.run_cmd(cmd, 5)

            target_script_path = "%s%s" % (embedded_path,
                                           os.path.basename(self.__scritp_path))
            self.__device_instance.push(self.__scritp_path,
                                        target_script_path,
                                        5)

            # Start playback
            self.__device_instance.run_cmd("adb shell setprop acs.ui_recorder.file %s" %
                                           target_script_path, 2)
            self.__device_instance.run_cmd("adb shell setprop acs.ui_recorder.replay_run start",
                                           2)

            self._set_result(Global.SUCCESS)
            return Global.SUCCESS
        else:
            self._set_result(Global.FAILURE)
