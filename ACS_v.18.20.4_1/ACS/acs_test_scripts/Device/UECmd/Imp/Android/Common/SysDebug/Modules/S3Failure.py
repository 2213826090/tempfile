...
class S3Failure(SysDebug):
    ...
    def __toxml(self, dicStats):
        ...
        :param dicStats: Statistic representation of informations.
                         Each item is a dictionary as follwing:
                             "failure" : {"device": pci_info,
                                          "reason": reason of the fail}

        :rtype: etree.Element
        :return: The Xml tree vue of statistic results of the module
        """
        xmltree = etree.Element("S3Failures")
        for failure in dicStats:
            current = etree.Element("failure")
            current.attrib["device"] = failure["device"]
            current.attrib["date"] = failure["date"]
            current.attrib["reason"] = failure["reason"]
