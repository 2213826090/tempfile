...
class TouchAction(UiAction):
    ...
    def _do_process_parameters(self):
        ...
        if isinstance(x_coordinate, str) and x_coordinate.startswith("[RAND("):
            self._x = RandomRange.get_value_from_range(x_coordinate)
        else:
            self._x = int(x_coordinate)
        ...
        if isinstance(y_coordinate, str) and y_coordinate.startswith("[RAND("):
            self._y = RandomRange.get_value_from_range(y_coordinate)
        else:
            self._y = int(y_coordinate)
        # Update the timeout
    ...
    def _do(self):
        ...
        result = self.get_monkey().touch(self._x, self._y, self._default_timeout)
