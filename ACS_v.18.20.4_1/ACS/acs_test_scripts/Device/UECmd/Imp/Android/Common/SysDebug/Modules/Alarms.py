...
class Alarms(SysDebug):

    """
    :summary: System UEcommands operations for Android platforms
              Fetching SysDebug informations on alarms
    """
    ...
    def __init__(self, device, _config=None):
        ...
        self._trigger_group = [["intent", "AlarmManager\s*:\s*triggered\s*:\s*((act)|(cmp))=", "[*\\/@.\\$a-zA-Z0-9-:_]+", "\s*.*"],
                               ["pkg", "Pkg:\s*", ".*", "$"]]
        self._aplog_level = "V"
    ...
    def __toxml(self, dicStats):
        ...
        """
        xmltree = etree.Element("Alarms")
        for alarm in dicStats:
            current = etree.Element("alarm")
            current.attrib["intent"] = alarm["intent"]
            current.attrib["pkg"] = alarm["pkg"]
            current.attrib["date"] = alarm["date"]
    ...
    def stats(self, start, stop):
        """
        Return alarms statistics which have been triggered between
        ...
        self._logger.debug("Alarm stats : %s" % self._sys_debug_infos)
