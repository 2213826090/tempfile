...
class EmModule(IEmModule, DeviceModuleBase):
    def __init__(self):
        super(EmModule, self).__init__()
    ...
    def parse_msic_response_from_shell(self, output, log_output=True):
        ...
        # check charger extra info
        charger_extra_info = document.findtext("CHARGER_EXTRA")
        if charger_extra_info is not None:
            value = str(charger_extra_info).split(":")[-1].strip()
            if value.isdigit():
                charger["MAX_CHARGING_CURRENT"] = (float(value) / 1000, "A")
        else:
            tmp_txt += "parsing charger extra info contains no information."
