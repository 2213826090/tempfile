...
class MerrifieldDeviceChecksModule(IDeviceChecksModule, DeviceModuleBase):

    def __init__(self):
        super(MerrifieldDeviceChecksModule, self).__init__()
    ...
    def get_mmio_reg_address(self, reg_name=None):
        ...
        return MRFD_MMIO_REG_DICT.get(reg_name)
    ...
    def check_pipe_hang(self):
        ...
        if (not _mode_en_bit_b == 0) and (not reg_val_b == 0xffffffff):
            ...
            for i in range(1,5):
                if (self.read_mmio_reg_32(reg_address=reg_addr_value) <> orig_reg_value):
                    hung_b = False
                    status ="%s , PipeB is not hung "%(status)
                    break
                else:
                    status = "%s , PipeB is hung "%(status)
