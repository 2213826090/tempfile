...
from acs_test_scripts.Device.Module.SSG.Flash.FlashManager.FlashManager import FlashManager
from acs_test_scripts.Device.Module.SSG.Flash.FlashManager.FlashDeviceInfo import FlashDeviceInfo
from UtilitiesFWK.Utilities import AcsConstants
from UtilitiesFWK.Utilities import Global


class GminFlashModule(IFlashModule, DeviceModuleBase):
    ...
    def __init__(self):
        super(GminFlashModule, self).__init__()
    ...
    def _collect_flash_device_info(self):
        ...
        # transfer flash configuration characteristics
        self._device_info.flash_configuration = ""
        self._device_info.flash_configuration_type = ""
        flash_configuration = self.configuration.get_value("flashConfiguration")
        flash_configuration_type = self.configuration.get_value("flashConfigurationType")
        if flash_configuration and flash_configuration_type:
            self._device_info.flash_configuration = flash_configuration.strip()
            self._device_info.flash_configuration_type = flash_configuration_type.strip()
        elif not flash_configuration and flash_configuration_type\
             or flash_configuration and not flash_configuration_type:
            self.logger.warning("Not valid combination for: Flash Configuration/Flash Configuration Type : %s/%s"%
                                (flash_configuration, flash_configuration_type))
            self.logger.warning("This parameters will be ignored and the functionality enabled by them will be "
                                "bypassed to continue the flashing procedure")
    ...
    def flash(self, timeout):
        ...
        if self._device_info.adb_over_ethernet and multiple_devices \
                and ip_address == pos_ip_adress and ip_address != "192.168.42.1" \
                or not self._device_info.adb_over_ethernet:
