...
from acs_test_scripts.Device.Module.NDG.Flash.FlashManager.Tool.FlashToolE2 import FlashToolE2
from acs_test_scripts.Device.Module.NDG.Flash.FlashManager.Tool.EdisonFlashTool import EdisonFlashTool
from acs_test_scripts.Device.Module.NDG.Flash.FlashManager.Tool.SensorEnabledDeviceFlashTool import SensorEnabledDeviceFlashTool
...
class FlashManager(object):
    ...
    Using flash tools supported by ACS (for instance: FTE2, EFT, SED_FT, ...)
    """
    # Define the list of flash tool supported in ACS
    BIOS_FLASH_TOOL_LIST = {}

    OS_FLASH_TOOL_LIST = {"FTE2": FlashToolE2,
                          "EFT": EdisonFlashTool,
                          "SED_FT": SensorEnabledDeviceFlashTool}
    ...
    def __init__(self, conf, logger):
        ...
        """
        self._os_flash_tool = conf.get_value("OSflashTool", "")
        self._allow_flashing_bios = conf.get_value("flashBIOS", "False", "str_to_bool")
        self._bios_flash_tool = conf.get_value("BIOSflashTool", "DEDIPROG")
        self._voltage = conf.get_value("voltage", "1.8V")
        self._flash_user_data = conf.get_value("flashUserData", "True", "str_to_bool")
        self._flash_options = conf.get_value("flashOptions", "")
