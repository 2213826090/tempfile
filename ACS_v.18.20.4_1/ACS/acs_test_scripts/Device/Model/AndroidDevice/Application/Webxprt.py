...
@organization: PSTV
@summary:  This script implements WebXPRT(HTML5)
@since: 12/08/2014
@author: sammathx
"""
import os
...
class Webxprt(IBrowsing):
    ...
    def generateLogcat(self):
        ...
        time.sleep(1)
        a=subprocess.Popen("adb shell \"su -c logcat -v time > /storage/sdcard0/logcatwebprt.log \"", shell=True)
        time.sleep(3)
        process_id=a.pid
    ...
    def __get_screenshot(self):
        ...
        self.__browser_mark_report_path = os.path.join(report_dir.get_report_path(), "webxprt_result")
        filename = self.__browser_mark_report_path+ "/" + "Webxprt%s.png" % time.ctime().replace(':', '-')
    ...
    def wait(self, timeout):
        ...
        time.sleep(5)
        overall_score=self.__device.run_cmd('adb shell cat /storage/sdcard0/logcatwebprt.log | grep "score" ', TIMEOUT)
        ...
        self._logger.info('adb shell \"su -c rm -rf /storage/sdcard0/logcatwebprt.log \"', TIMEOUT)
    ...
    def drive(self):
        ...
        time.sleep(20)
        self.__device.run_cmd('adb shell input text pilot', TIMEOUT)
        self.__device.run_cmd('adb shell input keyevent KEYCODE_TAB', TIMEOUT)
        self.__device.run_cmd('adb shell input text mosaic', TIMEOUT)
        self.__device.run_cmd('adb shell input keyevent KEYCODE_TAB', TIMEOUT)
