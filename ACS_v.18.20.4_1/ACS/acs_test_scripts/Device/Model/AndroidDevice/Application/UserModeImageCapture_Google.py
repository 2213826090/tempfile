...
class UserModeImageCapture_Google(IApplication):
    ...
    def start(self):
        ...
        """
        self._logger.debug("UserModeImageCapture - start begins")
        self._camera.launch()

        # take a screenshot
        filename = self.__umic_report_path + "/" + time.strftime("%Y%m%d.%H%M") + ".png"
        self._device.screenshot(filename=filename)
        self._logger.debug("UserModeImageCapture - start ends")
    ...
    def install(self, appuri, additionnals=None, arguments=None, destination=None):
        ...
        self._camera = self._device.get_uecmd("Camera")
