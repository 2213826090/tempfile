...
from acs_test_scripts.Device.Model.AndroidDevice.Application.Sunspider import Sunspider
from acs_test_scripts.Device.Model.AndroidDevice.Application.IApplication import IApplication


class SunspiderOTC(Sunspider):
    ...
    def __init__(self, device):
        ...
        self._sunspider_pattern = "Sunspider result: (?P<result>[\d\.]*)"
        self._results = {"score": []}
    ...
    def wait(self, timeout):
        ...
        if match_res:
            res = match_res.group(0).split(":")[1]
            self._results["score"].append(float(res))
        else:
            raise DeviceException(DeviceException.TIMEOUT_REACHED,
